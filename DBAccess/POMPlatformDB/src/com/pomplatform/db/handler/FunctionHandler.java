...
public class FunctionHandler implements GenericDao {
	...
	public static boolean isFunctionExists( com.pomplatform.db.bean.BaseFunction bean, String additional ) throws Exception {
	...
	public static int countFunction( com.pomplatform.db.bean.BaseFunction bean, String additional ) throws Exception {
	...
	public static BaseCollection<BaseFunction> queryFunction( com.pomplatform.db.bean.BaseFunction bean, int pageNo, int pageLines, String additionalCondition ) throws Exception {
		Function dao = new Function();
		setConditions(bean, dao);
	...
	public static BaseFunction addUpdateFunction ( BaseFunction function, boolean singleTransaction  ) throws Exception {
		if(function.getFunctionId() == null) return addToFunction(function);
		if(singleTransaction) ThreadConnection.beginTransaction();
	...
	public static int setDeleteConditions(BaseFunction bean, Function dao){
		...
		if(!foundKey) {
			...
			if(bean.getSecurityLevel() != null) {
				dao.setConditionSecurityLevel("=", bean.getSecurityLevel());
				count++;
			}
			if(bean.getRiskTip() != null) {
				dao.setConditionRiskTip("=", bean.getRiskTip());
				count++;
			}
		}
		return count;
	...
	public static int setConditions(BaseFunction bean, Function dao){
		...
		if(bean.getRiskTip() != null) {
			if(bean.getRiskTip().indexOf("%") >= 0)
				dao.setConditionRiskTip("like", bean.getRiskTip());
			else
				dao.setConditionRiskTip("=", bean.getRiskTip());
			count++;
		}
