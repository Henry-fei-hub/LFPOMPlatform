...
public class EmployeeHandler implements GenericDao {
	...
	public static boolean isEmployeeExists( BaseEmployee bean, String additional ) throws Exception {
	...
	public static int countEmployee( BaseEmployee bean, String additional ) throws Exception {
	...
	public static BaseCollection<BaseEmployee> queryEmployee( BaseEmployee bean, int pageNo, int pageLines, String additionalCondition ) throws Exception {
		Employee dao = new Employee();
		setConditions(bean, dao);
		dao.setCurrentPage(pageNo);
		dao.setPageLines(pageLines);
		List<BaseEmployee> result = dao.conditionalLoad(additionalCondition);
	...
	public static int setDeleteConditions(BaseEmployee bean, Employee dao){
		...
		if(!foundKey) {
			...
			if(bean.getBankCardAttachment() != null) {
				dao.setConditionBankCardAttachment("=", bean.getBankCardAttachment());
				count++;
			}
			if(bean.getBankCardAttachmentId() != null) {
				dao.setConditionBankCardAttachmentId("=", bean.getBankCardAttachmentId());
				count++;
			}
		}
		return count;
	...
	public static int setConditions(BaseEmployee bean, Employee dao){
		...
		if(bean.getBankCardAttachment() != null) {
			if(bean.getBankCardAttachment().indexOf("%") >= 0)
				dao.setConditionBankCardAttachment("like", bean.getBankCardAttachment());
			else
				dao.setConditionBankCardAttachment("=", bean.getBankCardAttachment());
			count++;
		}
