define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient', 'dl/msgbox', 'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojnavigationlist',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource','ojs/ojarraytabledatasource'],
        function (oj, ko, $, dl ,sc) {
            function doingViewModel(params) {
                var self = this;
                self.viewModelName = 'doingViewModel';
                ...
                self.iClicked = function (data, event) {
                    ...
                    //行政页面数组  定义人事页面数组 物品领用申请、用车申请申请、名片申请申请、刻章申请、采购申请、薪资发放申请 
                    //请假、非项目出差,项目出差申请、打卡申请、外出申请、加班申请、项目派驻申请、离职、转正、转岗   员工还款 网络权限 外部公司还款   项目报备 工资异常  40网络权限隐藏 '25','29',隐藏
                    var totalTypes=['13', '14','15','16','17', '18', '19', '20','21','26','27','28','30','31','33', '34', '36','37','41'];
                    //运营流程    //项目报备 /前期项目关闭 '42','43',隐藏
...
//                      我的工时
                    ...
                    self.rowData = d;
                    //保存页面路径
                    dl.pushPageStack({name: 'doing', params: self.parentViewModel, cacheKey: 'doing'});
...
//                  41外部公司还款 暂时隐藏,45
                ...
                self.datareceived = function (data) {
                    ...
                    for (var i = 0; i < self.menuData.length; i++) {
                        if (self.menuData[i].activityType == 1) {
//                           1进行中
                        typeId = self.menuData[i].processType;
