define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient', 'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojnavigationlist',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource', 'ojs/ojcollectiontabledatasource'],
        function (oj, ko, $, dl) {
            function myApplyBudgetManagementListViewModel(params) {
                var self = this;
                self.viewModelName = 'myApplyBudgetManagementListViewModel';
                ...
                self.iClicked = function (data, event) {
                    ...
                    dl.pushPageStack({name: 'reimbursement/myApplyBudgetManagementList', params: self.parentViewModel});
                    dl.getGlobalModule({name: pageName, params: self});
                    dl.pushHeader(dl.headerTitle());
                    dl.headerTitle(dl.getLabelByValue('employees', d.employeeId) + d.businessName);
                };

                myApplyBudgetManagementListModel = oj.Model.extend({
                    parse: dl.parseModel,
                    idAttribute: 'processInstanceId'
                });

                myApplyBudgetManagementListCollection = oj.Collection.extend({
                    model: new myApplyBudgetManagementListModel(),
                    parse: dl.parseCollection,
                    fetchSize: 10,
                    customPagingOptions: dl.parsePaging,
                    queryAction: 'NQ_BudgetManagementApplyWork',
                    url: dl._SERVERURL
                });
                self.collection = new myApplyBudgetManagementListCollection();
                    ...
                    switch (Number(activityType)) {
                        case 1:
                            self.searchData({
                                currentPage: 0,
                                pageLines: 1,
                                bdApplyEmployeeId: dl.getEmployeeId(),
            ...
            return myApplyBudgetManagementListViewModel;
        }
);
