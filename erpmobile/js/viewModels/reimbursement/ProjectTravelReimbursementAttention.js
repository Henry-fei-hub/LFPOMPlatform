define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient', 'ojs/ojknockout', 'promise', 'ojs/ojselectcombobox', 'ojs/ojnavigationlist',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource','ojs/ojarraytabledatasource'],
        function (oj, ko, $, dl) {
            function ProjectTravelReimbursementAttentionViewModel(params) {
                var self = this;
                self.viewModelName = 'ProjectTravelReimbursementAttentionViewModel';
                ...
                self.datareceived = function (data) {
                    ...
                    self.resultData = arr;
                    self.dataSource(new oj.ArrayTableDataSource(arr, {idAttribute: 'travelReimbursementId'}));
                    //差旅报销费用清单
                    self.dataSource1(new oj.ArrayTableDataSource(arrData[0].userData.detailTravelReimbursementDetail, {idAttribute: 'travelReimbursementDetailId'}));
                    //费用分拆
                    self.dataSource2(new oj.ArrayTableDataSource(arrData[0].userData.detailTravelReimbursementLinkProject, {idAttribute: 'travelReimbursementLinkProjectId'}));
                    //  某人的报销页面的标题以报销人+主题 
                    self.specialHeaderTitle(dl.getLabelByValue('employees', arrData[0].userData['applicant']) + arrData[0].userData['title']);
//                    进入即阅
...
//                进度按钮
                self.jinduClick = function (data, event) {
                    ...
                    dl.pushPageStack({name: 'reimbursement/ProjectTravelReimbursementAttention', params: self.parentViewModel, cacheKey: 'reimbursement/ProjectTravelReimbursementAttention'});
            ...
            return ProjectTravelReimbursementAttentionViewModel;
        }
);
