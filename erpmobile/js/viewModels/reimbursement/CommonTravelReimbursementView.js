define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient', 'dl/msgbox', 'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojselectcombobox', 'ojs/ojnavigationlist',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource', 'ojs/ojarraytabledatasource'],
        function (oj, ko, $, dl, sc) {
            function CommonTravelReimbursementViewViewModel(params) {
                var self = this;
                self.viewModelName = 'CommonTravelReimbursementViewViewModel';
...
//                进度按钮
                self.jinduClick = function (data, event) {
                    ...
                    dl.getGlobalModule({name: 'schedule', params: self});
                    dl.pushPageStack({name: 'reimbursement/CommonTravelReimbursementView', params: self.parentViewModel});
                    dl.pushHeader(dl.headerTitle());
                    dl.headerTitle("流程进度");
                };
//                打开附件
                self.openAttachment = function (data, event) {
                    self.attachment(data.attachment);
                    dl.getGlobalModule({name: 'attachment', params: self});
                    dl.pushPageStack({name: 'reimbursement/CommonTravelReimbursementView', params: self.parentViewModel});
                };
            ...
            return CommonTravelReimbursementViewViewModel;
        }
);
