define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient', 'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojselectcombobox', 'ojs/ojnavigationlist',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource', 'ojs/ojarraytabledatasource'],
        function (oj, ko, $, dl) {
            function preProjectsApplyViewViewModel(params) {
                var self = this;
                self.viewModelName = 'preProjectsApplyViewViewModel';
                ...
                self.datareceived = function (data) {
                    ...
                    if(dl.isNullOrUndefined(arrData[0].businessType)&& !dl.isNullOrUndefined(arrData[0].businessTypes)){
                         dl.setOptionsFromQuery('NQ_BusinessCategory', {}, function (response) {
                        for (var i = 0; i < response.length; i++) {
                            if (response[i].businessCategoryId == arrData[0].businessTypes) {
                                self.businessTypeName(response[i].businessCategoryName);
                            }
                        }
                    });
                    }else{
                         self.businessTypeName(arrData[0].businessType);
                    };
                   
                    self.dataSource(new oj.ArrayTableDataSource(arrData, {idAttribute: 'preProjectId'}));
                    //  某人的报销页面的标题以报销人+主题 
                ...
                self.jinduClick = function (data, event) {
                    ...
                    dl.getGlobalModule({name: 'schedule', params: self});
                    dl.pushPageStack({name: 'operation_processor/preProjects/preProjectsApplyView', params: self.parentViewModel});
                ...
                self.handleAttached = function (info) {
                    ...
                    var condition = {
                        preProjectId: params.rowData.businessId,
                        includeDetail: true
                    };
                    dl.callOperation("ST_PreProject", 'find', condition, self.datareceived);
                };
            ...
            return preProjectsApplyViewViewModel;
        }
);
