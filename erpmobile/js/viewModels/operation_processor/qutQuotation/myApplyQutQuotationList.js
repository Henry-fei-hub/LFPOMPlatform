define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient', 'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojnavigationlist',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource', 'ojs/ojcollectiontabledatasource'],
        function (oj, ko, $, dl) {
            function myApplyqutQuotationListViewModel(params) {
                var self = this;
                self.viewModelName = 'myApplyqutQuotationListViewModel';
                ...
                self.iClicked = function (data, event) {
                    ...
                    dl.pushPageStack({name: 'operation_processor/qutQuotation/myApplyQutQuotationList', params: self.parentViewModel});
                    dl.getGlobalModule({name: pageName, params: self});
                    dl.pushHeader(dl.headerTitle());
                    dl.headerTitle(dl.getLabelByValue('employees', d.employeeId) + d.businessName);
                };

                myApplyqutQuotationListModel = oj.Model.extend({
                    parse: dl.parseModel,
                    idAttribute: 'processInstanceId'
                });

                myApplyqutQuotationListCollection = oj.Collection.extend({
                    model: new myApplyqutQuotationListModel(),
                ...
                self.collection = new myApplyqutQuotationListCollection();
                ...
                self.handleAttached = function (info) {
                    ...
                    switch (Number(activityType)) {
                          case 1:
                            ...
                            self.queryAction('NQ_OnLoadApplyListOfOutQuotations');
                            break;
                        case 2:
                        case 3:
                            self.searchData({
            ...
            return myApplyqutQuotationListViewModel;
        }
);
