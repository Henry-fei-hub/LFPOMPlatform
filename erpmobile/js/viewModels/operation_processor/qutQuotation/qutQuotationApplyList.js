define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient', 'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojnavigationlist', 'ojs/ojlistview',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource', 'ojs/ojcollectiontabledatasource'],
        function (oj, ko, $, dl) {
            function qutQuotationApplyListViewModel(params) {
                var self = this;
                self.viewModelName = 'qutQuotationApplyListViewModel';
                ...
                qutQuotationApplyListModel = oj.Model.extend({
                    parse: dl.parseModel,
                    idAttribute: 'resetCostId'
                });

                qutQuotationApplyListCollection = oj.Collection.extend({
                    model: new qutQuotationApplyListModel(),
                    parse: dl.parseCollection,
                    fetchSize: 10,
                    customPagingOptions: dl.parsePaging,
                    queryAction: 'NQ_OnLoadAuditListOfOutQuotations',
                    url: dl._SERVERURL
                });
                self.collection = new qutQuotationApplyListCollection();
            ...
            return qutQuotationApplyListViewModel;
        }
);
