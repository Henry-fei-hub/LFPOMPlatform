define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient', 'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojnavigationlist', 'ojs/ojlistview',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource', 'ojs/ojcollectiontabledatasource'],
        function (oj, ko, $, dl) {
            function projectSettlementProcessApplyListViewModel(params) {
                var self = this;
                self.viewModelName = 'projectSettlementProcessApplyListViewModel';
                self.parentViewModel = params;
                self.dataSource = ko.observable();
                self.normalData = ko.observable();
                self.selfData = ko.observable();
                ...
                projectSettlementProcessApplyListModel = oj.Model.extend({
                    parse: dl.parseModel,
                    idAttribute: 'settlementId'
                });

                projectSettlementProcessApplyListCollection = oj.Collection.extend({
                    model: new projectSettlementProcessApplyListModel(),
                    parse: dl.parseCollection,
                    fetchSize: 10,
                    customPagingOptions: dl.parsePaging,
                    queryAction: 'NQ_OnLoadAuditListOfProjectSettlement',
                    url: dl._SERVERURL
                });
                self.collection = new projectSettlementProcessApplyListCollection();
            ...
            return projectSettlementProcessApplyListViewModel;
        }
);
