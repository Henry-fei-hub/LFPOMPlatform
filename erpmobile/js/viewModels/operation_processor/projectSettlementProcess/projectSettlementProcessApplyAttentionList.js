...
define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient','dl/msgbox',  'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojnavigationlist',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource', 'ojs/ojcollectiontabledatasource'],
        function (oj, ko, $, dl,sc) {
            function projectSettlementProcessApplyAttentionListViewModel(params) {
                var self = this;
                self.viewModelName = 'projectSettlementProcessApplyAttentionListViewModel';
...
//                一键标记已读的回调函数
                ...
                self.commonClick = function (data, event) {
                    ...
                    dl.pushPageStack({name: 'operation_processor/projectSettlementProcess/projectSettlementProcessApplyAttentionList', params: self.parentViewModel});
                ...
                projectSettlementProcessApplyAttentionListModel = oj.Model.extend({
                    parse: dl.parseModel,
                    idAttribute: 'systemProcessAttentionId'
                });

               projectSettlementProcessApplyAttentionListCollection = oj.Collection.extend({
                    model: new projectSettlementProcessApplyAttentionListModel(),
                    parse: dl.parseCollection,
                    fetchSize: 10,
                    customPagingOptions: dl.parsePaging,
                    queryAction: 'NQ_OnLoadAttentionListOfProjectSettlement',
                    url: dl._SERVERURL
                });
                self.collection = new projectSettlementProcessApplyAttentionListCollection();
            ...
            return projectSettlementProcessApplyAttentionListViewModel;
        }
);
