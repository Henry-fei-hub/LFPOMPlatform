define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient', 'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojnavigationlist',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource', 'ojs/ojcollectiontabledatasource'],
        function (oj, ko, $, dl) {
            function myApplyOperationListViewModel(params) {
                var self = this;
                self.viewModelName = 'myApplyOperationListViewModel';
                ...
                self.iClicked = function (data, event) {
                    ...
                    dl.pushPageStack({name: 'operation_processor/myApplyOperationList', params: self.parentViewModel});
                    dl.getGlobalModule({name: pageName, params: self});
                    dl.pushHeader(dl.headerTitle());
                    dl.headerTitle(dl.getLabelByValue('employees', d.employeeId) + d.businessName);
                };
                myApplyOperationListModel = oj.Model.extend({
                    parse: dl.parseModel,
                    idAttribute: 'processInstanceId'
                });

                myApplyOperationListCollection = oj.Collection.extend({
                    model: new myApplyOperationListModel(),
                ...
                self.collection = new myApplyOperationListCollection();
                    ...
                    // 0,1 查找正在进行中   3：查 完成  5：  查 驳回.
                       switch (Number(activityType)) {
                        case 1:
                            ...
                            self.queryAction("NQ_ApplyListOfPreProjectReport"); 
                            break;
                        case 2:
                        case 3:
                            self.searchData({
                            ...
                            self.queryAction("NQ_RefuseListOfPreProjectReport");
                            break;
                    }
                };
            ...
            return myApplyOperationListViewModel;
        }
);
