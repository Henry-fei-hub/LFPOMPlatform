define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient', 'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojnavigationlist',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource', 'ojs/ojcollectiontabledatasource'],
        function (oj, ko, $, dl) {
            function myApplyProjectStagesProcessListViewModel(params) {
                var self = this;
                self.viewModelName = 'myApplyProjectStagesProcessListViewModel';
                ...
                self.iClicked = function (data, event) {
                    ...
                    dl.pushPageStack({name: 'operation_processor/projectStagesProcess/myApplyProjectStagesProcessList', params: self.parentViewModel});
                    dl.getGlobalModule({name: pageName, params: self});
                    dl.pushHeader(dl.headerTitle());
                    dl.headerTitle(d.projectName);
                };

                myApplyProjectStagesProcessListModel = oj.Model.extend({
                    parse: dl.parseModel,
                    idAttribute: 'processInstanceId'
                });

                myApplyProjectStagesProcessListCollection = oj.Collection.extend({
                    model: new myApplyProjectStagesProcessListModel(),
                ...
                self.collection = new myApplyProjectStagesProcessListCollection();
                ...
                self.handleAttached = function (info) {
                    ...
                    switch (Number(activityType)) {
                        case 1:
                            ...
                            self.queryAction('NQ_OnLoadApplyListOfProjectStages');
                            break;
                        case 2:
                        case 3:
                            self.searchData({
            ...
            return myApplyProjectStagesProcessListViewModel;
        }
);
