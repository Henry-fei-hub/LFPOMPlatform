define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient', 'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojnavigationlist', 'ojs/ojlistview',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource', 'ojs/ojcollectiontabledatasource'],
        function (oj, ko, $, dl) {
            function projectStagesProcessApplyListViewModel(params) {
                var self = this;
                self.viewModelName = 'projectStagesProcessApplyListViewModel';
                ...
                projectStagesProcessApplyListModel = oj.Model.extend({
                    parse: dl.parseModel,
                    idAttribute: 'processInstanceId'
                });

                projectStagesProcessApplyListCollection = oj.Collection.extend({
                    model: new projectStagesProcessApplyListModel(),
                    parse: dl.parseCollection,
                    fetchSize: 10,
                    customPagingOptions: dl.parsePaging,
                    queryAction: 'NQ_OnLoadAuditListOfProjectStages',
                    url: dl._SERVERURL
                });
                self.collection = new projectStagesProcessApplyListCollection();
            ...
            return projectStagesProcessApplyListViewModel;
        }
);
