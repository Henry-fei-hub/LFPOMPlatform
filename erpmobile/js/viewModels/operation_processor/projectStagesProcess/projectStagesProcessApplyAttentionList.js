...
define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient','dl/msgbox',  'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojnavigationlist',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource', 'ojs/ojcollectiontabledatasource'],
        function (oj, ko, $, dl,sc) {
            function projectStagesProcessApplyAttentionListViewModel(params) {
                var self = this;
                self.viewModelName = 'projectStagesProcessApplyAttentionListViewModel';
...
//                一键标记已读的回调函数
                ...
                self.commonClick = function (data, event) {
                    ...
                    dl.pushPageStack({name: 'operation_processor/projectStagesProcess/projectStagesProcessApplyAttentionList', params: self.parentViewModel});
                ...
                projectStagesProcessApplyAttentionListModel = oj.Model.extend({
                    parse: dl.parseModel,
                    idAttribute: 'systemProcessAttentionId'
                });

               projectStagesProcessApplyAttentionListCollection = oj.Collection.extend({
                    model: new projectStagesProcessApplyAttentionListModel(),
                    parse: dl.parseCollection,
                    fetchSize: 10,
                    customPagingOptions: dl.parsePaging,
                    queryAction: 'NQ_OnLoadAttentionListOfProjectStages',
                    url: dl._SERVERURL
                });
                self.collection = new projectStagesProcessApplyAttentionListCollection();
            ...
            return projectStagesProcessApplyAttentionListViewModel;
        }
);
