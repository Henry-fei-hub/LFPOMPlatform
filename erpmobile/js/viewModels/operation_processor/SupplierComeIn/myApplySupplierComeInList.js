define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient', 'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojnavigationlist',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource', 'ojs/ojcollectiontabledatasource'],
        function (oj, ko, $, dl) {
            function myApplySupplierComeInListViewModel(params) {
                var self = this;
                self.viewModelName = 'myApplySupplierComeInListViewModel';
                ...
                self.iClicked = function (data, event) {
                    ...
                    dl.pushPageStack({name: 'operation_processor/SupplierComeIn/myApplySupplierComeInList', params: self.parentViewModel});
                    dl.getGlobalModule({name: pageName, params: self});
                    dl.pushHeader(dl.headerTitle());
                    dl.headerTitle(dl.getLabelByValue('employees', d.employeeId) + d.businessName);
                };
                myApplySupplierComeInListCollection = oj.Model.extend({
                    parse: dl.parseModel,
                    idAttribute: 'processInstanceId'
                });

                myApplySupplierComeInListCollection = oj.Collection.extend({
                    model: new myApplySupplierComeInListCollection(),
                ...
                self.collection = new myApplySupplierComeInListCollection();
                    ...
                    // 0,1 查找正在进行中   3：查 完成  5：  查 驳回.
                       switch (Number(activityType)) {
                        case 1:
                            ...
                            self.queryAction("NQ_ApplySupplierInfo"); 
                            break;
                        case 2:
                        case 3:
                            self.searchData({
                            ...
                            self.queryAction("NQ_RefuseSupplierInfo");
                            break;
                    }
                };
            ...
            return myApplySupplierComeInListViewModel;
        }
);
