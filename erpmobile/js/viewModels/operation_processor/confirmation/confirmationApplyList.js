define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient', 'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojnavigationlist', 'ojs/ojlistview',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource', 'ojs/ojcollectiontabledatasource'],
        function (oj, ko, $, dl) {
            function confirmationApplyListViewModel(params) {
                var self = this;
                self.viewModelName = 'confirmationApplyListViewModel';
              ...
              //                放查询条件
                ...
                confirmationApplyListModel = oj.Model.extend({
                    parse: dl.parseModel,
                    idAttribute: 'resetCostId'
                });

                confirmationApplyListCollection = oj.Collection.extend({
                    model: new confirmationApplyListModel(),
                    parse: dl.parseCollection,
                    fetchSize: 10,
                    customPagingOptions: dl.parsePaging,
                    queryAction: 'NQ_OnLoadAuditListOfPersonnel',
                    url: dl._SERVERURL
                });
                self.collection = new confirmationApplyListCollection();
            ...
            return confirmationApplyListViewModel;
        }
);
