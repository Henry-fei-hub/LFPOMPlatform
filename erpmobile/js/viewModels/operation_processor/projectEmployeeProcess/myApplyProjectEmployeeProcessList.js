define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient', 'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojnavigationlist',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource', 'ojs/ojcollectiontabledatasource'],
        function (oj, ko, $, dl) {
            function myApplyprojectEmployeeProcessListViewModel(params) {
                var self = this;
                self.viewModelName = 'myApplyprojectEmployeeProcessListViewModel';
                ...
                self.iClicked = function (data, event) {
                    ...
                    dl.pushPageStack({name: 'operation_processor/projectEmployeeProcess/myApplyProjectEmployeeProcessList', params: self.parentViewModel});
                    dl.getGlobalModule({name: pageName, params: self});
                    dl.pushHeader(dl.headerTitle());
                    dl.headerTitle(d.projectName);
                };

                myApplyprojectEmployeeProcessListModel = oj.Model.extend({
                    parse: dl.parseModel,
                    idAttribute: 'processInstanceId'
                });

                myApplyprojectEmployeeProcessListCollection = oj.Collection.extend({
                    model: new myApplyprojectEmployeeProcessListModel(),
                ...
                self.collection = new myApplyprojectEmployeeProcessListCollection();
                ...
                self.handleAttached = function (info) {
                    ...
                    switch (Number(activityType)) {
                        case 1:
                            ...
                            self.queryAction('NQ_OnLoadApplyListOfProjectEmployees');
                            break;
                        case 2:
                        case 3:
                            self.searchData({
            ...
            return myApplyprojectEmployeeProcessListViewModel;
        }
);
