define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient', 'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojnavigationlist', 'ojs/ojlistview',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource', 'ojs/ojcollectiontabledatasource'],
        function (oj, ko, $, dl) {
            function projectEmployeeProcessApplyListViewModel(params) {
                var self = this;
                self.viewModelName = 'projectEmployeeProcessApplyListViewModel';
                ...
                projectEmployeeProcessApplyListModel = oj.Model.extend({
                    parse: dl.parseModel,
                    idAttribute: 'resetCostId'
                });

                projectEmployeeProcessApplyListCollection = oj.Collection.extend({
                    model: new projectEmployeeProcessApplyListModel(),
                    parse: dl.parseCollection,
                    fetchSize: 10,
                    customPagingOptions: dl.parsePaging,
                    queryAction: 'NQ_OnLoadAuditListOfProjectEmployees',
                    url: dl._SERVERURL
                });
                self.collection = new projectEmployeeProcessApplyListCollection();
            ...
            return projectEmployeeProcessApplyListViewModel;
        }
);
