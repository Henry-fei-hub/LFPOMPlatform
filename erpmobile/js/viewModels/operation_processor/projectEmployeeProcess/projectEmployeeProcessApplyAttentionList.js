...
define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient','dl/msgbox',  'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojnavigationlist',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource', 'ojs/ojcollectiontabledatasource'],
        function (oj, ko, $, dl,sc) {
            function projectEmployeeProcessApplyAttentionListViewModel(params) {
                var self = this;
                self.viewModelName = 'projectEmployeeProcessApplyAttentionListViewModel';
...
//                一键标记已读的回调函数
                ...
                self.commonClick = function (data, event) {
                    ...
                    dl.pushPageStack({name: 'operation_processor/projectEmployeeProcess/projectEmployeeProcessApplyAttentionList', params: self.parentViewModel});
                ...
                projectEmployeeProcessApplyAttentionListModel = oj.Model.extend({
                    parse: dl.parseModel,
                    idAttribute: 'systemProcessAttentionId'
                });

               projectEmployeeProcessApplyAttentionListCollection = oj.Collection.extend({
                    model: new projectEmployeeProcessApplyAttentionListModel(),
                    parse: dl.parseCollection,
                    fetchSize: 10,
                    customPagingOptions: dl.parsePaging,
                    queryAction: 'NQ_OnLoadAttentionListOfProjectEmployees',
                    url: dl._SERVERURL
                });
                self.collection = new projectEmployeeProcessApplyAttentionListCollection();
            ...
            return projectEmployeeProcessApplyAttentionListViewModel;
        }
);
