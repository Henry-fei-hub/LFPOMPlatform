define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient','dl/msgbox', 'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojnavigationlist',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource','ojs/ojarraytabledatasource'],
        function (oj, ko, $, dl, sc) {
            function closePreProjectsAttentionListViewModel(params) {
                var self = this;
                self.viewModelName = 'closePreProjectsAttentionListViewModel';
                ...
                self.commonClick = function (event, selfData) {
                    ...
                    dl.pushPageStack({name: 'operation_processor/closePreProjects/closePreProjectsAttentionList', params: self.parentViewModel, cacheKey:  'operation_processor/closePreProjects/closePreProjectsAttentionList'});
...
//                一键标记已读的回调函数
                ...
                self.handleAttached = function (info) {
                    ...
                    if (processType === 44) {
                        if (Number(self.parentViewModel.handelType) === 2) {//知会
                            dl.callOperation("NQ_AttentionListOfPreProjectReport", 'find', condition, self.datareceived);
                        }
                    }
            ...
            return closePreProjectsAttentionListViewModel;
        }
);
