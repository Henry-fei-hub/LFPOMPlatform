define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient', 'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojnavigationlist',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource', 'ojs/ojcollectiontabledatasource'],
        function (oj, ko, $, dl) {
            function myApplyFailInAtenderListViewModel(params) {
                var self = this;
                self.viewModelName = 'myApplyFailInAtenderListViewModel';
                ...
                self.iClicked = function (data, event) {
                    ...
                    dl.pushPageStack({name: 'operation_processor/failInAtender/myApplyFailInAtenderList', params: self.parentViewModel});
                    dl.getGlobalModule({name: pageName, params: self});
                    dl.pushHeader(dl.headerTitle());
                    dl.headerTitle(dl.getLabelByValue('employees', d.employeeId) + d.projectName);
                };
                myApplyFailInAtenderListCollection = oj.Model.extend({
                    parse: dl.parseModel,
                    idAttribute: 'processInstanceId'
                });

                myApplyFailInAtenderListCollection = oj.Collection.extend({
                    model: new myApplyFailInAtenderListCollection(),
                ...
                self.collection = new myApplyFailInAtenderListCollection();
                    ...
                    // 0,1 查找正在进行中   3：查 完成  5：  查 驳回.
                       switch (Number(activityType)) {
                        case 1:
                            ...
                            self.queryAction("NQ_OnLoadApplyListOfBidPaySheetProject"); 
                            break;
                        case 2:
                        case 3:
                            self.searchData({
            ...
            return myApplyFailInAtenderListViewModel;
        }
);
