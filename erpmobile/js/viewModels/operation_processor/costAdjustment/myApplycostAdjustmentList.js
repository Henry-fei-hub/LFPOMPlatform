define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient', 'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojnavigationlist',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource', 'ojs/ojcollectiontabledatasource'],
        function (oj, ko, $, dl) {
            function myApplycostAdjustmentListViewModel(params) {
                var self = this;
                self.viewModelName = 'myApplycostAdjustmentListViewModel';
                ...
                self.iClicked = function (data, event) {
                    ...
                    dl.pushPageStack({name: 'operation_processor/costAdjustment/myApplycostAdjustmentList', params: self.parentViewModel});
                    dl.getGlobalModule({name: pageName, params: self});
                    dl.pushHeader(dl.headerTitle());
                    dl.headerTitle(dl.getLabelByValue('employees', d.employeeId) + d.businessName);
                };

                myApplycostAdjustmentListModel = oj.Model.extend({
                    parse: dl.parseModel,
                    idAttribute: 'processInstanceId'
                });

                myApplycostAdjustmentListCollection = oj.Collection.extend({
                    model: new myApplycostAdjustmentListModel(),
                ...
                self.collection = new myApplycostAdjustmentListCollection();
                    ...
                    switch (Number(activityType)) {
                        case 1:
                            self.searchData({
                                currentPage: 0,
                                pageLines: 1,
                                drafter: dl.getEmployeeId(),
                                processType: processType
                            });
                            self.queryAction('NQ_OnLoadApplyListOfCostAllocation');
                            break;
                        case 2:
                        case 3:
                            self.searchData({
                                ...
                                processType: processType,
                                keyValues: [{key: 'spi.process_status = ?', value: 'spi.process_status = ? and spi.process_status IN ( 3, 5 )'}]
                            });
                            self.queryAction('NQ_OnLoadRefuseListOfCostAllocation');
                            break;
                    }
                    ;
            ...
            return myApplycostAdjustmentListViewModel;
        }
);
