...
define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient','dl/msgbox',  'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojnavigationlist',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource', 'ojs/ojcollectiontabledatasource'],
        function (oj, ko, $, dl,sc) {
            function costAdjustmentApplyAttentionListViewModel(params) {
                var self = this;
                self.viewModelName = 'costAdjustmentApplyAttentionListViewModel';
...
//                一键标记已读的回调函数
                ...
                self.commonClick = function (data, event) {
                    ...
                    dl.pushPageStack({name: 'operation_processor/costAdjustment/costAdjustmentApplyAttentionList', params: self.parentViewModel});
                ...
                costAdjustmentApplyAttentionListModel = oj.Model.extend({
                    parse: dl.parseModel,
                    idAttribute: 'systemProcessAttentionId'
                });

               costAdjustmentApplyAttentionListCollection = oj.Collection.extend({
                    model: new costAdjustmentApplyAttentionListModel(),
                    parse: dl.parseCollection,
                    fetchSize: 10,
                    customPagingOptions: dl.parsePaging,
                    queryAction: 'NQ_OnLoadAttentionListOfCostAllocation',
                    url: dl._SERVERURL
                });
                self.collection = new costAdjustmentApplyAttentionListCollection();
            ...
            return costAdjustmentApplyAttentionListViewModel;
        }
);
