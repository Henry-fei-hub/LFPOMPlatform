define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient', 'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojnavigationlist',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource', 'ojs/ojcollectiontabledatasource'],
        function (oj, ko, $, dl) {
            function costAdjustmentExamineListViewModel(params) {
                var self = this;
                self.viewModelName = 'costAdjustmentExamineListViewModel';
...
//                  每个报销特有businessId  
                    ...
                    dl.pushPageStack({name: "myExamine/costAdjustmentExamineList", params: self.parentViewModel});
...
//                    headerTitle
                ...
                self.searchData ={
                                ...
                                keyValues: [{key: 'spia.status =?', value: 'spia.status =? and spia.status in (2,3,4,5,6)'},
                                    ...
                                    {key: 'A.department_id IN ( SELECT child_id FROM department_ids WHERE department_id =? )', value: 'A.department_id=?'}
                                ]
                            };
                costAdjustmentExamineListModel = oj.Model.extend({
                    parse: dl.parseModel,
                    idAttribute: 'processInstanceActivityId'
                });

                costAdjustmentExamineListCollection = oj.Collection.extend({
                    model: new costAdjustmentExamineListModel(),
                    parse: dl.parseCollection,
                    fetchSize: 10,
                    customPagingOptions: dl.parsePaging,
                    queryAction: 'NQ_OnLoadFlowAuditListOfCostAllocationBuisnessData',
                    url: dl._SERVERURL
                });
                self.collection = new costAdjustmentExamineListCollection();
            ...
            return costAdjustmentExamineListViewModel;
        }
);
