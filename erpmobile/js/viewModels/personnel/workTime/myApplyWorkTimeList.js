define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient', 'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojnavigationlist',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource', 'ojs/ojcollectiontabledatasource'],
        function (oj, ko, $, dl) {
            function myApplyWorkTimeListViewModel(params) {
                var self = this;
                self.viewModelName = 'myApplyWorkTimeListViewModel';
                ...
                self.iClicked = function (data, event) {
                    ...
                    dl.pushPageStack({name: 'personnel/workTime/myApplyWorkTimeList', params: self.parentViewModel});
                    dl.getGlobalModule({name: pageName, params: self});
                    dl.pushHeader(dl.headerTitle());
                    dl.headerTitle(dl.getLabelByValue('employees', d.employeeId) + d.businessName);
                };

                myApplyWorkTimeListModel = oj.Model.extend({
                    parse: dl.parseModel,
                    idAttribute: 'processInstanceId'
                });

                myApplyWorkTimeListCollection = oj.Collection.extend({
                    model: new myApplyWorkTimeListModel(),
                ...
                self.collection = new myApplyWorkTimeListCollection();
            ...
            return myApplyWorkTimeListViewModel;
        }
);
