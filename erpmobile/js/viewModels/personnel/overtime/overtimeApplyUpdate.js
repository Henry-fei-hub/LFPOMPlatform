define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient', 'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojselectcombobox', 'ojs/ojnavigationlist',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojdatetimepicker', 'ojs/ojjsontreedatasource','ojs/ojarraytabledatasource'],
        function (oj, ko, $, dl) {
            function overtimeUpdateViewModel(params) {
                var self = this;
                self.viewModelName = 'overtimeUpdateViewModel';
                ...
                self.datareceived = function (data) {
                    ...
                    for (var i = 0; i < arrData.length; i++) {
                        ...
                        if (!dl.isNullOrUndefined(arrData[i]['endDate'])) {
                            arrData[i]['endDate'] = dl.parseValueFromService(arrData[i]['endDate']);
                        }
                        //获取加班类型;1:项目外出;2:前期项目外出;3:非项目外出
                        var projectType = arrData[i]['projectType'];
                        if (Number(projectType) === 1) {
                            arrData[i]['projectTypeName'] = '项目外出';
                        } else if (Number(projectType) === 2) {
                            arrData[i]['projectTypeName'] = '前期项目外出';
                        } else if (Number(projectType) === 3) {
                            arrData[i]['projectTypeName'] = '非项目外出';
                        }
                    }
            ...
            return overtimeUpdateViewModel;
        }
);
