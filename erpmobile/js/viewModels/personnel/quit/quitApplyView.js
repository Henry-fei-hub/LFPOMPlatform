    ...
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource', 'ojs/ojarraytabledatasource'],
        function (oj, ko, $, dl, sc) {
            function quitApplyViewViewModel(params) {
                ...
                self.__processComment = ko.observable();
                ...
                self.__equivalentNumber = ko.observable();
                self.__transportation = ko.observable();
                self.__severance = ko.observable(0);
                self.__unit = ko.observable();
                self.__isRemoteCity = ko.observable(true);
                self.isenable = ko.observable(false);
                self.isShow = ko.observable(false);
                self.detailContShow =  ko.observable(false);
                var ProjectManagerResult = [];

                self.__fee.subscribe(function (val) {
                    self.__housingFinance(Number(self.__livingExpenses()) - Number(self.__fee()));
                });
                self.__housingFinance.subscribe(function (val) {
                    self.__fee(Number(self.__livingExpenses()) - Number(self.__housingFinance()));
                });
                var userData = {};
                self.isshow = function (id) {
                    if (!dl.isNullOrUndefined(params.rowData.backViewName) && (params.rowData.backViewName).indexOf("部门经理") >= 0) {
                        switch (id) {
                            case(1):
                                return true;
                                break;
                    ...
                    } else if (!dl.isNullOrUndefined(params.rowData.backViewName) && (params.rowData.backViewName).indexOf("FC经理") >= 0) {
                        switch (id) {
                            case(5):
                                return true;
                                break;
                    ...
                    } else if (!dl.isNullOrUndefined(params.rowData.backViewName) && (params.rowData.backViewName).indexOf("行政经理") >= 0) {
                        switch (id) {
                            case(6):
                                return true;
                                break;
                    ...
                    } else if (!dl.isNullOrUndefined(params.rowData.backViewName) && (params.rowData.backViewName).indexOf("运营中心分管领导") >= 0) {
                        //  processName_1
                        switch (id) {
                            case(2):
                                return true;
                    ...
                    } else if (!dl.isNullOrUndefined(params.rowData.backViewName) && (params.rowData.backViewName).indexOf("运营专员") >= 0) {
                        switch (id) {
                            case(2):
                                return true;
                                break;
                    ...
                    } else if (!dl.isNullOrUndefined(params.rowData.backViewName) && (params.rowData.backViewName).indexOf("招聘专员") >= 0) {
                        //  processName_2
                        switch (id) {
                            case(13):
                                return true;
                    ...
                    } else if (!dl.isNullOrUndefined(params.rowData.backViewName) && (params.rowData.backViewName).indexOf("运营管理员") >= 0) {
                        //  processName_3
                        switch (id) {
                            case(13):
                                return true;
                    ...
                    } else if (!dl.isNullOrUndefined(params.rowData.backViewName) && (params.rowData.backViewName).indexOf("人资总监") >= 0) {
                        //  processName_5
                        switch (id) {
                            case(2):
                                return true;
                    ...
                    } else if (!dl.isNullOrUndefined(params.rowData.backViewName) && (params.rowData.backViewName).indexOf("人事专员") >= 0) {
                        //  processName_7
                        switch (id) {
                            case(9):
                                return true;
                    ...
                    } else if (!dl.isNullOrUndefined(params.rowData.backViewName) && (params.rowData.backViewName).indexOf("资源经理") >= 0) {
                        //  processName_9
                        switch (id) {
                            case(2):
                                return true;
                    ...
                    } else if (!dl.isNullOrUndefined(params.rowData.backViewName) && (params.rowData.backViewName).indexOf("部门行政负责人") >= 0) {
                        //  processName_11
                        switch (id) {
                            case(2):
                                return true;
                    ...
                    } else if (!dl.isNullOrUndefined(params.rowData.backViewName) && (params.rowData.backViewName).indexOf("运维工程师") >= 0) {
                        switch (id) {
                            case(7):
                                return true;
                                break;
                    ...
                    } else if (!dl.isNullOrUndefined(params.rowData.backViewName) && (params.rowData.backViewName).indexOf("部门助理") >= 0) {
                        switch (id) {
                            case(8):
                                return true;
                                break;
                ...
                self.tixinMessage = function () {
                    if (!dl.isNullOrUndefined(params.rowData.backViewName) && (params.rowData.backViewName).indexOf("运营专员") >= 0) {
//                            1
                        return '';

                    } else if (!dl.isNullOrUndefined(params.rowData.backViewName) && (params.rowData.backViewName).indexOf("人事专员") >= 0) {
                        return '♥ 温馨提示：请填写社保/公积金停缴月份，并收回离职员工的工牌';
                    } else if (!dl.isNullOrUndefined(params.rowData.backViewName) && (params.rowData.backViewName).indexOf("部门助理") >= 0) {
                        return '♥ 温馨提示：请填写离职考勤信息';
                        return '';
                    } else if (!dl.isNullOrUndefined(params.rowData.backViewName) && (params.rowData.backViewName).indexOf("FC经理") >= 0) {
                        return '♥ 温馨提示：请填写该员工的借款情况并附上审核意见';
                    } else if (!dl.isNullOrUndefined(params.rowData.backViewName) && (params.rowData.backViewName).indexOf("行政经理") >= 0) {
//                            6 
                        return '♥ 温馨提示：请填写收回的拖柜钥匙数量和储物柜钥匙数量';
                        return '';
                    } else if (!dl.isNullOrUndefined(params.rowData.backViewName) && (params.rowData.backViewName).indexOf("运维工程师") >= 0) {
//                            7  
                        return '♥ 温馨提示：请填写收回的办公电脑等设备信息';
                        return '';
                    } else if (!dl.isNullOrUndefined(params.rowData.backViewName) && (params.rowData.backViewName).indexOf("部门经理") >= 0) {
...
//                         return '';
                ...
                self.datareceived = function (data) {
                    ...
                    self.__companyProvince(arrData[0].fee);
                    self.__numberOfBaby(arrData[0].numberOfBaby);
                    self.__equivalentNumber(arrData[0].equivalentNumber);
                    self.__transportation(arrData[0].transportation);
                    self.__severance(arrData[0].severance);
                    self.__unit(arrData[0].unit);
                    self.__isRemoteCity(arrData[0].isRemoteCity);

                    var type = arrData[0].type;
                    var businessId = arrData[0].personnelBusinessId;

                    if (self.parentViewModel.rowData.backViewName.indexOf('资源经理') >= 0) {
                        self.isenable(true);
                        var condition1 = {
                            optType: 'onGetLeftIntegralOfEmployee',
                            employeeId: arrData[0].employeeId
                        };
                        dl.callOperation("EP_AccountManageProcess", 'find', condition1, function (data) {
                            var arrData = dl.parseCollection(data);
                            self.__livingExpenses(arrData[0].integral);
                        });
                        // 查询该员工是否为积分管理员，如果是，其管理的项目是否已转交给其他积分管理员
                        var condition2 = {
                            optType: 'needToChangeProjectManager',
                            employeeId: arrData[0].employeeId
                        };
                        dl.callOperation("EP_MyPersonnelProcessor", 'find', condition2, function (data) {
                            ProjectManagerResult = data.ResultSet.Result;

                        });
                    }
//                    审批节点
                    if (self.parentViewModel.rowData.activityType == 3 || self.parentViewModel.rowData.activityType == 4) {
                        self.detailContShow(true);
                        var condition3 = {
                            optType: 'getWorkhandovers',
                            type: type,
                            businessId: businessId
                        };
                        dl.callOperation("EP_MyPersonnelProcessor", 'find', condition3, function (data) {
                            var arrData = dl.parseCollection(data);
                ...
                self.submitClick = function (data, event) {
                    ...
                    sc.confirm("确定" + inner + "吗?", function () {
                        ...
                        var processComment = self.__processComment();
                ...
                self.checkData = function (values) {
                    if (!dl.isNullOrUndefined(params.rowData.backViewName) && (params.rowData.backViewName).indexOf("部门经理") >= 0) {
                        ...
                        if (dl.isNullOrEmpty(values.contactPerson)) {
                            self.errorMessage('♥提示:请填写上该员工的借款情况');
                            return false;
                        } else {
                            return true;
                        }
                    } else if (!dl.isNullOrUndefined(params.rowData.backViewName) && (params.rowData.backViewName).indexOf("行政经理") >= 0) {
                        if (dl.isNullOrEmpty(values.companyProvince)) {
                            self.errorMessage('♥提示:请填写收回的拖柜钥匙数量');
                            return false;
                        } else if (dl.isNullOrEmpty(values.numberOfBaby)) {
                            self.errorMessage('♥提示:请填写收回的储物柜钥匙数量');
                            return false;
                        } else {
                            return true;
                        }
                    } else if (!dl.isNullOrUndefined(params.rowData.backViewName) && (params.rowData.backViewName).indexOf("运维工程师") >= 0) {
                        if (dl.isNullOrEmpty(values.equivalentNumber)) {
                            self.errorMessage('♥提示:请填写收回的办公电脑等设备信息');
                            return false;
                        } else {
                            return true;
                        }
                        ...
                        if (dl.isNullOrEmpty(values.transportation)) {
                            self.errorMessage('♥提示:请核对上该员工的离职考勤');
                            return false;
                        } else {
                            return true;
                        }
                        ...
                        if (dl.isNullOrEmpty(values.unit)) {
                            self.errorMessage('♥提示:请填写社保/公积金停缴月份');
                            return false;
                        } else if (dl.isNullOrEmpty(values.isRemoteCity) || !(values.isRemoteCity)) {
                            self.errorMessage('♥提示:请收回员工的工卡，并勾选上“工牌是否已回收”一项');
                            return false;
                        } else {
                            return true;
                        }
                    } else if (!dl.isNullOrUndefined(params.rowData.backViewName) && (params.rowData.backViewName).indexOf("资源经理") >= 0) {
                        if (dl.isNullOrEmpty(values.livingExpenses) || values.livingExpenses < 0) {
                            self.errorMessage('♥提示:该员工的实时积分余额小于0，不能通过审批');
                            return false;
                        } else if (!dl.isNullOrEmpty(ProjectManagerResult)) {
                            self.errorMessage('该离职员工是积分管理员，请及时将其管理的订单更换积分管理员，再通过审批！');
                        } else if (dl.isNullOrEmpty(values.housingFinance)) {
                            self.errorMessage('请填写上该员工的冻结积分');
                        } else if (dl.isNullOrEmpty(values.fee)) {
                            self.errorMessage('请填写上该员工的离职奖金');
                        } else {
                            return true;
                        }
                    } else {
                ...
                self.jinduClick = function (data, event) {
                    ...
                    dl.getGlobalModule({name: 'schedule', params: self});
                    dl.pushPageStack({name: 'personnel/quit/quitApplyView', params: self.parentViewModel});
