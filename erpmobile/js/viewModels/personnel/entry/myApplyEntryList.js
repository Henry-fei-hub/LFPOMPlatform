define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient', 'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojnavigationlist',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource', 'ojs/ojcollectiontabledatasource'],
        function (oj, ko, $, dl) {
            function myApplyEntryListViewModel(params) {
                var self = this;
                self.viewModelName = 'myApplyEntryListViewModel';
                ...
                self.iClicked = function (data, event) {
                    ...
                    dl.pushPageStack({name: 'personnel/entry/myApplyEntryList', params: self.parentViewModel});
                    dl.getGlobalModule({name: pageName, params: self});
                    dl.pushHeader(dl.headerTitle());
                    dl.headerTitle(dl.getLabelByValue('employees', d.employeeId) + d.businessName);
                };
               
                myApplyEntryListModel = oj.Model.extend({
                    parse: dl.parseModel,
                    idAttribute: 'processInstanceId'
                });

                myApplyEntryListCollection = oj.Collection.extend({
                    model: new myApplyEntryListModel(),
                ...
                self.collection = new myApplyEntryListCollection();
                ...
                self.handleAttached = function (info) {
                    ...
                    // 0,1 查找正在进行中   3：查 完成  5：  查 驳回.
                        ...
                        case 1:
                            ...
                            self.queryAction('NQ_ApplyOfEmployeeOnboard'); 
                            break;
                        case 2:
                        case 3:
                            self.searchData({
                            ...
                            self.queryAction('NQ_RefuseOfEmployeeOnboard');
                            break;
                    };
                };
            ...
            return myApplyEntryListViewModel;
        }
);
