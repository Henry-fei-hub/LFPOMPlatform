...
define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient','dl/msgbox', 'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojnavigationlist',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource', 'ojs/ojcollectiontabledatasource'],
        function (oj, ko, $, dl,sc) {
            function entryApplyAttentionListViewModel(params) {
                var self = this;
                self.viewModelName = 'entryApplyAttentionListViewModel';
...
//                一键标记已读的回调函数
                ...
                self.commonClick = function (data, event) {
                    ...
                    dl.pushPageStack({name: 'personnel/entry/entryApplyAttentionList', params: self.parentViewModel});
                ...
                entryApplyAttentionListModel = oj.Model.extend({
                    parse: dl.parseModel,
                    idAttribute: 'systemProcessAttentionId'
                });

               entryApplyAttentionListCollection = oj.Collection.extend({
                    model: new entryApplyAttentionListModel(),
                    parse: dl.parseCollection,
                    fetchSize: 10,
                    customPagingOptions: dl.parsePaging,
                    queryAction: 'NQ_AttentionOfEmployeeOnboard',
                    url: dl._SERVERURL
                });
                self.collection = new entryApplyAttentionListCollection();
            ...
            return entryApplyAttentionListViewModel;
        }
);
