define(['ojs/ojcore', 'knockout', 'jquery', 'dl/commclient', 'dl/workflow', 'ojs/ojknockout', 'promise', 'ojs/ojselectcombobox', 'ojs/ojnavigationlist',
    'ojs/ojdatagrid', 'ojs/ojrowexpander', 'ojs/ojflattenedtreedatagriddatasource', 'ojs/ojjsontreedatasource','ojs/ojarraytabledatasource'],
        function (oj, ko, $, dl) {
            function salaryReportApplyAttentionViewModel(params) {
                var self = this;
                self.viewModelName = 'salaryReportApplyAttentionViewModel';
                ...
                self.datareceived = function (data) {
                    ...
                    if (!dl.isNullOrUndefined(params.rowData.backViewName)) {
                        ...
                        if (type === 1) {//薪资
                            ...
                            var condition = {
                                'personnelBusinessId': personnelBusinessId
                            };
                            if (params.rowData.backViewName.indexOf("人资部") >= 0 || params.rowData.backViewName.indexOf("总经理") >= 0
                                    || params.rowData.backViewName.indexOf("财务经理") >= 0 || params.rowData.backViewName.indexOf("报税会计") >= 0) {
                                //奖金总表
                                ...
                                dl.callOperation('EP_CustomSemployeeProcess', 'getPlateByRole', getPalteCondition, function (response) {
                                    if (response['ResultSet']['status'] > 0) {
                                        var tmpSql = "GROUP BY e.plate_id ,a.year,a.month ORDER BY e.plate_id,a.year,a.month";
                                        var keyvalues = [];
                                        var ids = response['ResultSet']['errors'].toString();
                                        var kv = {
                                            key: tmpSql,
                            ...
                            if (params.rowData.backViewName.indexOf("人资部") >= 0 || params.rowData.backViewName.indexOf("总经理") >= 0
                                ...
                                dl.callOperation('EP_CustomSemployeeProcess', 'getPlateByRole', getPalteCondition, function (response) {
                                    if (response['ResultSet']['status'] > 0) {
                                        var tmpSql = "GROUP BY plate_id,year ORDER BY plate_id,year";
                                        var keyvalues = [];
                                        var ids = response['ResultSet']['errors'].toString();
                                        var kv = {
                                            key: tmpSql,
...
//                };
                ...
                self.goYearBonusDetail = function(data, event){
                    var persionnelBusinessId = params.rowData.businessId;
                    data['persionnelBusinessId'] = persionnelBusinessId;
                ...
                self.jinduClick = function (data, event) {
                    ...
                    dl.pushPageStack({name: 'administration/salaryReport/salaryReportApplyAttention', params: self.parentViewModel, cacheKey: 'administration/salaryReport/salaryReportApplyAttention'});
                ...
                self.handleAttached = function (info) {
                    ...
                    if (!dl.isNullOrUndefined(pageFlag) && pageFlag === 1) {
            ...
            return salaryReportApplyAttentionViewModel;
        }
);
