define(function() {
  ...
  support.getMaxNodeBounds = function (layoutContext, excludeLabelBounds) {
  ...
  support.centerNodeAndLabel = function (layoutContext, node, x, y) {
    support.centerNode(node, x, y);
    support.positionNodeLabel(layoutContext, node);
  };

  support.centerNode = function (node, x, y) {
    var bounds = node.getContentBounds();
    node.setPosition({'x':x - bounds.x - bounds.w * .5, 'y':y - bounds.y - bounds.h * .5});
  };

  support.positionNodeLabels = function (layoutContext) {
  ...
  support.positionNodeLabel = function (layoutContext, node) {
  ...
  support.getNodeComparator = function (attribute) {
  ...
  support.layoutLinks = function (layoutContext) {
  ...
  support.getEndpoints = function(layoutContext, link) {
    ...
    if (bLinkToBounds) {
      ...
      var startP = support._intersectRect(b1, startX, startY, endX, endY, link.getStartConnectorOffset());
      var endP = support._intersectRect(b2, endX, endY, startX, startY, link.getEndConnectorOffset());
      startX = startP.x;
      startY = startP.y;
      endX = endP.x;
      endY = endP.y;
  ...
  support._intersectRect = function (rect, startX, startY, endX, endY, connOffset) {
    var SIDE_TOP = 0;
    var SIDE_RIGHT = 1;
    var SIDE_BOTTOM = 2;
    var SIDE_LEFT = 3;
