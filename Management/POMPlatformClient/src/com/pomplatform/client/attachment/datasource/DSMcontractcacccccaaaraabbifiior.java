...
public class DSMcontractcacccccaaaraabbifiior extends DBDataSource {

	public static DSMcontractcacccccaaaraabbifiior instance = null;

	public static DSMcontractcacccccaaaraabbifiior getInstance() {
		if (instance == null) {
			instance = new DSMcontractcacccccaaaraabbifiior("DSMcontractcacccccaaaraabbifiior");
		}
		return instance;
	}
	...
	private final DataSourceFloatField checkAmountField;
	private final DataSourceFloatField currentBorrowMoneyField;
	private final DataSourceFloatField currentAmountField;
	private final DataSourceDateTimeField capitalDateField;
	private final DataSourceDateTimeField settlementDateField;
	private final DataSourceDateTimeField invoiceDateField;

	public DSMcontractcacccccaaaraabbifiior(String dataSourceID) {

		super();
		setID(dataSourceID);
		setActionMode(ACTION_MODE_QUERY);
		setActionName("Mcontractcacccccaaaraabbifiior");

		contractInfoAuditedField = new DataSourceBooleanField("contractInfoAudited", "合同信息无误");
		contractInfoAuditedField.setRequired(false);
		...
		contractIdField.setForeignKey("DSMcontractcacccccaaaraabbifiior.contractId");
...
//		parentIdField.setValueMap(KeyValueManager.getValueMap("project_types"));
		...
		projectAmountField = new DataSourceFloatField("projectAmount", "累积含税营收");
		projectAmountField.setLength(16);
		...
		checkAmountField = new DataSourceFloatField("checkAmount", "确认函累计回款");
		checkAmountField.setLength(16);
		checkAmountField.setDecimalPad(2);
		checkAmountField.setFormat("##,###,###,###,##0.00");
		checkAmountField.setRequired(false);
		checkAmountField.setHidden(false);
		
		currentBorrowMoneyField = new DataSourceFloatField("currentBorrowMoney", "当期回款金额");
		currentBorrowMoneyField.setLength(16);
		currentBorrowMoneyField.setDecimalPad(2);
		currentBorrowMoneyField.setFormat("##,###,###,###,##0.00");
		currentBorrowMoneyField.setRequired(false);
		currentBorrowMoneyField.setHidden(false);
		
		currentAmountField = new DataSourceFloatField("currentAmount", "当期开票金额");
		currentAmountField.setLength(16);
		currentAmountField.setDecimalPad(2);
		currentAmountField.setFormat("##,###,###,###,##0.00");
		currentAmountField.setRequired(false);
		currentAmountField.setHidden(false);
		
		capitalDateField = new DataSourceDateTimeField("capitalDate", "最近一次收款日期");
		capitalDateField.setRequired(false);
		capitalDateField.setHidden(true);
		
		settlementDateField = new DataSourceDateTimeField("settlementDate", "最近一次积分确认日期");
		settlementDateField.setRequired(false);
		settlementDateField.setHidden(true);
		
		invoiceDateField = new DataSourceDateTimeField("invoiceDate", "最近一次发票开具日期");
		invoiceDateField.setRequired(false);
		invoiceDateField.setHidden(true);

		setFields(contractInfoAuditedField, contractIdField, contractCodeField, contractNameField, customerNameField, startDateField,
				endDateField, statusField, projectCodeField, contractTypeField, projectLevelField, signingMoneySumField,
				projectAmountField, invoiceAmountField, borrowMoneyField, totalProjectTimeField, currentBorrowMoneyField, currentAmountField,
				designAreasField, contractPriceField, projectAddressField, contractFollowerField, remarkField, contractDateField,
				signingCompanyNameField, createTimeField, updateTimeField, businessTypeField, flowStatusField,
				...
				orderNoField, parentIdField, infoCodeField, contractStatusField, clientContractIdField, isFinishField,
				isSystemAddField, operatorField, countField, checkAmountField, capitalDateField, settlementDateField, invoiceDateField);
	}
