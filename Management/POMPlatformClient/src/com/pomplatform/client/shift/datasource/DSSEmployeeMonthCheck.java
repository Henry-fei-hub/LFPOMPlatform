...
public class DSSEmployeeMonthCheck extends DBDataSource {

	public static DSSEmployeeMonthCheck instance = null;

	public static DSSEmployeeMonthCheck getInstance() {
		if (instance == null) {
			instance = new DSSEmployeeMonthCheck("DSSEmployeeMonthCheck");
		}
		return instance;
	}
	...
	private final DataSourceFloatField overtimePayBeforeField;
	private final DataSourceFloatField overtimePaySubField;
	private final DataSourceFloatField overtimePaySubBeforeField;
	private final DataSourceFloatField weekendOvertimeHoursField;
	private final DataSourceFloatField holidayOvertimeHoursField;
	private final DataSourceBooleanField isFullTimeField;

	public DSSEmployeeMonthCheck(String dataSourceID) {

		super();
		setID(dataSourceID);
		setActionMode(ACTION_MODE_QUERY);
		setActionName("OnGetEmployeeMonthCheck");
		...
		employeeNameField.setLength(20);
		...
		otherLeaveDaysField = new DataSourceFloatField("otherLeaveDays", "其它假类(天)");
		...
		lateForMinutesField = new DataSourceFloatField("lateForMinutes", "迟到分钟");
		...
		adjustField = new DataSourceFloatField("adjust", "调休假");
		...
		overtimePayBeforeField = new DataSourceFloatField("overtimePayBefore", "加班餐补");
		overtimePayBeforeField.setLength(10);
		overtimePayBeforeField.setEmptyDisplayValue("0");
		overtimePayBeforeField.setDecimalPad(2);
		overtimePayBeforeField.setFormat("##,###,###0.00");
		overtimePayBeforeField.setRequired(false);
		overtimePayBeforeField.setHidden(true);
		...
		overtimePaySubBeforeField = new DataSourceFloatField("overtimePaySubBefore", "加班补助");
		overtimePaySubBeforeField.setLength(10);
		overtimePaySubBeforeField.setEmptyDisplayValue("0");
		overtimePaySubBeforeField.setDecimalPad(2);
		overtimePaySubBeforeField.setFormat("##,###,###0.00");
		overtimePaySubBeforeField.setRequired(false);
		overtimePaySubBeforeField.setHidden(false);
		
		isFullTimeField = new DataSourceBooleanField("isFullTime", "是否全勤");
		isFullTimeField.setRequired(false);

		setFields(employeeIdField,employeeNoField, employeeNameField, departmentIdField,statusField,yearField,monthField, numberOfLateField, lateForMinutesField, numberOfLeaveField, LeaveForMinutesField
				, lateChargeField,thingLeavehoursField,  nornalOvertimeHoursField, weekendOvertimeHoursField, holidayOvertimeHoursField,overtimePayField,overtimePayBeforeField,overtimePaySubField,overtimePaySubBeforeField,adjustField, annuaLeaveField, thingLeaveDayField, illLeaveDaysField,otherLeaveDaysField,
				otherLeaveTimesField, outForBusinessField, shouldDaysField, factDaysField, absenceDaysField,isFullTimeField );
	}
