 ...
 *
 */
public class AttachmentInvoiceContracticcisiiorPanel extends VLayout {
	...
	public AttachmentInvoiceContracticcisiiorPanel(ListGridRecord applyRecord) {
		final Integer contractId = applyRecord.getAttributeAsInt("contractId");
		final String customerName = null;
		Integer invoiceApplyId = null;
		setMargin(5);
		...
		leftSignZeroInvoicePanel.setInvoiceType(null);
...
//				}
				...
				if (rightSignOneInvoicePanel.SourceGrid.getRecords().length > 0) {
					if (rightSignOneInvoicePanel.checkData()) {
						...
						double addAllMoney = 0;
						for (ListGridRecord listGridRecord : records) {
							addAllMoney += listGridRecord.getAttributeAsDouble("invoiceAmount");
...
//									}
		...
		mcontractreceivableiicorPanel.hide();
