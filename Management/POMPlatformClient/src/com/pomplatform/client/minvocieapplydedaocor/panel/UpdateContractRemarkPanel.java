...
public class UpdateContractRemarkPanel extends AbstractWizadPage implements HasHandlers{
	
	private final TextAreaItem attachmentRemarkItem;
	...
	public UpdateContractRemarkPanel(){
//		setBorder(BORDER_STYLE);
		...
		attachmentRemarkItem = new TextAreaItem("attachmentRemark", "合同特殊事项备注");
		attachmentRemarkItem.setRequired(true);
		attachmentRemarkItem.setColSpan(4);
		attachmentRemarkItem.setRowSpan(3);
		...
		saveBtn.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				if(checkData()){
					SC.ask("提示", "确定修改?", new BooleanCallback() {
						
						@Override
						public void execute(Boolean value) {
							if(value){
								Map params = __form.getValues();
								params.put("opt_type", "updateContractAttachmentReamrk");
