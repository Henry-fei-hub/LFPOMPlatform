...
public class UpdateMinvocieapplydedaocorPanel extends AbstractWizadPage implements HasHandlers{
	...
	public UpdateMinvocieapplydedaocorPanel(ListGridRecord selectRecord, boolean isUpdate){
		Integer contractId = selectRecord.getAttributeAsInt("contractId");
		Integer invoiceApplyId = selectRecord.getAttributeAsInt("invoiceApplyId");
		int invoiceApplyStatus = selectRecord.getAttributeAsInt("invoiceApplyStatus");
//		setBorder(BORDER_STYLE);
		...
		mcontractisubccacccorPanel.setUpdate(isUpdate);
		...
		mcontractreceivableiicorPanel = new McontractreceivableiicorPanel(isUpdate);
		mcontractreceivableiicorPanel.setContractId(contractId);
...
//        });  
		...
		__form.setValues(selectRecord.toMap());
		...
		if(isUpdate)btnLayout.addMember(saveBtn);
		...
		saveBtn.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				if(checkData()){
					SC.ask("提示", "确认修改?", new BooleanCallback() {
						...
						public void execute(Boolean value) {
							if(value){
								...
								params.put("opt_type", "updateApplyInvoice");
								DBDataSource.callOperation("EP_CustomMinvocieapplydedaocor", "updateApplyInvoice", params, new DSCallback() {
		...
		IButton useInvoiceBtn = new IButton("确认开票");
		useInvoiceBtn.setIcon("[SKIN]/actions/save.png");
		if(!isUpdate && invoiceApplyStatus == 1)btnLayout.addMember(useInvoiceBtn);
		btnLayout.addSpacer(10);
		useInvoiceBtn.addClickHandler(new ClickHandler() {
			
			@Override
			public void onClick(ClickEvent event) {
				PopupWindow pw = new PopupWindow();
				UseInvoiceContracticcisiiorPanel panel = new UseInvoiceContracticcisiiorPanel(selectRecord);
...
//				});
		...
		PermissionControl.createPermissionButton("驳回申请", ERPPermissionStatic.REJECT_APPLY_INVOICES_BTN);
		if(isUpdate || ClientUtil.checkIsHavePermission(ERPPermissionStatic.CONFIRM_ADD_INVOICES_BTN)){
