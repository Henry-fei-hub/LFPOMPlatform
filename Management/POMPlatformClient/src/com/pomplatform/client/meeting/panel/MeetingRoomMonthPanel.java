 ...
 * @author xiaolong
 */
public class MeetingRoomMonthPanel extends DelicacyPanel implements HasHandlers {
    ...
    public Canvas getViewPanel() {
        ...
        saveButton.addClickHandler(new ClickHandler() {
            ...
            public void onClick(ClickEvent event) {
                ...
                if (!BaseHelpUtils.isNullOrZero(monthSelectItem.getValue())) {
                    param.put("monthSelect", monthSelectItem.getValue());
                } else {
                    SC.say("月例会时间不能为空");
                }

                if (!BaseHelpUtils.isNullOrZero(monthDayItem.getValue())) {
                    param.put("monthDay", monthDayItem.getValue());
                } else {
                    SC.say("第几周不能为空");
                }

                if (!BaseHelpUtils.isNullOrZero(wm_startTimeItem.getValue())) {
                    param.put("wmStartTime", wm_startTimeItem.getValue());
                } else {
                    SC.say("开始时间不能为空");
                }

                if (!BaseHelpUtils.isNullOrZero(wm_endTimeItem.getValue())) {
                    param.put("wmEndTime", wm_endTimeItem.getValue());
                } else {
                    SC.say("结束时间不能为空");
                }

                if (!BaseHelpUtils.isNullOrZero(meetingThemeItem.getValue())) {
                    param.put("meetingTheme", meetingThemeItem.getValue());
                } else {
                    SC.say("会议主题不能为空");
                }

                if (!BaseHelpUtils.isNullOrZero(meetingRoomNameItem.getValue())) {
                ...
                if (startTime >= endTime) {
                    ...
                    param.put("optType", "saveMonthMeeting");
