...
/**
 * @author: xiaolong
 * @Date: 2019/8/24 18:45
 */
public class MeetingAppointmentTimeMonthPanel extends DelicacyPanel implements HasHandlers {
    ...
    public void MeetingRoomRecordMonth(int status, int meetingStatus, int meetingRelevanceId, int meetingRoomId, String meetingTheme, int startTime,
                                      int endTime, int weekTime,int howManyWeeks) {
        ...
        monthSelectItem.setValue(weekTime);
        monthDayItem.setValue(howManyWeeks);
    }
    ...
    public Canvas getViewPanel() {
        ...
        monthDayItem = new SelectItem("monthDay", "每月第几周");
        monthDayItem.setWidth("*");
        monthDayItem.setValueMap(KeyValueManager.getValueMap("system_dictionary_233"));

        monthSelectItem = new SelectItem("monthSelect", "月例会时间");
        monthSelectItem.setWidth("*");
        monthSelectItem.setValueMap(KeyValueManager.getValueMap("system_dictionary_23"));
         ...
         * 显示form表单
         */
        meetingForm.setItems(meetingRoomNameItem, meetingThemeItem, wm_startTimeItem, wm_endTimeItem, monthDayItem, monthSelectItem,meetingDateItemStart);
        ...
        saveButton.addClickHandler(new ClickHandler() {
            ...
            public void onClick(ClickEvent event) {
                ...
                if (!BaseHelpUtils.isNullOrEmpty(monthSelectItem.getValue())) {
                    param.put("monthSelect", monthSelectItem.getValue());
                } else {
                    SC.say("月例会时间不能为空");
                }

                if (!BaseHelpUtils.isNullOrEmpty(monthDayItem.getValue())) {
                    param.put("monthDay", monthDayItem.getValue());
                } else {
                    SC.say("第几周不能为空");
                }
                ...
                param.put("status", 1);
                param.put("recordDate", meetingDateItemStart.getValue());


                //设置要下发的人员列表
                MapUtils.convertRecordToMap(employeeResultGrid.getDataSource(), employeeResultGrid.getRecords(), param, "detailConfree");
    ...
    public class Factory implements PanelFactory {
        ...
        public Canvas create() {
            MeetingRoomMonthPanel cm = new MeetingRoomMonthPanel();
            id = cm.getID();
            return cm;
        }
