 ...
 * @author xiaolong
 */
public class MeetingRoomWeekPanel extends DelicacyPanel implements HasHandlers {
    ...
    public Canvas getViewPanel() {
         ...
         * 显示form表单
         */
        meetingForm.setItems(meetingRoomNameItem, meetingThemeItem, wm_startTimeItem, wm_endTimeItem, weekSelectItem);
        ...
        saveButton.addClickHandler(new ClickHandler() {
            ...
            public void onClick(ClickEvent event) {
                ...
                if (BaseHelpUtils.isNullOrZero(wm_startTimeItem.getValue())) {
                    SC.say("开始时间不能为空");
                    return;
                } else if (BaseHelpUtils.isNullOrEmpty(wm_endTimeItem.getValue())) {
                    SC.say("结束时间不能为空");
                    return;
                } else if (BaseHelpUtils.isNullOrEmpty(meetingThemeItem.getValue())) {
                    SC.say("会议主题不能为空");
                    return;
                } else if (BaseHelpUtils.isNullOrEmpty(meetingRoomNameItem.getValue())) {
                    SC.say("会议室不能为空");
                    return;
                } else if (BaseHelpUtils.isNullOrZero(weekSelectItem.getValue())) {
                    SC.say("周例会时间不能为空");
                    return;
                } else {
                ...
                if (startTime >= endTime) {
                    ...
                    param.put("meetingStatus", 0);
                    param.put("optType", "saveWeekMeeting");
    ...
    public class Factory implements PanelFactory {
        ...
        public Canvas create() {
            MeetingRoomWeekPanel cm = new MeetingRoomWeekPanel();
            id = cm.getID();
            return cm;
        }
