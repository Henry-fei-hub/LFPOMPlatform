...
import com.pomplatform.client.purchase.datasource.DSSpurchasepaymentor;
import com.pomplatform.client.purchase.form.SpurchasepaymentorSearchForm;
import com.pomplatform.client.purchase.form.SpurchasepaymentorViewer;
...
public class SpurchasepaymentorPanel extends AbstractSearchPanel {
	...
	public static class Factory implements PanelFactory {
		...
		public Canvas create() {
			SpurchasepaymentorPanel cm = new SpurchasepaymentorPanel();
			id = cm.getID();
			return cm;
		}
	...
	public void initComponents() {
		...
		IButton paymentBtn = new IButton("申请付款");
		controlLayout.addMember(paymentBtn);
		paymentBtn.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				if (resultGrid.anySelected()) {
					...
					if (null == dealStatus || dealStatus == 0 || dealStatus == 2) {
						...
						selectRecord.setAttribute("initDataType", 1);
						distribution.setData(selectRecord);
						distribution.setTitle("采购付款申请");
		...
		IButton viewProcessBtn = new IButton("查看付款流程");
		...
		viewProcessBtn.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				if (resultGrid.anySelected()) {
					Record selectRecord = resultGrid.getSelectedRecord();
					Integer dealStatus = selectRecord.getAttributeAsInt("dealStatus");
					Integer toPersonnelBusinessId = selectRecord.getAttributeAsInt("processBusinessId");
		...
		IButton groupButton = new IButton("分组设置");
		controlLayout.addMember(groupButton);
		groupButton.setTop(525);
		groupButton.setWidth(150);
	...
	public void showDetail() {
		GWT.runAsync(new RunAsyncCallback() {
			...
			public void onSuccess() {
				...
				detail.setTitle("采购付款计划");
				detail.setWidth100();
				detail.setHeight100();
				SpurchasepaymentorViewer detailForm = new SpurchasepaymentorViewer();
	...
	public boolean checkSearchCriteria(Map criteria) {
		criteria.put("addtionalCondition", "order by purchase_payment_id");
		criteria.put("purchasePaymentStatus", 3);
		return true;
	}

	@Override
	public SearchForm generateSearchForm() {
		return new SpurchasepaymentorSearchForm();
	}

	@Override
	public DataSource getDataSource() {
		return DSSpurchasepaymentor.getInstance();
	}
