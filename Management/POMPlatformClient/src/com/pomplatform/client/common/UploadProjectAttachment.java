...
public final class UploadProjectAttachment extends PopupWindow implements HasHandlers {
    private static final Logger __logger = Logger.getLogger("");
    private Map<String, Object> params = new HashMap<>();

    public UploadProjectAttachment(final int projectId,final int stageId,final String remark,final Date timestamp) {
        super("确认函文件上传");
        setWidth("50%");
        setHeight("50%");
        centerInPage();
...
//        vertical.setText(1, 0, "公司名称：" + name);
        ...
        vertical.setHTML(3, 0, "<span><b>注:上传文件只支持.pdf格式</b></span>");
        ...
        button.addClickHandler(new ClickHandler() {
            ...
            public void onClick(ClickEvent event) {
                ...
                if (!ClientUtil.isPdfFile(fileName1)) {
                    SC.say("上传失败，文件格式不对。");
                    return;
                }
                formPanel.setAction(DBDataSource.getServerURL("UP_UploadProjectAttachment"));
                formPanel.submit();
            }
        });
    }
