...
import com.pomplatform.client.projectreportrecorddetail.panel.SettlementEidtDatePanel;
import com.pomplatform.client.salary.datasource.DSSalaryOfFactBonus;
import com.pomplatform.client.salary.form.SalaryOfFactBonusSearchForm;
import com.pomplatform.client.salary.form.SalaryOfFactBonusViewer;
...
public class SalaryOfFactBonusPanel extends AbstractSearchPanel
{
	...
	public static class Factory implements PanelFactory {
		...
		public Canvas create() {
			SalaryOfFactBonusPanel cm = new SalaryOfFactBonusPanel();
			id = cm.getID();
			return cm;
		}
	...
	public void initComponents() {
		...
		resultGrid.addEditCompleteHandler(new EditCompleteHandler() {
			...
			public void onEditComplete(EditCompleteEvent event) {
				...
				if(!BaseHelpUtils.isNullOrEmpty(record)){
					//获取应发奖金和应纳个税，计算实发奖金
					BigDecimal shouldBonus = BaseHelpUtils.getBigDecimalValue(record.getAttribute("shouldBonus"));
					BigDecimal shouldTax = BaseHelpUtils.getBigDecimalValue(record.getAttribute("shouldTax"));
					record.setAttribute("factBonus", shouldBonus.subtract(shouldTax));
		...
		downloadTemplate.addClickHandler(new ClickHandler() {
            @Override
            public void onClick(ClickEvent event) {
            	DownloadFileByUrl.download("/files/factBonus.xlsx");
            }
        });
		...
		IButton updataTimeBut = new IButton("修改日期");
		controlLayout.addMember(updataTimeBut);
		updataTimeBut.addClickHandler(new ClickHandler() {
            @Override
            public void onClick(ClickEvent event) {
				...
				final PopupWindow popupWindow = new PopupWindow("修改日期");
				popupWindow.setWidth("30%");
				popupWindow.setHeight("30%");
				SalaryEidtDatePanel buttonPanel = new SalaryEidtDatePanel();
				Record[] records=resultGrid.getSelectedRecords();
				buttonPanel.setRecord(records);
		...
		IButton updataFlagBut = new IButton("转为应发");
		controlLayout.addMember(updataFlagBut);
		updataFlagBut.addClickHandler(new ClickHandler() {
            @Override
            public void onClick(ClickEvent event) {
            	if (!resultGrid.anySelected()) {
					SC.say("请至少选择一条数据");
	...
	public void showDetail() {
		GWT.runAsync(new RunAsyncCallback() {
			...
			public void onSuccess() {
				...
				detail.setTitle("SalaryOfFactBonus"); 
				detail.setWidth100(); 
				detail.setHeight100(); 
				SalaryOfFactBonusViewer detailForm = new SalaryOfFactBonusViewer();
	...
	public SearchForm generateSearchForm() {
		return new SalaryOfFactBonusSearchForm();
	}

	@Override
	public DataSource getDataSource() {
		return DSSalaryOfFactBonus.getInstance();
	}
