...
public class NormalReimbursementLinkMainProjectPopupPanel extends VLayout implements HasHandlers {

	private static final Logger logger = Logger.getLogger("");
	private final IntegerItem normalReimbursementLinkMainProjectIdItem;
	...
	private LinkedHashMap<Integer, String> projectMap = new LinkedHashMap<>();
	...
	public NormalReimbursementLinkMainProjectPopupPanel(boolean update, LinkedHashMap<Integer, String> allProjectMap,LinkedHashMap<String,String> mainItemMap,LinkedHashMap<String,String> subItemMap) {
		...
		normalReimbursementLinkMainProjectIdItem = new IntegerItem("normalReimbursementLinkMainProjectId", "日常报销关联项目ID");
		normalReimbursementLinkMainProjectIdItem.setHidden(true);
		...
		inputTaxItem.addChangedHandler(new ChangedHandler() {
			...
			public void onChanged(ChangedEvent event) {
				...
				if (null != event.getValue() && null != amountItem.getValue()) {
					FormItem item = event.getItem();
					BigDecimal inputTaxItemRebate = BaseHelpUtils.getBigDecimalValue(inputTaxRebateItem.getValue());
					BigDecimal newValue = new BigDecimal(item.getValue().toString())
							.divide(BigDecimal.ONE, 2, BigDecimal.ROUND_DOWN);
					...
					if (newValue.compareTo(amount) > 0 || newValue.compareTo(BigDecimal.ZERO) < 0) {
						...
						item.setValue(newValue);
						finalAmountItem.setValue(amount.subtract(newValue).add(inputTaxItemRebate));
					}
				}else{
...
//					inputTaxItem.setValue(0);
			...
			public void onChanged(ChangedEvent event) {
				if (null != event.getValue() && null != amountItem.getValue()) {
					...
					if (newValue.compareTo(amount) > 0 || newValue.compareTo(BigDecimal.ZERO) < 0) {
						...
						item.setValue(newValue);
						inputTaxItem.setValue(amount.subtract(newValue).subtract(inputTaxItemRebate));
					}
				}else{
					inputTaxItem.setValue(amountItem.getValue());
		...
		form.setNumCols(2);
		form.setItems(mainItemIdItem, subItemIdItem,normalReimbursementLinkMainProjectIdItem, projectNormalReimbursementIdItem, 
				projectItem, plateIdItem, amountItem, leftAmountItem, inputTaxItem,inputTaxRebateItem, finalAmountItem, haveElectronicInvoiceItem, remarkItem);
...
//				projectItem.setDefaultValue(record.getAttribute("mainProjectId"));
...
//		}
		...
		BigDecimal inputTax = BaseHelpUtils.getBigDecimalValue(inputTaxItem.getValue());
		BigDecimal inputTaxItemRebate = BaseHelpUtils.getBigDecimalValue(inputTaxRebateItem.getValue());
		if (inputTax.compareTo(inputTaxItemRebate) == -1){
