...
import com.pomplatform.client.preprojects.panel.BeProjectBasicInformationUpdate;
import com.pomplatform.client.preprojects.panel.BeProjectBasicInformationUpdateDisable;
import com.pomplatform.client.preprojects.panel.BeProjectDetailInformationUpdate;
import com.pomplatform.client.preprojects.panel.BeProjectDetailInformationUpdateDisable;
...
public class UpdateBeProjectApplyWorkflow extends AbstractProcessPanel {
	...
	public void load() {
		...
		DBDataSource.callOperation("NQ_MainProjectApply", "find", new DSCallback() {
			@Override
	...
	public List<AbstractWizadPage> getPages() {
		...
		SC.debugger();
		BeProjectBasicInformationUpdateDisable basic = new BeProjectBasicInformationUpdateDisable();
		BeProjectDetailInformationUpdateDisable detail = new BeProjectDetailInformationUpdateDisable();
		MainProjectDetailStage detailStage = new MainProjectDetailStage();
		detail.setMainProjectDetailStage(detailStage);
		basic.setR(get__instanceData());
		detail.setR(get__instanceData());
		detailStage.setR(get__instanceData());
		res.add(basic);
		res.add(detail);
		res.add(detailStage);
		return res;
	...
	public Map getValues() {
		...
		for (AbstractWizadPage p : __pages) {
			...
			Object detailProjectStage = map.get("detailProjectStage");
			...
			if(!BaseHelpUtils.isNullOrEmpty(detailProjectStage)){
				values.put("detailProjectStage", detailProjectStage);
			}
		}
		return values;
	}
