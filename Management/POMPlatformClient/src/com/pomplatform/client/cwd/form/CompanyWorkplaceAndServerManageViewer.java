...
public class CompanyWorkplaceAndServerManageViewer extends AbstractDetailViewer
{


	private final DelicacyListGrid companyWorkplaceServersGrid = new DelicacyListGrid();
	private SearchForm __parentSearchForm;


	public CompanyWorkplaceAndServerManageViewer() {
	}

	@Override
	public void initComponents() {
		companyWorkplaceServersGrid.setAutoFitFieldWidths(false);
		super.initComponents();
		companyWorkplaceServersGrid.setDataSource(DSCompanyWorkplaceServer.getInstance());
		companyWorkplaceServersGrid.addDoubleClickHandler(new DoubleClickHandler(){
			@Override
			public void onDoubleClick(DoubleClickEvent event) {
				GenericViewWindow detail = new GenericViewWindow();
				detail.setTitle("工作场所服务器配置信息");
				detail.setWidth(700); detail.setHeight(500); 
				CompanyWorkplaceServerViewer viewer = new CompanyWorkplaceServerViewer();
	...
	public void load() {
		...
		params.put("companyWorkplaceId", getBusinessId());
		DBDataSource.callOperation("NQ_CompanyWorkplaceAndServerManage", "find", new DSCallback() {
			@Override
	...
	public void viewDetailData(){
		...
		condition.put("workplaceId", selected.getAttributeAsString("companyWorkplaceId"));
		DBDataSource.callOperation("ST_CompanyWorkplaceServer", "find", condition, new DSCallback(){
			@Override
			public void execute(DSResponse dsResponse, Object data, DSRequest dsRequest) {
				if(dsResponse.getStatus() >= 0){
					companyWorkplaceServersGrid.setData(dsResponse.getData());
				}
			}
		});
	}
	...
	public List<ListGrid> getDetailListGrids() {
		...
		res.add(companyWorkplaceServersGrid);
		return res;
	}
