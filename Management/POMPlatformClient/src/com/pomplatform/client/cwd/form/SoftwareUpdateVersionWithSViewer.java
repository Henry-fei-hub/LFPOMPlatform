...
public class SoftwareUpdateVersionWithSViewer extends AbstractDetailViewer
{


	private final DelicacyListGrid softwareUpdateFilesGrid = new DelicacyListGrid();


	public SoftwareUpdateVersionWithSViewer() {
	}

	@Override
	public void initComponents() {
		super.initComponents();
		softwareUpdateFilesGrid.setDataSource(DSSoftwareUpdateFile.getInstance());
		softwareUpdateFilesGrid.addDoubleClickHandler(new DoubleClickHandler(){
			@Override
			public void onDoubleClick(DoubleClickEvent event) {
				GenericViewWindow detail = new GenericViewWindow();
				detail.setTitle("");
				detail.setWidth(700); detail.setHeight(500); 
				SoftwareUpdateFileViewer viewer = new SoftwareUpdateFileViewer();
	...
	public void load() {
		...
		params.put("softwareUpdateVersionId", getBusinessId());
		DBDataSource.callOperation("ST_SoftwareUpdateVersionWithS", "find", new DSCallback() {
			@Override
	...
	public void viewDetailData(){
		...
		condition.put("softwareUpdateVersionId", selected.getAttributeAsString("softwareUpdateVersionId"));
		DBDataSource.callOperation("ST_SoftwareUpdateFile", "find", condition, new DSCallback(){
			@Override
			public void execute(DSResponse dsResponse, Object data, DSRequest dsRequest) {
				if(dsResponse.getStatus() >= 0){
					softwareUpdateFilesGrid.setData(dsResponse.getData());
				}
			}
		});
	}
	...
	public List<ListGrid> getDetailListGrids() {
		...
		res.add(softwareUpdateFilesGrid);
		return res;
	}
