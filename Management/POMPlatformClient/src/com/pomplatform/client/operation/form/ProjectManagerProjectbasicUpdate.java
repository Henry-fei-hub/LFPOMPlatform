...
public class ProjectManagerProjectbasicUpdate extends AbstractWizadPage {
	...
	public ProjectManagerProjectbasicUpdate(boolean isAuto) {
...
//		projectManageIdItem.setValueMap(KeyValueManager.getValueMap("use_employees"));
...
//		projectManageIdItem.setWidth("*");
		...
		//如果项目积分变化了,则预留积分和投诉保证金都要变化
//		projectIntegralItem.addChangedHandler(new ChangedHandler() {
//			@Override
//			public void onChanged(ChangedEvent event) {
//				//获取项目总积分
//				BigDecimal totalIntegral = BaseHelpUtils.getBigDecimalValue(totalIntegralItem.getValue());
//				BigDecimal projectIntegral = BaseHelpUtils.getBigDecimalValue(event.getValue());
//				setReserveIntegral(projectIntegral);
//				if(projectIntegral.compareTo(BigDecimal.ZERO) <= 0){
//					SC.say("提示","项目积分不可小于0或等于0");
//					return;
//				}
//				//获取投诉保证金
//				BigDecimal complaintIntegral = BaseHelpUtils.getBigDecimalValue(complaintIntegralItem.getValue());
//				//设置预留积分=项目总积分-项目积分-投诉保证金
//				BigDecimal reserveIntegral = totalIntegral.subtract(projectIntegral).subtract(complaintIntegral);
//				//如果预留积分为负数，则说明项目积分分配过多
//				if(reserveIntegral.compareTo(BigDecimal.ZERO) < 0){
//					SC.say("提示","当前分配项目积分过多,目前最多可分配积分("+totalIntegral.subtract(complaintIntegral)+")");
//					projectIntegralItem.setValue(totalIntegral.subtract(complaintIntegral));
//					reserveIntegral = BigDecimal.ZERO;
//				}
//				
//				reserveIntegralItem.setValue(reserveIntegral);
//				//设置核算比例=预留积分/(项目积分+预留积分)
//				BigDecimal percentage = reserveIntegral.multiply(BigDecimal.valueOf(100)).divide((projectIntegral.add(reserveIntegral)),5,BigDecimal.ROUND_DOWN);
...
//		initiateDesignItem.setValueMap(KeyValueManager.getValueMap("employees"));
...
//		if(!BaseHelpUtils.isNullOrEmpty(__form.getItem("timeCycle"))){
//			if(BaseHelpUtils.isNullOrEmpty(timeCycleItem.getValue())){
//				SC.say("请填写时间周期(月)");
//				return false;
//			}
//		}
//		if(BaseHelpUtils.getBoolean(isOrNotItem.getValue())) {
//			if(BaseHelpUtils.isNullOrEmpty(initiateDesignItem.getValue())||BaseHelpUtils.getIntValue(initiateDesignItem.getValue())<=0) {
//				SC.say("方案主创不能为空");
//				return false;
//			}
//		}
