...
public class OnProjectQuoteTemplateNewForm extends AbstractWizadPage
{
	...
	public OnProjectQuoteTemplateNewForm() {
		...
		aFields[a].addChangedHandler(new ChangedHandler() {
			...
			public void onChanged(ChangedEvent event) {
				...
				//获取单价
				BigDecimal areaPrice=BaseHelpUtils.getBigDecimalValue(record.getAttribute("areaPrice"));
				//小计
		...
		grid.addDataChangedHandler(new DataChangedHandler() {
			...
			public void onDataChanged(DataChangedEvent event) {
				...
				for(int i=0;i<recordGet.length;i++) {
					...
					areaPrices=BaseHelpUtils.getBigDecimalValue(records[i].getAttribute("areaPrice"));
	...
	public void startEdit() {
		...
		if(record != null) {
			Record[] records=record.getAttributeAsRecordArray("info");
			Record formRecord=record.getAttributeAsRecord("formRecord");
			Record topLeftFormRecord=record.getAttributeAsRecord("topLeftFormRecord");
			Record topRightFormRecord=record.getAttributeAsRecord("topRightFormRecord");
			form.editRecord(formRecord);
			String  customerName=BaseHelpUtils.getString(formRecord.getAttribute("customerName"));
			...
			int projectQuoteTemplatesParentId=BaseHelpUtils.getIntValue(projectQuoteTemplatesParentIdItem.getValue());
			maps.put("projectQuoteTemplatesParentId",projectQuoteTemplatesParentId);
			...
			topLeftForm.editRecord(topLeftFormRecord);
			topRightForm.editRecord(topRightFormRecord);
	...
	public void setValueManage(ValuesManager manager) {
		manager.setDataSource(DSOnProjectQuoteTemplate.getInstance());
		manager.addMember(__form);
	}

	@Override
	public java.util.Map getValuesAsMap() {
		//detailProjectQuoteDetail
...
//		MapUtils.convertRecordToMap(bdDetailGrid,bdDetailGrid.getRecords(),valuesMap,"detailBudgetManagementDetail");
		...
		Map<String,Object> valuesMap= topRightForm.getValues();
		//获取项目id
		int projectId=BaseHelpUtils.getIntValue(projectIdItem.getValue());
		//获取业务类别Id
		int businessType=BaseHelpUtils.getIntValue(projectTypeItem.getValue());
		int projectQuoteTemplatesParentId=BaseHelpUtils.getIntValue(projectQuoteTemplatesParentIdItem.getValue());
		valuesMap.put("projectId",projectId);
		valuesMap.put("businessType",businessType);
		valuesMap.put("remark",remarkItem.getValue());
		valuesMap.put("operateEmployeeId",ClientUtil.getUserId());
		valuesMap.put("operateTime",new Date());
		valuesMap.put("projectQuoteTemplatesParentId",projectQuoteTemplatesParentId);
		
        
		MapUtils.convertRecordToMap(grid,grid.getRecords(),valuesMap,"detailProjectQuoteDetail");
