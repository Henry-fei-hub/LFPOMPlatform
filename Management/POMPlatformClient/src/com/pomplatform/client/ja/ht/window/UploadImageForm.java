package com.pomplatform.client.ja.ht.window;
...
public final class UploadImageForm extends PopupWindow implements HasHandlers {

    private static final Logger __logger = Logger.getLogger("");
    private Map<String, Object> params = new HashMap<>();

    public UploadImageForm(final int outQuotationId,final String infoCode, final String quotationNumber,final String quotationName) {
        super("文件上传");
        setWidth("30%");
        setHeight("26%");
        centerInPage();
...
//                        SC.say("文件上传成功");
        ...
        vertical.setText(1, 0, "报价单名称：" + quotationName);
        final FileUpload fileUpload = new FileUpload();
        fileUpload.setName("上传报价单(原附件)");
        vertical.setWidget(2, 0, fileUpload);
        vertical.setHTML(3, 0, "<span><b>注:图片应为 JPG、PNG 或 GIF 格式且不超过 4 MB</b></span>");

        for (int i = 0; i < 4; i++) {
            vertical.getRowFormatter().getElement(i).getStyle().setHeight(14, Unit.PX);
        }
        ...
        button.addClickHandler(new ClickHandler() {
            ...
            public void onClick(ClickEvent event) {
                ...
                params.put("quotationNumber", quotationNumber);
                params.put("quotationName", quotationName);
                vertical.setWidget(4, 0, MapUtils.generateFormDataForUpload(params));
                if (fileName1 == null || fileName1.trim().length() <= 0) {
                    SC.say("请点击 选择文件 按钮选择你要上传的Img");
                    return;
                }
                if (!ClientUtil.isImageFileName(fileName1)) {
                    SC.say("上传失败，图片格式不对。");
                    return;
                }
                formPanel.setAction(DBDataSource.getServerURL("UP_UploadQuotationImage"));
                formPanel.submit();
            }
        });
    }
