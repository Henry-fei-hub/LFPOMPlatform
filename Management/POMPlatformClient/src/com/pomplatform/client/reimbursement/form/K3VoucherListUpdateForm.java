...
public class K3VoucherListUpdateForm extends AbstractWizadPage
{
	...
	public K3VoucherListUpdateForm() {
...
//		secondaryCodeIdItem.setValueMap(KeyValueManager.getValueMap("k3_code_name"));
		...
		amountOfReimbursementItem = new DoubleItem("amountOfReimbursement", "报销金额");
		amountOfReimbursementItem.setWidth("*");
		amountOfReimbursementItem.setLength(18);
		amountOfReimbursementItem.setDecimalPad(2);
		amountOfReimbursementItem.setFormat("#,###,###,###,###,##0.00");
		amountOfReimbursementItem.addChangedHandler(new ChangedHandler() {
			@Override
			public void onChanged(ChangedEvent event) {
				numberFormat(event);
				if(null != event.getValue()){
					debitSideItem.setValue(ClientUtil.checkAndGetDoubleValue(amountOfReimbursementItem.getValue()) - ClientUtil.checkAndGetDoubleValue(inputTaxItem.getValue()));
				}
			}
		});
		__formItems.add(amountOfReimbursementItem);
		
		inputTaxItem = new DoubleItem("inputTax", "进项税");
		inputTaxItem.setWidth("*");
		inputTaxItem.setLength(18);
		inputTaxItem.setDecimalPad(2);
	...
	public void startEdit() {
		...
		if(record != null){
			...
			if(secondaryCodeType == 1){
				secondaryCodeItem.setOptionDataSource(DSCompanyK3Code.getInstance());
			}else if(secondaryCodeType == 2){
				secondaryCodeItem.setOptionDataSource(DSEmployeeK3Code.getInstance());
			}
		}
	}
