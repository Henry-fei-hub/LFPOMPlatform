...
import com.pomplatform.client.workflow.commonNormalReimbursement.datasource.DSNormalReimbursementWithDepartmentDispose;
 ...
 * @author CL
 */
public class CustomAddSubContractMainTwoDisposePanel extends AbstractWizadPage implements HasHandlers {
    private static final Logger __logger = Logger.getLogger("");
    public static CustomAddChildContractDisposePanel childContract;//子订单列表
    public static CustomAddConsiderationPanel consideration;//项目信息列表
    public static CustomAddChildManageContractDisposePanel childManageContract;//管理订单列表
    ...
    final int __contractId;
    final int __orderInfoId;

    @SuppressWarnings({"deprecation", "static-access"})
    public CustomAddSubContractMainTwoDisposePanel(int contractId, int orderInfoId, Record record, int flowStatus, Map<String, String> map) {
        SC.debugger();
        setHeight100();
        setMembersMargin(5);
        __contractId = contractId;
        __orderInfoId = orderInfoId;
        // 获取项目编号
        ...
        childManageContract = new CustomAddChildManageContractDisposePanel(flowStatus, map);
        childManageContract.setContractId(contractId);
        childManageContract.setOrderInfoId(orderInfoId);
        ...
        childContract = new CustomAddChildContractDisposePanel(flowStatus, consideration);
        childContract.setHeight("50%");
        childContract.setContractId(contractId);
        childContract.setOrderInfoId(orderInfoId);
                ...
                if (records.length > 0) {
                    SC.ask("提示", "确认创建？", new BooleanCallback() {
                        ...
                        public void execute(Boolean value) {
                            ...
                            if (value) {
                                for (Record r : childContract.SourceGrid.getRecords()) {
                                ...
                                int re = diffValue.compareTo(BigDecimal.ZERO);
                                if (re > 0) {
                                    SC.debugger();
                                    Record r = records[0];
                                    ...
                                    if (!BaseHelpUtils.isNullOrEmpty(r)) {
                                        ...
                                        DBDataSource.callOperation("EP_CustomSubContractFlowHandler", "", params, new DSCallback() {
                                            ...
                                            public void execute(DSResponse dsResponse, Object data, DSRequest dsRequest) {
                                                if (dsResponse.getStatus() >= 0) {
                                                    SC.debugger();
                                                    childContract.SourceGrid.startEditingNew(dsResponse.getData()[0]);
                                                    childContract.redraw();
//												consideration.startEdit();
                                                }
                                            }
                                        });
            ...
            public void onClick(ClickEvent event) {
                ...
                if (records.length > 0) {
                    ...
                    SC.ask("提示", "确认撤销？", new BooleanCallback() {
                        ...
                        public void execute(Boolean value) {
                            if (value) {
                                ListGridRecord selected = childContract.SourceGrid.getSelectedRecord();
                                int indexNum = childContract.SourceGrid.getRowNum(selected);
                                childContract.SourceGrid.removeData(selected);
                                childContract.SourceGrid.selectSingleRecord(indexNum);
//                                // 获取子合同主键
...
//				childManageContract.SourceGrid.selectRecord(newRecord);
                ...
                childManageContract.SourceGrid.startEditingNew(manageList[0]);
                childManageContract.redraw();
                // 先将数据保存数据库
//                Map<String, Object> params = new HashMap<>();
//                params.put("opt_type", "addProjectManageSubContract");
//                params.put("contractId", __contractId);
...
//                });
        ...
        delMlgBtn.addClickHandler(new ClickHandler() {
            @Override
            public void onClick(ClickEvent event) {
                ListGridRecord selected = childManageContract.SourceGrid.getSelectedRecord();
                if (BaseHelpUtils.isNullOrEmpty(selected)) {
                    SC.say("提示", "请选择一条数据");
                    return;
                }
//                if (BaseHelpUtils.isNullOrEmpty(selected.getAttribute("subContractId"))) {
                    int indexNum = childManageContract.SourceGrid.getRowNum(selected);
                    childManageContract.SourceGrid.removeData(selected);
                    childManageContract.SourceGrid.selectSingleRecord(indexNum);
//                }else{
//                    Map<String, Object> params = new HashMap<>();
//                    params.put("subContractId", BaseHelpUtils.getIntValue(selected.getAttribute("subContractId")));
...
//                }
        ...
        __infoForm.setDataSource(DSNormalReimbursementWithDepartmentDispose.getInstance());
        __infoForm.setNumCols(2);
        ClientUtil.DynamicFormProcessAccordingToDevice(__infoForm);
        subContractRemark = new TextAreaItem("remark", "订单备注");
        ...
        attachmentNameItem = new TextItem("fileName", "附件名称");
...
//        rightButtonLayout.addMember(submitButto);//提交订单
        rightButtonLayout.addMember(breakLabel);
        rightButtonLayout.addMember(breakLabel);
        rightButtonLayout.addMember(breakLabel);
        rightButtonLayout.addMember(breakLabel);
//        rightButtonLayout.addMember(saveRemarkBtn);//保存按钮
        ...
        //加载业务部门的数据集
        onLoadPlateRecords();
        onLoadSubContractRemark(orderInfoId);
    }


    public static void onLoadSubContractRemark(int orderInfoId) {
        Map<String, Object> params = new HashMap<>();
        ...
        DBDataSource.callOperation("EP_CustomSubContractFlowHandler", params, new DSCallback() {
            ...
            public void execute(DSResponse dsResponse, Object data, DSRequest dsRequest) {
                ...
                if (dsResponse.getStatus() >= 0 && dsResponse.getData().length > 0
                    ...
                    __infoForm.getItem("remark").setValue(BaseHelpUtils.getString(r.getAttribute("remark")));
                    __infoForm.getItem("fileUrl").setValue(BaseHelpUtils.getString(r.getAttribute("fileUrl")));
                    __infoForm.getItem("fileId").setValue(BaseHelpUtils.getString(r.getAttribute("fileId")));
                    __infoForm.getItem("fileName").setValue(BaseHelpUtils.getString(r.getAttribute("fileName")));
                } else {
                    ClientUtil.displayErrorMessage(dsResponse);
                }
            }
...
//	}
    ...
    public Map getValuesAsMap() {
        Record[] manageRecords = childManageContract.SourceGrid.getRecords();
        Record[] childRecords = childContract.SourceGrid.getRecords();
        Record[] mainRecords = consideration.resultGrid.getRecords();
        Map<String, Object> params = new HashMap<>(0);
        for (Record r:childRecords){
            if(!BaseHelpUtils.isNullOrEmpty(r.getAttribute("orderInfoId"))){
                params.put("orderInfoId", r.getAttribute("orderInfoId"));
                break;
            }
        }
//        params.put("opt_type", "saveContentInfo");
        ...
        params.put("contractId", __contractId);
        params.put("flowStatus", 3);
        params.put("attachment", attachmentItem.getValue());
        params.put("attachmentId", attachmentIdItem.getValue());
        params.put("attachmentName", attachmentNameItem.getValue());
        params.put("contentInfo", getReplaceAll(__infoForm.getItem("remark").getValue() + ""));
//        params.putAll(childManageContract.getValuesAsMap());
        params.put("manageRecords",manageRecords);
        params.put("childRecords",childRecords);
        params.put("mainRecord",mainRecords[0]);
        return params;
    }
    ...
    public boolean checkData() {
        SC.debugger();
        Map<String, Object> params = new HashMap<>();
        params.put("opt_type", "checkedDepartmentManage");
        params.put("contractId", __contractId);
        DBDataSource.callOperation("EP_CustomMainContractHandler", params, new DSCallback() {
            @Override
            public void execute(DSResponse dsResponse, Object data, DSRequest dsRequest) {
