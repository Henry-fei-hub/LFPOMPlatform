...
public class ScapitaldistributionorUpdateForm extends AbstractWizadPage {
	...
	public static Boolean __isFrance = false;
	
	private DelicacyListGrid subContractGrid;//子订单Grid

	public ScapitaldistributionorUpdateForm(String processName) {
		...
		capitalDistributionIdItem.setDisabled(true);
		capitalDistributionIdItem.setRequired(true);
		IsIntegerValidator capitalDistributionIdValidator = new IsIntegerValidator();
		capitalDistributionIdItem.setValidators(capitalDistributionIdValidator);
		capitalDistributionIdItem.hide();
		__formItems.add(capitalDistributionIdItem);
...
//						}
	...
	public void startEdit() {
		if (getRecord() != null){
			...
			int capitalId = BaseHelpUtils.getIntValue(capitalIdItem.getValue());
			setCapitalsSelectValue(contractId);
			setMainProfessional(contractId);
			setContractInfo(contractId);
			setCapitalAmount(capitalId);
			if(__isFrance)setGridData(contractIdItem.getValue(), mainProfessionalItem.getValue());
		};
	}
