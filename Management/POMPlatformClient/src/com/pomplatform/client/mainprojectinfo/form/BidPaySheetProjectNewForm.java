...
//	private final BooleanItem provideAccommodationItem;
	...
	public BidPaySheetProjectNewForm(Record data) {
...
//		employeeIdItem.hide();
		...
		employeeNoItem.setDefaultValue(data.getAttribute("contractCode"));
		employeeNoItem.hide();
		employeeNoItem.setDisabled(true);
		employeeNoItem.setWidth("*");
		__formItems.add(employeeNoItem);
		planItem = new TextItem("plan", "合同名称");
		planItem.setDefaultValue(data.getAttribute("contractName"));
		planItem.hide();
		...
		unitItem.setDefaultValue(data.getAttribute("infoCode"));
		unitItem.setDisabled(true);
		unitItem.setWidth("*");
		__formItems.add(unitItem);
		
		projectCodeItem = new TextItem("projectCode", "项目编号");
		projectCodeItem.setDefaultValue(data.getAttribute("projectCode"));
		projectCodeItem.setDisabled(true);
		projectCodeItem.setWidth("*");
		__formItems.add(projectCodeItem);
		
		projectNameItem = new TextItem("projectName", "项目名称");
		projectNameItem.setDefaultValue(data.getAttribute("projectName"));
		...
		companyProvinceItem.setDefaultValue(data.getAttribute("designTeam"));
		...
		transportationItem.setDefaultValue(data.getAttribute("customerName"));
		transportationItem.setDisabled(true);
		transportationItem.setWidth("*");
		__formItems.add(transportationItem);
		
		livingExpensesItem = new DoubleItem("livingExpenses", shouldNotBeNull+"投标金额(元)");
//		livingExpensesItem.setDefaultValue(data.getAttribute("signingMoneySum"));
		livingExpensesItem.setWidth("*");
		__formItems.add(livingExpensesItem);
		startDateItem = new DateItem("startDate", shouldNotBeNull+"投标日期");
		startDateItem.setWidth("*");
		startDateItem.setUseTextField(true);
		__formItems.add(startDateItem);
		startDateDetailItem = new SelectItem("startDateDetail",shouldNotBeNull+"投标类型");
		...
		endDateDetailItem = new IntegerItem("endDateDetail",shouldNotBeNull+"投标单位总数");
		endDateDetailItem.setWidth("*");
		__formItems.add(endDateDetailItem);
		addressItem = new TextItem("address",shouldNotBeNull+"我方排名");
		addressItem.setWidth("*");
		__formItems.add(addressItem);
		severanceItem = new DoubleItem("severance",shouldNotBeNull+"甲方补偿金额");
		severanceItem.setWidth("*");
		__formItems.add(severanceItem);
		
		projectTypeItem = new SelectItem("projectType",shouldNotBeNull+"关闭原因");
		...
		reasonItem = new TextAreaItem("reason", shouldNotBeNull+"落标原因详情");
		reasonItem.setWidth("*");
		...
		linkItem.addTitleClickHandler(new TitleClickHandler() {
			...
			public void onTitleClick(TitleClickEvent event) {
				new CommonUploadWindow("上传报价单附件", false, 100, null, 1, new UploadCallBack() {
