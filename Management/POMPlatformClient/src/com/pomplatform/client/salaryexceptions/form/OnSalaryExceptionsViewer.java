...
public class OnSalaryExceptionsViewer extends AbstractDetailViewer
{


	private final DelicacyListGrid salaryExceptionsGrid = new DelicacyListGrid();
	private SearchForm __parentSearchForm;


	public OnSalaryExceptionsViewer() {
	}
    
	@Override
	public void initComponents() {
		super.initComponents();
		salaryExceptionsGrid.setDataSource(DSSalaryException.getInstance());
		salaryExceptionsGrid.addDoubleClickHandler(new DoubleClickHandler(){
			@Override
			public void onDoubleClick(DoubleClickEvent event) {
				GenericViewWindow detail = new GenericViewWindow();
				detail.setTitle("工资异常申请表");
				detail.setWidth(700); detail.setHeight(500); 
				SalaryExceptionViewer viewer = new SalaryExceptionViewer();
	...
	public void load() {
		...
		DBDataSource.callOperation("NQ_OnSalaryExceptions", "find", new DSCallback() {
			@Override
	...
	public void viewDetailData(){
		...
		DBDataSource.callOperation("ST_SalaryException", "find", condition, new DSCallback(){
			...
			public void execute(DSResponse dsResponse, Object data, DSRequest dsRequest) {
				if(dsResponse.getStatus() >= 0){
					salaryExceptionsGrid.setData(dsResponse.getData());
				}
			}
		});
	}
