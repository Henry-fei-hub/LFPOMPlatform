...
public class RightSignTwoBottomInvoicePanel extends AbstractWizadPage implements HasHandlers{
	...
	public RightSignTwoBottomInvoicePanel(){
			...
			public void onChanged(ChangedEvent event) {
				if(null != event.getValue()){
					...
					if(!BaseHelpUtils.isNullOrEmpty(df.getField("taxRate").getValue())){
						...
						double outputTax = BaseHelpUtils.get2Double(thisVal / (1 + taxRate/100));
						double noTaxAmount = BaseHelpUtils.get2Double(thisVal - outputTax);
			...
			public void onChanged(ChangedEvent event) {
				if(null != event.getValue()){
					...
					if(!BaseHelpUtils.isNullOrEmpty(df.getField("invoiceAmount").getValue())){
						...
						double outputTax = BaseHelpUtils.get2Double(invoiceAmount / (1 + thisVal/100));
						double noTaxAmount = BaseHelpUtils.get2Double(invoiceAmount - outputTax);
						df.getField("noTaxAmount").setValue(noTaxAmount);
						df.getField("outputTax").setValue(outputTax);
					}
				}
