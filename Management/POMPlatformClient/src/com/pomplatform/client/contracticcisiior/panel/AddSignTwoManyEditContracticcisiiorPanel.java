 ...
 *
 */
public class AddSignTwoManyEditContracticcisiiorPanel extends VLayout {
	...
	public final RightSignTwoEditInvoicePanel rightSignTwoEditInvoicePanel; // 选中的发票数据

	public AddSignTwoManyEditContracticcisiiorPanel(final Integer contractId, final String contractName) {
		setMargin(5);
		...
		rightBtn.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				DelicacyListGrid  toGrid = rightSignTwoEditInvoicePanel.SourceGrid;
				...
				if (fromGrid.anySelected()) {
					...
					for (ListGridRecord listGridRecord : records) {
						listGridRecord.setAttribute("contractId", contractId);
						listGridRecord.setAttribute("sign", 2);
		...
		leftBtn.addClickHandler(new ClickHandler() {
			...
			@Override
			public void onClick(ClickEvent event) {
				DelicacyListGrid fromGrid = rightSignTwoEditInvoicePanel.SourceGrid;
		...
		saveBtn.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				if (rightSignTwoEditInvoicePanel.SourceGrid.getRecords().length > 0) {
					if (rightSignTwoEditInvoicePanel.checkData()) {
						SC.ask("提示", "确认保存?保存后将会使用选择的发票为负数发票！！！", new BooleanCallback() {
							...
							public void execute(Boolean value) {
								if (value) {
									...
									// 获取使用的发票信息
									ListGridRecord[] records = rightSignTwoEditInvoicePanel.SourceGrid.getRecords();
									for (ListGridRecord listGridRecord : records) {
		...
		rightSignTwoEditInvoicePanel = new RightSignTwoEditInvoicePanel();

		// 面板布局添加
		rightMainLayout.setPadding(5);
		rightMainLayout.addMember(rightSignTwoEditInvoicePanel); // ******************
