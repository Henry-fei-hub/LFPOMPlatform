 ...
 *
 */
public class AddSignZeroContracticcisiiorPanel extends VLayout {
	...
	public final ContractReceivablePanel contractReceivablePanel; // 合同阶段面板

	public AddSignZeroContracticcisiiorPanel(final Integer contractId, final String contractName) {
		setMargin(5);
				...
				if (fromGrid.anySelected()) {
					...
					for (ListGridRecord listGridRecord : records) {
						...
						if(!flag){
							...
							if(null != contractReceivablePanel.customerName){
								listGridRecord.setAttribute("clientName", contractReceivablePanel.customerName);
							}
...
//				}
					...
					if (rightSignOneInvoicePanel.checkData()) {
						SC.ask("提示", "确认保存?保存后将会使用选择的发票！！！", new BooleanCallback() {
							...
							public void execute(Boolean value) {
								if (value) {
									Object contractId = contractReceivablePanel.contractIdItem.getValue();
		...
		contractReceivablePanel = new ContractReceivablePanel(contractId);
		contractReceivablePanel.setContractId(contractId);
		contractReceivablePanel.setContractName(contractName);
		contractReceivablePanel.startEdit();
		contractReceivablePanel.setHeight(60);
		// 面板布局添加
