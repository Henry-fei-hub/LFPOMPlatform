...
public final class UploadReimburseData extends PopupWindow implements HasHandlers {
    private static final Logger __logger = Logger.getLogger("");
    private Map<String, Object> params = new HashMap<>();
    
    private DelicacyListGrid grid;

    private DelicacyListGrid detailGrild;
    
    private ListGrid oneGrid;
    
    UploadReimburseReturnListen returnListen = null;
	...
	public ListGrid getOneGrid() {
		return oneGrid;
	}

	public void setOneGrid(ListGrid oneGrid) {
		this.oneGrid = oneGrid;
	}
	
	public void setReturnDataListen(UploadReimburseReturnListen listen) {
		returnListen = listen;
	};

	public UploadReimburseData( final String title, final Map<String, Object> map, final String action) {
        super(title);
        setWidth("50%");
        setHeight("50%");
        centerInPage();
        ...
        formPanel.addSubmitCompleteHandler(new SubmitCompleteHandler() {
            ...
            public void onSubmitComplete(SubmitCompleteEvent event) {
                try {
                    ...
                    if (res.getStatus() >= 0) {
                        if(null != getGrid()){
                        	getGrid().setData(res.getData());
                        }
                        if(null != getOneGrid()){
                        	getOneGrid().setData(res.getData());
                        }
                        if(null != returnListen) {
                        	returnListen.returnData(res.getData());
                        }
                    } else {
                        SC.say("警告", res.getErrors().get("errorMsg").toString());
                    }
                    Record record = new Record();
                    record.setAttribute("status", res.getStatus());
                    record.setAttribute("errorMsg", res.getErrors().get("errorMsg").toString());
...
//        vertical.setText(1, 0, "公司名称：" + name);
        ...
        vertical.setHTML(3, 0, "<span><b>注:支持导入文件包括(.xls .xlsx)</b></span>");
