...
public class NormalReimbursementLinkDepartmentPopupPanel extends VLayout implements HasHandlers {
	...
	public NormalReimbursementLinkDepartmentPopupPanel(boolean update, boolean editInputTax,
			LinkedHashMap<String, String> mainItemMap, final LinkedHashMap<String, String> subItemMap) {
		...
		amountItem.addChangedHandler(new ChangedHandler() {
			...
			public void onChanged(ChangedEvent event) {
				if (null != event.getValue() && null != amount) {
					...
					BigDecimal newValue = new BigDecimal(item.getValue().toString()).divide(BigDecimal.ONE, 2,
							BigDecimal.ROUND_DOWN);
					item.setValue(newValue);
					// leftAmountItem.setValue(getAmount().subtract(newValue));
					finalAmountItem.setValue(newValue.subtract(new BigDecimal(inputTaxItem.getValueAsString())
		...
		inputTaxItem.addChangedHandler(new ChangedHandler() {
			...
			public void onChanged(ChangedEvent event) {
				...
				if (null != event.getValue() && null != amountItem.getValue()) {
					...
					BigDecimal newValue = new BigDecimal(item.getValue().toString()).divide(BigDecimal.ONE, 2,
							BigDecimal.ROUND_DOWN);
					BigDecimal amount = new BigDecimal(amountItem.getValue().toString()).divide(BigDecimal.ONE, 2,
							BigDecimal.ROUND_DOWN);
		...
		form.setNumCols(2);
		form.setItems(mainItemIdItem, subItemIdItem, normalReimbursementLinkDepartmentIdItem,
				projectNormalReimbursementIdItem, departmentItem, amountItem, leftAmountItem, inputTaxItem,
				inputTaxRebateItem, finalAmountItem, haveElectronicInvoiceItem, remarkItem);
	...
	public void startEdit(Record record, BigDecimal amount, boolean update) {
		if (update) {
			...
			if (null != amount) {
				...
				inputTaxItem.setDefaultValue(0);
			...
			if (record != null) {
				...
				if (!BaseHelpUtils.isNullOrEmpty(budgetManagementId) && budgetManagementId > 0) {// 如果有预算
					// 获取付款金额
					...
					amounts = payAmount.add(reverseAmount);
					amountItem.setValue(amounts);
					finalAmountItem.setValue(amounts);
					TreeNode[] nodes = tree.getAllNodes();
					String value = BaseHelpUtils.getString(ClientUtil.getDepartmentId());
					for (TreeNode node : nodes) {
						if (node.getName().equals(value)) {
							departmentItem.setValue(tree.getPath(node));
							break;
						}
					}
				}
	...
	public boolean DataValidate() {
		...
		// 进项税和成本的和
		BigDecimal moneySum = (BaseHelpUtils.getBigDecimalValue(inputTaxItem.getValue())
				.add(BaseHelpUtils.getBigDecimalValue(finalAmountItem.getValue())))
						.subtract(BaseHelpUtils.getBigDecimalValue(inputTaxRebateItem.getValue()));
		logger.info("money:" + money + "; moneySum:" + moneySum);
		if (moneySum.compareTo(money) != 0) {
			SC.say("进项税和成本之和减去进项税回扣不等于费用，请重新调整数据");
			return false;
		}

		BigDecimal inputTax = BaseHelpUtils.getBigDecimalValue(inputTaxItem.getValue());
		BigDecimal inputTaxRebate = BaseHelpUtils.getBigDecimalValue(inputTaxRebateItem.getValue());
		if (inputTax.compareTo(inputTaxRebate) == -1) {// 进项税转出大于
