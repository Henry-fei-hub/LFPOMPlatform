...
public class StayawayApplyNewForm extends AbstractWizadPage
{
...
//		endDateDetailItem.setDefaultValue(1);
...
//			public void onChange(ChangeEvent event) {
//				if(null != startDateItem.getValue() && null != endDateItem.getValue()){
//					countDays(startDateItem.getValueAsDate(), endDateItem.getValueAsDate(), event);
//				}
//			}
//		});
...
//		c.addCriteria("employeeId", ClientUtil.getUserId());
		...
		projectSelectItem.addChangedHandler(new ChangedHandler() {
			...
			public void onChanged(ChangedEvent event) {
				if(null != event.getValue()){
					...
					remarkItem.setValue(record.getAttributeAsString("projectCode")+"-"+record.getAttributeAsString("projectName"));
				}
			}
		});
		...
		preProjectSelectItem.addChangedHandler(new ChangedHandler() {
			@Override
			public void onChanged(ChangedEvent event) {
				if(null != event.getValue()){
					Record record = preProjectSelectItem.getSelectedRecord();
					remarkItem.setValue(record.getAttributeAsString("customNumber")+"-"+record.getAttributeAsString("projectName"));
				}
			}
		});
		...
		ct.addCriteria("plateId", ClientUtil.getPlateId());
...
//		Label label = new Label("<font style=\"font-weight:bold\">温馨提示：以<font style=\"color:#FF7F27\">外出日期</font>的月份为准，该月份的加班/外出/出差申请须在次月5日前（包括次月5日）提出申请</font>");
