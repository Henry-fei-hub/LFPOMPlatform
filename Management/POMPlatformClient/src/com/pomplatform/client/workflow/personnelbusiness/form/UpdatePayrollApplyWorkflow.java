...
public class UpdatePayrollApplyWorkflow extends AbstractProcessPanel {
	...
	public List<AbstractWizadPage> getPages() {
		...
		PayrollApplyUpdateForm mainForm = new PayrollApplyUpdateForm(getProcessName());
		mainForm.setPageMode(AbstractWizadPage.PAGE_MODE_UPDATE);
		mainForm.setProcessName(getProcessName());
		mainForm.initPanel();
		PersonnelBusinessLinkTaxAmountPanel panel = new PersonnelBusinessLinkTaxAmountPanel();
		panel.setPageMode(AbstractWizadPage.PAGE_MODE_UPDATE);
		res.add(mainForm);
		if(this.processName.contains("报税")){
			res.add(panel);
		}
		return res;
	}
