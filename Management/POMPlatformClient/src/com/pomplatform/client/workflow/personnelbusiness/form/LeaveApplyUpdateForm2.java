...
public class LeaveApplyUpdateForm2 extends AbstractWizadPage
{
	...
	private IButton attachmentButton;
	private Object subtractDays = BigDecimal.ZERO;
	private Object subtractHours = BigDecimal.ZERO;
	...
	public LeaveApplyUpdateForm2() {
		DSLeaveApply ds = DSLeaveApply.getInstance();
		formLeft.setDataSource(ds);
		formRight.setDataSource(ds);
		left = new VLayout();
		left.setWidth100();
		left.setHeight100();
		...
		nextAuditorItem = new ComboBoxItem("nextAuditor", "交接人");
		...
		typeItem.addChangedHandler(new ChangedHandler() {
			...
			public void onChanged(ChangedEvent event) {
				...
				if(null == event.getValue()){
					...
					maxDays = ClientUtil.checkAndGetDoubleValue(daysMap.get(event.getValue().toString()));
					Map<String, Object> params = new HashMap<>();
					params.put("optType", "getLeaveTypes");
					params.put("employeeId", employeeIdItem.getValue());
...
//								daysItem.setTitle("请假天数(最多可请假<font style = 'color:red'>"+actualDays+"</font>小时)");
		...
		numberOfBabyItem.addChangedHandler(new ChangedHandler() {
			...
			public void onChanged(ChangedEvent event) {
				...
				for(String obj : arr){
					if(KeyValueManager.getValue("leave_types", obj).equals(LEAVE_TYPE_10)){
					}
					days += ClientUtil.checkAndGetDoubleValue(daysMap.get(obj));
				}
				actualDays = days;
		...
		startDateItem.addChangeHandler(new ChangeHandler() {
			...
			public void onChange(ChangeEvent event) {
				if(null != event.getValue() && null != typeItem.getValue() && null != endDateItem.getValue()){
					...
					params.put("subtractDays", subtractDays);
					params.put("subtractHours", subtractHours);
					DBDataSource.callOperation("EP_MyPersonnelProcessor", params, new DSCallback() {
						@Override
						public void execute(DSResponse dsResponse, Object data, DSRequest dsRequest) {
							if(dsResponse.getStatus() >= 0){
		...
		endDateItem.addBlurHandler(new BlurHandler() {
		...
		unitItem = new TextItem("unit",shouldNotBeNull + "亲人过世");
...
//		formRight.setItems(companyProvinceItem, typeItem, subTypeItem, numberOfBabyItem, targetProvinceItem, reasonItem, remarkItem);
		...
		viewButton = new IButton("查看请假规则", new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {
		...
		attachmentButton = new IButton("查看附件");
		attachmentButton.setBaseStyle("customButton");
		attachmentButton.setWidth(120);
		attachmentButton.setHeight(30);
		attachmentButton.setLeft(0);
		attachmentButton.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				if(null == attachmentItem.getValue()){
					...
					DownloadFileByUrl.download(attachmentItem.getValue().toString());
				}
			}
		});
		...
		viewButton.addClickHandler(new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {
				if(null != employeeIdItem.getValue() && null != startDateItem.getValue() && null != endDateItem.getValue()){
					PopupWindow window = new PopupWindow(KeyValueManager.getValue("employees", employeeIdItem.getValueAsString())+"请假前一天考勤明细");
					Map<String, Object> params = getValuesAsMap();
		...
		buttonLayout.addMember(attachmentButton);
		buttonLayout.addMember(viewButton);
		label = new Label("<font style='color:#FF7F27; font-weight:bold;'>♥ 温馨提示：病假、婚假、产假、产检假、陪护假请点击上方“上传附件”按钮，上传相关证明文件</font>");
		label.setHeight(40);
	...
	public void startEdit() {
		if (getRecord() != null) {
			...
			record.setAttribute("hours", record.getAttribute("hours"));
			formLeft.editRecord(record);
			formRight.editRecord(record);
			attachmentItem.setLinkTitle(record.getAttribute("attachmentName"));
			if(null != typeItem.getValue()){
				if(typeItem.getValue().toString().equals("9")){
					unitItem.show();
					reasonItem.hide();
					//丧假
					targetProvinceItem.show();
				}else if(typeItem.getValue().toString().equals("8")){
					//婚假
					certificationTimeItem.show();
				}else if(typeItem.getValue().toString().equals("21")){//调休
					unitItem.hide();
					reasonItem.hide();
					remarkItem.hide();
				}else {
			...
			if(null != daysItem.getValue()){
				subtractDays = daysItem.getValue();
			}
			if(null != hoursItem.getValue()){
				subtractHours = hoursItem.getValue();
			}
			Map<String, Object> params = new HashMap<>();
			params.put("optType", "getLeaveTypes");
			params.put("employeeId", employeeIdItem.getValue());
			...
			DBDataSource.callOperation("EP_MyPersonnelProcessor", params, new DSCallback() {
				...
				public void execute(DSResponse dsResponse, Object data, DSRequest dsRequest) {
					if(dsResponse.getStatus() >= 0){
						...
						typeItem.setValueMap(valueMap);
						canApply = true;
						maxDays = ClientUtil.checkAndGetDoubleValue(daysMap.get(typeItem.getValue()));
						params.put("optType", "getLeaveTypes");
						params.put("employeeId", employeeIdItem.getValue());
						params.put("areaId", companyProvinceItem.getValue());
						params.put("parentId", typeItem.getValue());
						DBDataSource.callOperation("EP_MyPersonnelProcessor", params, new DSCallback() {
							@Override
							public void execute(DSResponse dsResponse, Object data, DSRequest dsRequest) {
								if(dsResponse.getStatus() >= 0){
						...
						String value = KeyValueManager.getValue("leave_types", typeItem.getValue().toString());
						//切换背景图片
						resetBackgroundImage(value);
						if(value.equals(LEAVE_TYPE_7)){
							subTypeItem.show();
						...
						if(!value.equals(LEAVE_TYPE_7) && !value.equals(LEAVE_TYPE_8) && !value.equals(LEAVE_TYPE_9)){
							if(maxDays > 0){
								if(value.equals(LEAVE_TYPE_3)){
									daysItem.setTitle("请假天数");//20240602调整：所有员工不显示可调休时长
//									daysItem.setTitle("请假天数(最多可请假<font style = 'color:red'>"+maxDays+"</font>小时)");
								}else{
									daysItem.setTitle("请假天数(最多可请假<font style = 'color:red'>"+maxDays+"</font>天)");
								}
							}else{
								daysItem.setTitle("请假天数");
							}
	...
	public boolean checkData() {
		...
		if(!typeItem.getValue().toString().equals("9") && !typeItem.getValue().toString().equals("21")) {
	...
	public DynamicForm getFormLeft() {
		return formLeft;
	}

	public DynamicForm getFormRight() {
		return formRight;
	}
	...
	public IButton getViewButton() {
		return viewButton;
	}

	public IButton getAttachmentButton() {
