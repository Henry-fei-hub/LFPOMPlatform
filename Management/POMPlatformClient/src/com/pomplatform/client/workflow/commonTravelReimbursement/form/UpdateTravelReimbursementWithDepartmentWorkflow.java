...
public class UpdateTravelReimbursementWithDepartmentWorkflow extends AbstractProcessPanel
{
	...
	public List<AbstractWizadPage> getPages(){
		...
		final TravelReimbursementWithDepartmentUpdateForm main = new TravelReimbursementWithDepartmentUpdateForm();
		...
		detail.setTitle("普通差旅报销费用清单");
		detail.setPageMode(AbstractWizadPage.PAGE_MODE_UPDATE);
		res.add(detail);
		TravelReimbursementLinkDepartmentDetail link = null;
        if(null == processName){
        	link = new TravelReimbursementLinkDepartmentDetail(true, processName);
        }else{
        	//编辑进项税和成本的面板
        	link = new TravelReimbursementLinkDepartmentDetail(true, processName);
        }
        link.setProcessType(5);
        link.setMainInfoPage(main);
        link.setTitle("费用分拆");
        link.setPageMode(AbstractWizadPage.PAGE_MODE_UPDATE);
        res.add(link);
        detail.setDepartmentPanel(link);
