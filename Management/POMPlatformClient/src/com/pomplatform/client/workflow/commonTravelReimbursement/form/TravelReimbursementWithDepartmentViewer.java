...
public class TravelReimbursementWithDepartmentViewer extends AbstractDetailViewer
{


	private final DelicacyListGrid travelReimbursementDetailsGrid = new DelicacyListGrid();
	private final DelicacyListGrid travelReimbursementLinkDepartmentsGrid = new DelicacyListGrid();
	...
	public TravelReimbursementWithDepartmentViewer() {
	}
	...
	public void initComponents() {
		...
		travelReimbursementLinkDepartmentsGrid.setDataSource(DSTravelReimbursementLinkDepartment.getInstance());
		travelReimbursementLinkDepartmentsGrid.addDoubleClickHandler(new DoubleClickHandler(){
			@Override
			public void onDoubleClick(DoubleClickEvent event) {
				GenericViewWindow detail = new GenericViewWindow();
				detail.setTitle("");
				detail.setWidth(700); detail.setHeight(500); 
				TravelReimbursementLinkDepartmentViewer viewer = new TravelReimbursementLinkDepartmentViewer();
				viewer.setLayoutMode(AbstractDetailViewer.COMPONENT_LAYOUT_NORMAL);
				viewer.initComponents();
				viewer.viewSelectedData((ListGrid)event.getSource());
				detail.setContent(viewer);
	...
	public void load() {
		...
		DBDataSource.callOperation("NQ_TravelReimbursementWithDepartment", "find", new DSCallback() {
			@Override
	...
	public void viewDetailData(){
		...
		DBDataSource.callOperation("ST_TravelReimbursementLinkDepartment", "find", condition, new DSCallback(){
			...
			public void execute(DSResponse dsResponse, Object data, DSRequest dsRequest) {
				if(dsResponse.getStatus() >= 0){
					travelReimbursementLinkDepartmentsGrid.setData(dsResponse.getData());
				}
			}
		});
	...
	public List<ListGrid> getDetailListGrids() {
		...
		res.add(travelReimbursementLinkDepartmentsGrid);
