...
public class NewNewProjectTravelReimbursementWorkflow extends AbstractProcessPanel
{
private static final Logger logger = Logger.getLogger("");
	...
	public List<AbstractWizadPage> getPages(){
		...
		Record bdRecord=getData();
		List<AbstractWizadPage> res = new ArrayList<>();
		final NewProjectTravelReimbursementNewForm main = new NewProjectTravelReimbursementNewForm();
		main.setProcessId(getProcessId());
		res.add(main);
		final TravelReimbursementDetail detail = new TravelReimbursementDetail();
		if(bdRecord!=null) {
			detail.setBdRecord(bdRecord);
		}
		detail.setMainInfoPage(main);
		detail.setTitle("项目差旅报销费用清单");
		res.add(detail);
		final TravelReimbursementLinkProjectDetail link = new TravelReimbursementLinkProjectDetail(true, null);
		if(bdRecord!=null) {
		...
		if(!BaseHelpUtils.isNullOrEmpty(bdRecord)) {
			bdRecord.setAttribute("processType", 4);
