...
public class NewProjectTravelReimbursementViewer extends AbstractDetailViewer
{


	private final DelicacyListGrid travelReimbursementDetailsGrid = new DelicacyListGrid();
	private final DelicacyListGrid travelReimbursementLinkProjectsGrid = new DelicacyListGrid();
	...
	public NewProjectTravelReimbursementViewer() {
	}
	...
	public void initComponents() {
		...
		travelReimbursementLinkProjectsGrid.setDataSource(DSTravelReimbursementLinkProject.getInstance());
		travelReimbursementLinkProjectsGrid.addDoubleClickHandler(new DoubleClickHandler(){
			@Override
			public void onDoubleClick(DoubleClickEvent event) {
				GenericViewWindow detail = new GenericViewWindow();
				detail.setTitle("");
				detail.setWidth(700); detail.setHeight(500); 
				TravelReimbursementLinkProjectViewer viewer = new TravelReimbursementLinkProjectViewer();
				viewer.setLayoutMode(AbstractDetailViewer.COMPONENT_LAYOUT_NORMAL);
				viewer.initComponents();
				viewer.viewSelectedData((ListGrid)event.getSource());
				detail.setContent(viewer);
...
//		aFields[a].setValueMap(KeyValueManager.getValueMap("projects"));
	...
	public void load() {
		...
		DBDataSource.callOperation("NQ_NewProjectTravelReimbursement", "find", new DSCallback() {
			@Override
	...
	public void viewDetailData(){
		...
		DBDataSource.callOperation("ST_TravelReimbursementLinkProject", "find", condition, new DSCallback(){
			...
			public void execute(DSResponse dsResponse, Object data, DSRequest dsRequest) {
				if(dsResponse.getStatus() >= 0){
					travelReimbursementLinkProjectsGrid.setData(dsResponse.getData());
				}
			}
		});
		condition = new HashMap<>();
	...
	public List<ListGrid> getDetailListGrids() {
		...
		res.add(travelReimbursementLinkProjectsGrid);
