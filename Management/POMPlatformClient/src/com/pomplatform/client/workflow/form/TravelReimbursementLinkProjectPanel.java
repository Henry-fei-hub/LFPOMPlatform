...
public class TravelReimbursementLinkProjectPanel extends VLayout implements HasHandlers {

	private static final Logger logger = Logger.getLogger("");
	private final IntegerItem travelReimbursementLinkProjectIdItem;
	...
	public TravelReimbursementLinkProjectPanel(boolean update, LinkedHashMap<Integer, String> allProjectMap, boolean editInputTax) {
		...
		travelReimbursementLinkProjectIdItem = new IntegerItem("travelReimbursementLinkProjectId", "差旅报销关联项目ID");
		travelReimbursementLinkProjectIdItem.setHidden(true);
...
//		KeyValueManager.loadValueMap("projects",projectItem);
		...
		projectItem.addChangedHandler(new ChangedHandler() {
			...
			public void onChanged(ChangedEvent event) {
				String value = BaseHelpUtils.getString(projectItem.getDisplayValue());
		...
		projectItem.addChangedHandler(new ChangedHandler() {
			...
			public void onChanged(ChangedEvent event) {
				if(null != projectItem.getValue() && null != projectMap && !projectMap.isEmpty()){
					...
					if(length >= 1 && null != arr[0]){
						contractCodeItem.setValue(arr[0]);
					}
					if(length >= 3 && ClientUtil.checkAndGetIntValue(arr[length-1]) > 0){
						plateIdItem.setValue(arr[length-1]);
					}
				}
			}
		});
		...
		form.setNumCols(2);
		form.setItems(travelReimbursementLinkProjectIdItem, travelReimbursementIdItem, projectItem, contractCodeItem,
	...
	public void startEdit(final Record editRecord, int employeeId, BigDecimal amount, final Map selectedProject, boolean update) {
		...
		if (update) {
			if (null != editRecord.getAttribute("projectId")) {
				projectItem.setDefaultValue(ClientUtil.checkAndGetIntValue(editRecord.getAttribute("projectId")));
			}
