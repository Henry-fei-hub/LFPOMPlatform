...
public class ApplicationFilingNewForm extends AbstractWizadPage
{
	...
	private final MultiComboBoxItem nextAuditorItem;
	...
	public ApplicationFilingNewForm() {
		...
		nextAuditorItem = new MultiComboBoxItem("unit", shouldNotBeNull + "调档人员");
...
//		nextAuditorItem.setPickListFields(eNoField, eNameField, departmentIdField);
		...
		nextAuditorItem.addChangedHandler(new ChangedHandler() {
			...
			public void onChanged(ChangedEvent changedEvent) {
				HashMap<String, Integer> params = new HashMap<>();
				String employeeId =  BaseHelpUtils.getString(nextAuditorItem.getValue());
				String[] employeeIds = employeeId.replaceAll("\\[", "").replaceAll("]", "").replaceAll(" ", "").split(",");
				int length = employeeIds.length;

				int id = BaseHelpUtils.getIntValue(employeeIds[length-1]);
				SC.debugger();
				Object employeeName = nextAuditorItem.getDisplayValue();
				String name = BaseHelpUtils.getString(attachmentItem.getValue());
					attachmentItem.setValue(employeeName);
				...
				DBDataSource.callOperation("ST_Employee", "find", params, new DSCallback() {
					...
					public void execute(DSResponse dsResponse, Object o, DSRequest dsRequest) {
						...
						if (status >= 0) {
							...
							if (null != record) {
								SC.debugger();
								int statuss = record.getAttributeAsInt("status");
								if(statuss==1){
									SC.say("选择的人员已经离职");
									return;
								}
								String departmentId = record.getAttributeAsString("departmentId");
								String companyRecordId = record.getAttributeAsString("ownedCompany");
								String employeeName = record.getAttributeAsString("employeeName");

								HashMap<String, String> param = new HashMap<>();
								HashMap<String, String> paramss = new HashMap<>();
								paramss.put("companyRecordId",companyRecordId);
								DBDataSource.callOperation("ST_CompanyRecord", "find", paramss, new DSCallback() {
									public void execute(DSResponse dsResponse, Object o, DSRequest dsRequest) {
										int status = dsResponse.getStatus();
										if (status >= 0) {
											Record record = dsResponse.getData()[0];
											if (null != record) {
												String companyName = record.getAttributeAsString("companyName");
												salary1Item.setValue(companyName);
											}
										}
									}
								});
								...
								DBDataSource.callOperation("ST_Department", "find", param, new DSCallback() {
									public void execute(DSResponse dsResponse, Object o, DSRequest dsRequest) {
										...
										if (status >= 0) {
											Record record = dsResponse.getData()[0];
											if (null != record) {
												String departmentName = record.getAttributeAsString("departmentName");

												salary2Item.setValue(departmentName);
											}
										}
									}
								});
								SC.say(employeeName+"所属部门："+salary2Item.getValue());
							}
						}
					}
				});
		...
		salary2Item = new TextItem("salary2", "调档人归属部门");
		...
		monthPayItem = new TextItem("monthPay", "调档人姓名");
		monthPayItem.hide();
		...
		Label label = new Label("<font style='color:#FF7F27; font-weight:bold;'>♥ 温馨提示：可查看调档人员的员工编号、姓名、入职时间、所属部门、出生日期、年龄、参加工作时间、毕业院校、所学专业、身份证附件、职称证附件、学历证附件</font>");
		label.setHeight(200);
		label.setLeft(200);
