...
import com.pomplatform.client.workflow.datasource.DSBonusPaymentDetail;

public class BonusPaymentRecordWithBViewer extends AbstractDetailViewer
{


	private final DelicacyListGrid bonusPaymentDetailsGrid = new DelicacyListGrid();
	private SearchForm __parentSearchForm;


	public BonusPaymentRecordWithBViewer() {
	}

	@Override
	public void initComponents() {
		super.initComponents();
		bonusPaymentDetailsGrid.setDataSource(DSBonusPaymentDetail.getInstance());
		bonusPaymentDetailsGrid.addDoubleClickHandler(new DoubleClickHandler(){
			@Override
			public void onDoubleClick(DoubleClickEvent event) {
				GenericViewWindow detail = new GenericViewWindow();
				detail.setTitle("");
				detail.setWidth(700); detail.setHeight(500); 
				BonusPaymentDetailViewer viewer = new BonusPaymentDetailViewer();
	...
	public void viewDetailData(){
		...
		condition.put("bonusPaymentRecordId", selected.getAttributeAsString("bonusPaymentRecordId"));
		DBDataSource.callOperation("ST_BonusPaymentDetail", "find", condition, new DSCallback(){
			@Override
			public void execute(DSResponse dsResponse, Object data, DSRequest dsRequest) {
				if(dsResponse.getStatus() >= 0){
					bonusPaymentDetailsGrid.setData(dsResponse.getData());
				}
			}
		});
	}
	...
	public List<ListGrid> getDetailListGrids() {
		...
		res.add(bonusPaymentDetailsGrid);
		return res;
	}
