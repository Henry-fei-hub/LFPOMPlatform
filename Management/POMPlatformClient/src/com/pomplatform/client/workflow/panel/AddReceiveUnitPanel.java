...
public class AddReceiveUnitPanel extends VLayout implements HasHandlers {
	...
	public AddReceiveUnitPanel() {
		...
		receiveUnitItem = new TextItem("receiveUnit", "单位名称");
		receiveUnitItem.setWidth("*");
		receiveUnitItem.setRequired(true);

		receiveUnitAddressItem = new TextItem("receiveUnitAddress", "收款单位地址");
...
//		cityItem.setChangeOnKeypress(false);
		...
		form.setItems(receiveUnitItem, receiveUnitAddressItem, bankIdItem, bankNameItem, bankAccountItem, bankAddressItem, provinceItem,
				...
				passportItem, voucherTypeItem, supplierTypeItem);
	...
	public boolean DataValidate() {
		...
		if(null != bankIdItem.getValue() && BaseHelpUtils.getIntValue(bankIdItem.getValue()) == 0) {
			SC.say("开户行只能从下拉列表中选择数据");
			return false;
		}
		if(null != provinceItem.getValue() && BaseHelpUtils.getIntValue(provinceItem.getValue()) == 0) {
			SC.say("开户行所在省只能从下拉列表中选择数据");
			return false;
		}
		if(null != cityItem.getValue() && BaseHelpUtils.getIntValue(cityItem.getValue()) == 0) {
			SC.say("开户行所在市只能从下拉列表中选择数据");
			return false;
		}
		if(null != countryIdItem.getValue() && BaseHelpUtils.getIntValue(countryIdItem.getValue()) == 0) {
			SC.say("收款人常驻国家（地区）只能从下拉列表中选择数据");
			return false;
		}
