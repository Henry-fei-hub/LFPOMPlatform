...
public class DSSealApplication extends DBDataSource
{


	public static DSSealApplication instance = null;

	public static DSSealApplication getInstance() {
		if(instance == null) {
			instance = new DSSealApplication("DSSealApplication");
		}
		return instance;
	}
	...
	public DSSealApplication(String dataSourceID) {

		super();
		setID(dataSourceID);
		setActionMode(ACTION_MODE_QUERY);
		setActionName("SealApplication");
		...
		salary1Field = new DataSourceTextField("salary1", "请选择印章种类");
		...
		unitField = new DataSourceTextField("unit", "盖章内容扫描件");
		unitField.setLength(255);
		unitField.setRequired(true);
		unitField.setHidden(false);
		unitField.setType(FieldType.LINK);

		contactPersonField = new DataSourceTextField("contactPerson", "选中的印章种类");
		...
		companyProvinceField = new DataSourceIntegerField("companyProvince", "所属公司");
		companyProvinceField.setLength(11);
		companyProvinceField.setRequired(false);
		companyProvinceField.setHidden(false);
		companyProvinceField.setValueMap(KeyValueManager.getValueMap("system_dictionary_26"));
		...
		projectIdField = new DataSourceIntegerField("projectId", "盖章项目");
		...
		cardField = new DataSourceTextField("card", "日常盖章内容");
		...
		setFields(personnelBusinessIdField, employeeIdField, processTypeField, employeeNoField, departmentIdField, companyIdField, typeField, startDateField, startDateDetailField, endDateField, endDateDetailField, daysField, reasonField, remarkField, addressField, nextAuditorField,planField,salary2Field,cardField, transportationField, feeField, unitField, contactPersonField, createTimeField, deleteFlagField, subTypeField, hoursField, companyProvinceField, targetProvinceField, numberOfBabyField, attachmentField, isCompletedField, subTypeStrField, projectTypeField, projectIdField, projectNameField, projectCodeField, parentIdField, isRemoteCityField, provideAccommodationField, livingExpensesField, housingFinanceField, drafterField, dutyIdField, equivalentNumberField, linkIdField, severanceField, salary1Field, originalPayField, monthPayField, basicProportionStrField, basicProportionField, monthBasicPayField, monthPerformancePayField, foremanPayField, otherSubsidyField, totalPayField, annualPerformanceField, costAttributionField, certificationTimeField, projectIdsField, paymentSequenceIdField, budgetManagementIdField, budgetAttachmentIdField, stageIdField, attachmentIdField, attachmentNameField);
	}
