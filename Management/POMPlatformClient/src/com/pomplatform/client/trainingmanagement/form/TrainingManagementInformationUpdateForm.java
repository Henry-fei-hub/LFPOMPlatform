...
public class TrainingManagementInformationUpdateForm extends AbstractWizadPage
{
...
//	private final SelectItem employeeIdItem;
	...
	public TrainingManagementInformationUpdateForm() {
		...
		trainingManagementIdItem.setRequired(true);
		trainingManagementIdItem.hide();
		IsIntegerValidator trainingManagementIdValidator = new IsIntegerValidator();
		trainingManagementIdItem.setValidators(trainingManagementIdValidator);
		__formItems.add(trainingManagementIdItem);
		subjectItem = new TextItem("subject", shouldNotBeNull+"主题");
		__formItems.add(subjectItem);
		subjectItem.setWidth("*");
		courseScoreItem = new TextItem("courseScore", shouldNotBeNull+"课程分数");
...
//		addMember(__form);
		...
		upButton.addClickHandler( new ClickHandler(){
			...
			public void onClick(ClickEvent event) {
				new CommonUploadWindow("上传附件", false, 500, 3, 1, new UploadCallBack() {
					@Override
...
//			__form.editRecord(record2);
			 trainingManagementIdItem.setDefaultValue(record2.getAttribute("trainingManagementId"));
			 subjectItem.setDefaultValue(record2.getAttribute("subject"));
			 compereTypeItem.setDefaultValue(record2.getAttribute("compereType"));
			 compereItem.setDefaultValue(record2.getAttribute("compere"));
			 startTimeItem.setDefaultValue(record2.getAttributeAsDate("startTime"));
		     endTimeItem.setDefaultValue(record2.getAttributeAsDate("endTime"));
			 departmentIdItem.setDefaultValue(record2.getAttribute("departmentId"));
			 courseScoreItem.setDefaultValue(record2.getAttribute("courseScore"));
			 signInDelayItem.setDefaultValue(record2.getAttribute("signInDelay"));
			 
			 String attribute = record2.getAttribute("signOutDelay");
			 signOutDelayItem.setDefaultValue(record2.getAttribute("signOutDelay"));
			 employeeIdItem.setDefaultValue(record2.getAttribute("employeeId"));
			 siteTextItem.setDefaultValue(record2.getAttribute("site"));
			 plateIdItem.setDefaultValue(record2.getAttribute("plateId"));
			 isSiteItem.setDefaultValue(record2.getAttribute("isSite"));
			 trainingTypeItem.setDefaultValue(record2.getAttribute("trainingType"));
			 trainingFeeItem.setDefaultValue(record2.getAttribute("trainingFee"));
			 fileIdItem.setDefaultValue(record2.getAttribute("fileId"));
			 fileUrlItem.setDefaultValue(record2.getAttribute("fileUrl"));
			 isSendItem.setDefaultValue(record2.getAttribute("isSend"));
			if(!BaseHelpUtils.isNullOrEmpty(record2.getAttribute("plateids"))) {
				String[] split = record2.getAttribute("plateids").split(",");
				plateidsItem.setDefaultValues(split);
			}
		}
	}
	...
	public boolean checkData() {
		...
		if(null==courseScoreItem.getValue()) {
			SC.warn("提示","课程分数不能为空");
			return false;
		}
