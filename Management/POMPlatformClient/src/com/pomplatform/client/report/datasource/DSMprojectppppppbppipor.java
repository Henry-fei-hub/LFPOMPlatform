...
public class DSMprojectppppppbppipor extends DBDataSource
{


	public static DSMprojectppppppbppipor instance = null;

	public static DSMprojectppppppbppipor getInstance() {
		if(instance == null) {
			instance = new DSMprojectppppppbppipor("DSMprojectppppppbppipor");
		}
		return instance;
	}
	...
	private final DataSourceFloatField regularPriceField;
	private final DataSourceFloatField progressPercentageField;
	private final DataSourceFloatField priceSpreadField;
	private final DataSourceFloatField money1Field;
	private final DataSourceFloatField money2Field;
	private final DataSourceFloatField advanceIntegral1Field;
	private final DataSourceFloatField advanceIntegral2Field;

	public DSMprojectppppppbppipor(String dataSourceID) {

		super();
		setID(dataSourceID);
		setActionMode(ACTION_MODE_QUERY);
		setActionName("Mprojectppppppbppipor");
		...
		pricingTypeField.setValueMap(KeyValueManager.getValueMap("system_dictionary_207"));
	
		regularPriceField = new DataSourceFloatField("regularPrice", "常规定价");
		regularPriceField.setLength(18);
		regularPriceField.setDecimalPad(2);
		regularPriceField.setFormat("#,###,###,###,###,##0.00");
		regularPriceField.setRequired(true);
		regularPriceField.setHidden(false);


		priceSpreadField = new DataSourceFloatField("priceSpread", "差额");
		priceSpreadField.setLength(18);
		priceSpreadField.setDecimalPad(2);
		priceSpreadField.setFormat("#,###,###,###,###,##0.00");
		priceSpreadField.setRequired(true);
		priceSpreadField.setHidden(false);


		money1Field = new DataSourceFloatField("money1", "运营补贴汇总");
		money1Field.setLength(18);
		money1Field.setDecimalPad(2);
		money1Field.setFormat("#,###,###,###,###,##0.00");
		money1Field.setRequired(true);
		money1Field.setHidden(false);


		money2Field = new DataSourceFloatField("money2", "公司补贴汇总");
		money2Field.setLength(18);
		money2Field.setDecimalPad(2);
		money2Field.setFormat("#,###,###,###,###,##0.00");
		money2Field.setRequired(true);
		money2Field.setHidden(false);
		
		progressPercentageField = new DataSourceFloatField("progressPercentage", "进度（%）");
		progressPercentageField.setLength(18);
		...
		advanceIntegral1Field = new DataSourceFloatField("advanceIntegral1", "部门");
		advanceIntegral1Field.setLength(18);
		advanceIntegral1Field.setDecimalPad(2);
		advanceIntegral1Field.setFormat("#,###,###,###,###,##0.00");
		advanceIntegral1Field.setRequired(true);
		advanceIntegral1Field.setHidden(true);


		advanceIntegral2Field = new DataSourceFloatField("advanceIntegral2", "项目补贴(公司)");
		advanceIntegral2Field.setLength(18);
		advanceIntegral2Field.setDecimalPad(2);
		advanceIntegral2Field.setFormat("#,###,###,###,###,##0.00");
		advanceIntegral2Field.setRequired(true);
		advanceIntegral2Field.setHidden(false);

		setFields(projectIdField, contractCodeField, projectCodeField, projectNameField, plateIdField, projectTypeField, designTypeField, departmentIdField, designPhaseField, projectLevelField, statusField, projectManageIdField, workHoursField, planStartDateField, planEndDateField, startDateField, endDateField, commonAreaField, logisticsAreaField, percentageField, oldProjectDiscountField, budgetField, reserveIntegralField, totalAmountField, projectIntegralField, projectSizeField, projectDescriptionField, auditReasonField, departmentManagerIdField, bimAreaField, cadAreaField, humanAmountField, appendTotalIntegralField, appendIntegralField, remarkField, projectFinishPercentField, parentIdField, totalFinishPercentField, projectGradeTypeField, plateManagerIdField, lastPercentField, complaintPercentageField, complaintIntegralField, revenuePlateIdField,  createTimeField, projectFlagField, businessTypeField, contractIdField, subContractIdField, isEcmcProjectField, moneySumField, projectSetupFlagField, isLockField, isAutoField, timeCycleField, reserveSheetIdField, businessLineField, pricingTypeField,sheetAmountField, regularPriceField, totalIntegralField, priceSpreadField,progressPercentageField, projectProgressField, money1Field, money2Field,advanceIntegral1Field,advanceIntegral2Field);
	}
