...
public class SwebsitenewntforUpdateForm extends AbstractWizadPage {
	...
	public SwebsitenewntforUpdateForm() {
		DSSwebsitenewntfor ds = DSSwebsitenewntfor.getInstance();
		__form.setWidth100();
		__form.setHeight100();
		websiteNewIdItem = new TextItem("websiteNewId", "主键");
		websiteNewIdItem.hide();
		websiteNewIdItem.setDisabled(true);
		websiteNewIdItem.setRequired(true);
		IsIntegerValidator websiteNewIdValidator = new IsIntegerValidator();
		websiteNewIdItem.setValidators(websiteNewIdValidator);
		__formItems.add(websiteNewIdItem);
		titleItem = new TextItem("title", "主题");
		titleItem.setWidth("*");
		__formItems.add(titleItem);
		fileUrlItem = new TextItem("fileUrl", "文件路径");
		fileUrlItem.setWidth("*");
		fileUrlItem.setDisabled(true);
		fileUrlItem.setWidth("*");
	...
	public void startEdit() {
		if (getRecord() != null){
			...
			//获取附件值，如果有值，则重定义标题显示
			String attachmentFile = BaseHelpUtils.getString(getRecord().getAttribute("fileUrl"));
