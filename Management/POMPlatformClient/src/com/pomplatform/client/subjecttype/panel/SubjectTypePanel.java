...
public class SubjectTypePanel extends AbstractSearchTree
{
	...
	public static class Factory implements PanelFactory {
		...
		public Canvas create() {
			SubjectTypePanel cm = new SubjectTypePanel();
			id = cm.getID();
			return cm;
		}
...
//		fields[idx].setWidth(700);
		...
		resultGrid.addDropCompleteHandler(new DropCompleteHandler() {
			...
			public void onDropComplete(DropCompleteEvent event) {
				...
				for (Record r : rs) {
					DBDataSource.callOperation("ST_SubjectType", "update", r.toMap(), new DSCallback() {
						@Override
		...
		resultGrid.addEditCompleteHandler(new EditCompleteHandler() {
			...
			public void onEditComplete(EditCompleteEvent event) {
				...
				DBDataSource.callOperation("ST_SubjectType", "saveOrUpdate", params, new DSCallback() {
					@Override
					public void execute(DSResponse dsResponse, Object data, DSRequest dsRequest) {
						if (dsResponse.getStatus() < 0) {
							ClientUtil.displayErrorMessage(dsResponse);
		...
		removeButton.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				if(null != selected.getAttribute("subjectTypeId")){//防止删除所有ID为0的父级科目
					
					DBDataSource.callOperation("ST_SubjectType", "delete", selected.toMap(), new DSCallback() {
						@Override
						public void execute(DSResponse dsResponse, Object data, DSRequest dsRequest) {
							if (dsResponse.getStatus() < 0) {
								ClientUtil.displayErrorMessage(dsResponse);
	...
	public DataSource getDataSource() {
		return DSSubjectType.getInstance();
	}
