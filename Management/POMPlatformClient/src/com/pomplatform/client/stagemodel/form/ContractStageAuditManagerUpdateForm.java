...
public class ContractStageAuditManagerUpdateForm extends AbstractWizadPage
{
	...
	private final TextAreaItem managerCommentItem;
	private final TextItem businessTypeItem;
	private final TextItem parentStageNameItem;

	public ContractStageAuditManagerUpdateForm() {
		...
		managerCommentItem = new TextAreaItem("managerComment", "审批意见");
		managerCommentItem.setWidth("*");
		managerCommentItem.setHeight(150);
		managerCommentItem.hide();
		__formItems.add(managerCommentItem);
		...
		managerCommentItem.setColSpan(4);
		managerCommentItem.setRowSpan(4);
				...
				public void execute(DSResponse dsResponse, Object data, DSRequest dsRequest) {
					if(dsResponse.getStatus() >= 0){
						...
						if(stageName.indexOf("评审") >= 0){
							remarkItem.setTitle("评审结论</font>");
							managerCommentItem.show();
	...
	public boolean checkData() {
		if(null != getRecord()){
			...
			if(stageName.indexOf("评审") >= 0 && BaseHelpUtils.isNullOrEmpty(managerCommentItem.getValue())){
				SC.say(managerCommentItem.getTitle()+"不能为空");
				return false;
			}
		}
		return true;
