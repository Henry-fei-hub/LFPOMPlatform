...
public class OnDevelopProjectNewForm extends AbstractWizadPage
{
	...
	public OnDevelopProjectNewForm() {
...
//		plateIdItem.setDisabled(true);
		...
		projectFlagItem = new TextItem("projectFlag", "项目类型标示：1:设计项目;2:研发项目;3:管理项目");
		projectFlagItem.hide();
		__formItems.add(projectFlagItem);
		projectSetupFlagItem = new TextItem("projectSetupFlag", "项目立项标示:1:公司立项;2:部门立项");
		projectSetupFlagItem.hide();
		__formItems.add(projectSetupFlagItem);
	...
	public void startEdit() {
		if(getRecord() != null) {
			...
			int plateId = BaseHelpUtils.getIntValue(getRecord().getAttribute("plateId"));
			RoleDefinition.setEmployeeByPlateIdAndRoleId(plateManagerIdItem,plateId, 2);
			RoleDefinition.setPlateOfDepartmentId(departmentIdItem,plateId);
		}
