...
import com.pomplatform.client.employeereportrecorddetail.datasource.CDEmployeeReportRecordDetail;
...
public class EmployeeReportRecordDetailSearchForm extends SearchForm {
	...
	public EmployeeReportRecordDetailSearchForm() {
		final String plateIds = PomPlatformClientUtil.getRolePlateId();
		boolean onlyCheckMyself = false;
		if (!ClientUtil.checkIsHavePermission(ERPPermissionStatic.PERSONAL_ACCOUNT_MANAGEMENT_CHECK_ALL_DATA)
				...
				&& ClientUtil.checkIsHavePermission(ERPPermissionStatic.PERSONAL_ACCOUNT_MANAGEMENT_CHECK_MYSELF_DATA)) {
			// 该用户只有查看自己数据的权限
			onlyCheckMyself = true;
		}
		setWidth100();
		setHeight100();
		setDataSource(CDEmployeeReportRecordDetail.getInstance());
