...
public class ApplyTemplatePanel extends AbstractWizadPage implements HasHandlers{
	...
	private BigDecimal signingMoneySum;
	
	private final ComboBoxItem templateItem;
	
	public ApplyTemplatePanel(){
//		setBorder(BORDER_STYLE);
		...
		templateItem = new ComboBoxItem("选择模板");
		templateItem.setWidth("*");
		templateItem.setChangeOnKeypress(false);
		templateItem.setRequired(true);
		templateItem.setTextMatchStyle(TextMatchStyle.SUBSTRING);
		__formItems.add(templateItem);
		...
		IButton saveBtn = new IButton("应用");
		...
		saveBtn.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				if(checkData()){
					SC.ask("提示", "应用模板会清除原来的收款阶段数据，确定应用?", new BooleanCallback() {
						...
						public void execute(Boolean value) {
							if(value){
								...
								//获取合同数据
								params.put("templateId", templateItem.getValue());
								params.put("contractId", contractId);
								params.put("signingMoneySum", signingMoneySum);
								params.put("opt_type", "applyTemplate");
	...
	private void setSelectValue(){
		...
		params.put("opt_type", "getParentTemplate");
		KeyValueManager.setValueMapFromQuery("EP_CustomReceivableTemplateProcess", MapUtils.toJSON(params), templateItem);
	}
	...
	public BigDecimal getSigningMoneySum() {
		return signingMoneySum;
	}

	public void setSigningMoneySum(BigDecimal signingMoneySum) {
		this.signingMoneySum = signingMoneySum;
	}
