...
public class SupplierWorkflowViewer extends AbstractDetailViewer
{


	private final DelicacyListGrid supplierGrid = new DelicacyListGrid();
	private SearchForm __parentSearchForm;


	public SupplierWorkflowViewer() {
	}

	@Override
	public void initComponents() {
		super.initComponents();
		supplierGrid.setDataSource(DSSupplier.getInstance());
		supplierGrid.addDoubleClickHandler(new DoubleClickHandler(){
			...
			public void onDoubleClick(DoubleClickEvent event) {
				...
				SupplierViewer viewer = new SupplierViewer();
	...
	public void viewDetailData(){
		...
		condition.put("receiveUnitManageId", selected.getAttributeAsString("receiveUnitManageId"));
		DBDataSource.callOperation("ST_Supplier", "find", condition, new DSCallback(){
			@Override
			public void execute(DSResponse dsResponse, Object data, DSRequest dsRequest) {
				if(dsResponse.getStatus() >= 0){
					supplierGrid.setData(dsResponse.getData());
				}
			}
		});
	}
