...
import pomplatform.ht.bean.ConditionInitiationProject;

public class QueryInitiationProject extends AbstractQuery<BaseMainProject, ConditionInitiationProject>
{

	private static final Logger __logger = Logger.getLogger(QueryInitiationProject.class);

	public QueryInitiationProject() throws java.sql.SQLException
	{
	...
	@Override
	public BaseCollection<BaseMainProject> executeQuery( KeyValuePair[] replacements, ConditionInitiationProject condition ) throws java.sql.SQLException {
		...
		return executeQuery(replacements,
				...
				condition.getManager(),
				condition.getChargePerson()
		);
	}

	public BaseCollection<BaseMainProject> runQuery( KeyValuePair[] replacements, ConditionInitiationProject condition ) throws java.sql.SQLException {
		if(condition.getCurrentPage() > 0) {
			setCurrentPage(condition.getCurrentPage());
			setPageLines(condition.getPageLines());
		}
	...
	public BaseCollection<BaseMainProject> execute( KeyValuePair[] replacements, ConditionInitiationProject condition ) throws java.sql.SQLException {
	...
	public BaseMainProject generateBase(Object[] __data) throws java.sql.SQLException {
		...
		if((val = __data[count++]) != null) __base.setOutsource(GenericBase.__getString(val));
		if((val = __data[count++]) != null) __base.setSecretary(GenericBase.__getString(val));
		if((val = __data[count++]) != null) __base.setManager(GenericBase.__getString(val));
	...
	private final static String __SQLText = "SELECT mp.main_project_id, mp.project_code, mp.project_name, mp.project_english_name, mp.project_status, mp.design_status, mp.sign_company, mp.project_origination, mp.project_origination_remark, mp.design_cooperation_name, mp.design_cooperation_english_name, mp.project_sales_team, mp.design_team, mp.is_high_rise_building, mp.info_register_time, mp.project_approval_time, mp.has_contract, mp.project_base_on, mp.info_code, mp.project_model, mp.design_qualification, mp.business_type, mp.fire_resistance_rating, mp.estimate_the_design_areas, mp.project_total_investment, mp.actual_design_areas, mp.total_building_areas, mp.building_height, mp.building_floors, mp.decorate_money_control, mp.project_level, mp.province, mp.city, mp.detail_address, mp.project_end_time, mp.project_type, mp.client_id, mp.customer_name, mp.customer_address, mp.bid_project, mp.construction_unit, mp.architectural_design_units, mp.rate_of_bid_winning, mp.follow_up_date, mp.equityedge_remark, mp.order_sign_up_time, mp.design_service_book, mp.service_suggest_book, mp.total_money, mp.covered_area, mp.design_area, mp.has_formal_contracts, mp.has_out_quotations, mp.has_split_price, has_order_release, has_deposit, has_rate_of_payment, mp.parent_id, has_formal_contract_time, has_out_quotation_time, has_order_release_time, has_split_price_time, bid_type, ( SELECT CASE COUNT ( 1 ) WHEN 0 THEN '无' ELSE'有' END AS outsource FROM main_project_specialty mps WHERE mps.main_project_id = mp.main_project_id AND mps.is_outsourcing = TRUE ) AS outsource, mpe1.employee_name AS secretary, mpe3.employee_name AS manager, mpe4.employee_name AS leader, mpe5.employee_name AS charge_person FROM main_projects mp LEFT JOIN ( SELECT array_to_string( ARRAY_AGG ( employee_name ), '，' ) employee_name, main_project_id, relation FROM main_project_employees mpe LEFT JOIN employees e ON mpe.employee_id = e.employee_id GROUP BY main_project_id, relation ) mpe1 ON mp.main_project_id = mpe1.main_project_id AND mpe1.relation = 1 LEFT JOIN ( SELECT array_to_string( ARRAY_AGG ( employee_name ), '，' ) employee_name, main_project_id, relation FROM main_project_employees mpe LEFT JOIN employees e ON mpe.employee_id = e.employee_id GROUP BY main_project_id, relation ) mpe3 ON mp.main_project_id = mpe3.main_project_id AND mpe3.relation = 3 LEFT JOIN ( SELECT array_to_string( ARRAY_AGG ( employee_name ), '，' ) employee_name, main_project_id, relation FROM main_project_employees mpe LEFT JOIN employees e ON mpe.employee_id = e.employee_id GROUP BY main_project_id, relation ) mpe4 ON mp.main_project_id = mpe4.main_project_id AND mpe4.relation = 4 LEFT JOIN ( SELECT array_to_string( ARRAY_AGG ( employee_name ), '，' ) employee_name, main_project_id, relation FROM main_project_employees mpe LEFT JOIN employees e ON mpe.employee_id = e.employee_id GROUP BY main_project_id, relation ) mpe5 ON mp.main_project_id = mpe5.main_project_id AND mpe5.relation = 5 LEFT JOIN plate_records AS pr ON mp.design_team = pr.plate_id WHERE mp.project_code != '' and mp.project_code like ? and mp.project_name like ? and mp.has_contract = ? and mp.project_base_on = ? and mp.info_code like ? and pr.plate_name like ? and mpe4.employee_name like ? and mpe1.employee_name like ? and mpe3.employee_name like ? and mpe5.employee_name like ? and mp.process_status = 2 ORDER BY mp.info_code, mp.info_register_time DESC" ;
	private final static String __originalSQL = "SELECT mp.main_project_id, mp.project_code, mp.project_name, mp.project_english_name, mp.project_status, mp.design_status, mp.sign_company, mp.project_origination, mp.project_origination_remark, mp.design_cooperation_name, mp.design_cooperation_english_name, mp.project_sales_team, mp.design_team, mp.is_high_rise_building, mp.info_register_time, mp.project_approval_time, mp.has_contract, mp.project_base_on, mp.info_code, mp.project_model, mp.design_qualification, mp.business_type, mp.fire_resistance_rating, mp.estimate_the_design_areas, mp.project_total_investment, mp.actual_design_areas, mp.total_building_areas, mp.building_height, mp.building_floors, mp.decorate_money_control, mp.project_level, mp.province, mp.city, mp.detail_address, mp.project_end_time, mp.project_type, mp.client_id, mp.customer_name, mp.customer_address, mp.bid_project, mp.construction_unit, mp.architectural_design_units, mp.rate_of_bid_winning, mp.follow_up_date, mp.equityedge_remark, mp.order_sign_up_time, mp.design_service_book, mp.service_suggest_book, mp.total_money, mp.covered_area, mp.design_area, mp.has_formal_contracts, mp.has_out_quotations, mp.has_split_price, has_order_release, has_deposit, has_rate_of_payment, mp.parent_id, has_formal_contract_time, has_out_quotation_time, has_order_release_time, has_split_price_time, bid_type, ( SELECT CASE COUNT ( 1 ) WHEN 0 THEN '无' ELSE'有' END AS outsource FROM main_project_specialty mps WHERE mps.main_project_id = mp.main_project_id AND mps.is_outsourcing = TRUE ) AS outsource, mpe1.employee_name AS secretary, mpe3.employee_name AS manager, mpe4.employee_name AS leader, mpe5.employee_name AS charge_person FROM main_projects mp LEFT JOIN ( SELECT array_to_string( ARRAY_AGG ( employee_name ), '，' ) employee_name, main_project_id, relation FROM main_project_employees mpe LEFT JOIN employees e ON mpe.employee_id = e.employee_id GROUP BY main_project_id, relation ) mpe1 ON mp.main_project_id = mpe1.main_project_id AND mpe1.relation = 1 LEFT JOIN ( SELECT array_to_string( ARRAY_AGG ( employee_name ), '，' ) employee_name, main_project_id, relation FROM main_project_employees mpe LEFT JOIN employees e ON mpe.employee_id = e.employee_id GROUP BY main_project_id, relation ) mpe3 ON mp.main_project_id = mpe3.main_project_id AND mpe3.relation = 3 LEFT JOIN ( SELECT array_to_string( ARRAY_AGG ( employee_name ), '，' ) employee_name, main_project_id, relation FROM main_project_employees mpe LEFT JOIN employees e ON mpe.employee_id = e.employee_id GROUP BY main_project_id, relation ) mpe4 ON mp.main_project_id = mpe4.main_project_id AND mpe4.relation = 4 LEFT JOIN ( SELECT array_to_string( ARRAY_AGG ( employee_name ), '，' ) employee_name, main_project_id, relation FROM main_project_employees mpe LEFT JOIN employees e ON mpe.employee_id = e.employee_id GROUP BY main_project_id, relation ) mpe5 ON mp.main_project_id = mpe5.main_project_id AND mpe5.relation = 5 LEFT JOIN plate_records AS pr ON mp.design_team = pr.plate_id WHERE mp.project_code != '' AND mp.project_code LIKE :project_code AND mp.project_name LIKE :project_name AND mp.has_contract = :has_contract AND mp.project_base_on = :project_base_on AND mp.info_code LIKE :info_code AND pr.plate_name like :design_team AND mpe4.employee_name LIKE :leader AND mpe1.employee_name LIKE :secretary AND mpe3.employee_name LIKE :manager AND mpe5.employee_name LIKE :charge_person AND mp.process_status = 2 ORDER BY mp.info_code, mp.info_register_time DESC";
	private final static String[] __queryConditions = {
			"[{\"fullCondition\": \"mp.project_code != '' AND mp.project_code LIKE :project_code AND mp.project_name LIKE :project_name AND mp.has_contract = :has_contract AND mp.project_base_on = :project_base_on AND mp.info_code LIKE :info_code AND pr.plate_name like :design_team AND mpe4.employee_name LIKE :leader AND mpe1.employee_name LIKE :secretary AND mpe3.employee_name LIKE :manager AND mpe5.employee_name LIKE :charge_person AND mp.process_status = 2\",\"startIndex\": 2923,\"stopIndex\":3347,\"variableNum\": 0,\"variableCount\": 0,\"isor\":false,\"hasVariable\":false,\"hasBracket\":false,\"isVariableFirst\":false,\"topLevel\":true,\"children\": [{\"fullCondition\": \"mp.project_code != ''\",\"nextToken\":\"and\",\"startIndex\": 2923,\"stopIndex\":2943,\"variableNum\": 0,\"variableCount\": 0,\"isor\":false,\"hasVariable\":false,\"hasBracket\":false,\"isVariableFirst\":false,\"topLevel\":false,\"children\": []},{\"fullCondition\": \"mp.project_code LIKE :project_code\",\"fullFieldName\":\"mp.project_code\",\"operationName\":\"like\",\"variableName\":\"projectCode\",\"nextToken\":\"and\",\"startIndex\": 2949,\"stopIndex\":2982,\"variableNum\": 0,\"variableCount\": 1,\"isor\":false,\"hasVariable\":true,\"hasBracket\":false,\"isVariableFirst\":false,\"topLevel\":false,\"children\": []},{\"fullCondition\": \"mp.project_name LIKE :project_name\",\"fullFieldName\":\"mp.project_name\",\"operationName\":\"like\",\"variableName\":\"projectName\",\"nextToken\":\"and\",\"startIndex\": 2988,\"stopIndex\":3021,\"variableNum\": 1,\"variableCount\": 1,\"isor\":false,\"hasVariable\":true,\"hasBracket\":false,\"isVariableFirst\":false,\"topLevel\":false,\"children\": []},{\"fullCondition\": \"mp.has_contract = :has_contract\",\"fullFieldName\":\"mp.has_contract\",\"operationName\":\"=\",\"variableName\":\"hasContract\",\"nextToken\":\"and\",\"startIndex\": 3027,\"stopIndex\":3057,\"variableNum\": 2,\"variableCount\": 1,\"isor\":false,\"hasVariable\":true,\"hasBracket\":false,\"isVariableFirst\":false,\"topLevel\":false,\"children\": []},{\"fullCondition\": \"mp.project_base_on = :project_base_on\",\"fullFieldName\":\"mp.project_base_on\",\"operationName\":\"=\",\"variableName\":\"projectBaseOn\",\"nextToken\":\"and\",\"startIndex\": 3063,\"stopIndex\":3099,\"variableNum\": 3,\"variableCount\": 1,\"isor\":false,\"hasVariable\":true,\"hasBracket\":false,\"isVariableFirst\":false,\"topLevel\":false,\"children\": []},{\"fullCondition\": \"mp.info_code LIKE :info_code\",\"fullFieldName\":\"mp.info_code\",\"operationName\":\"like\",\"variableName\":\"infoCode\",\"nextToken\":\"and\",\"startIndex\": 3105,\"stopIndex\":3132,\"variableNum\": 4,\"variableCount\": 1,\"isor\":false,\"hasVariable\":true,\"hasBracket\":false,\"isVariableFirst\":false,\"topLevel\":false,\"children\": []},{\"fullCondition\": \"pr.plate_name like :design_team\",\"fullFieldName\":\"pr.plate_name\",\"operationName\":\"=\",\"variableName\":\"designTeam\",\"nextToken\":\"and\",\"startIndex\": 3138,\"stopIndex\":3166,\"variableNum\": 5,\"variableCount\": 1,\"isor\":false,\"hasVariable\":true,\"hasBracket\":false,\"isVariableFirst\":false,\"topLevel\":false,\"children\": []},{\"fullCondition\": \"mpe4.employee_name LIKE :leader\",\"fullFieldName\":\"mpe4.employee_name\",\"operationName\":\"like\",\"variableName\":\"leader\",\"nextToken\":\"and\",\"startIndex\": 3172,\"stopIndex\":3202,\"variableNum\": 6,\"variableCount\": 1,\"isor\":false,\"hasVariable\":true,\"hasBracket\":false,\"isVariableFirst\":false,\"topLevel\":false,\"children\": []},{\"fullCondition\": \"mpe1.employee_name LIKE :secretary\",\"fullFieldName\":\"mpe1.employee_name\",\"operationName\":\"like\",\"variableName\":\"secretary\",\"nextToken\":\"and\",\"startIndex\": 3208,\"stopIndex\":3241,\"variableNum\": 7,\"variableCount\": 1,\"isor\":false,\"hasVariable\":true,\"hasBracket\":false,\"isVariableFirst\":false,\"topLevel\":false,\"children\": []},{\"fullCondition\": \"mpe3.employee_name LIKE :manager\",\"fullFieldName\":\"mpe3.employee_name\",\"operationName\":\"like\",\"variableName\":\"manager\",\"nextToken\":\"and\",\"startIndex\": 3247,\"stopIndex\":3278,\"variableNum\": 8,\"variableCount\": 1,\"isor\":false,\"hasVariable\":true,\"hasBracket\":false,\"isVariableFirst\":false,\"topLevel\":false,\"children\": []},{\"fullCondition\": \"mpe5.employee_name LIKE :charge_person\",\"fullFieldName\":\"mpe5.employee_name\",\"operationName\":\"like\",\"variableName\":\"chargePerson\",\"nextToken\":\"and\",\"startIndex\": 3284,\"stopIndex\":3321,\"variableNum\": 9,\"variableCount\": 1,\"isor\":false,\"hasVariable\":true,\"hasBracket\":false,\"isVariableFirst\":false,\"topLevel\":false,\"children\": []},{\"fullCondition\": \"mp.process_status = 2\",\"startIndex\": 3327,\"stopIndex\":3347,\"variableNum\": 0,\"variableCount\": 0,\"isor\":false,\"hasVariable\":false,\"hasBracket\":false,\"isVariableFirst\":false,\"topLevel\":false,\"children\": []}]}]"
	};
	private final static String RESULTSETFIELDLIST = "main_project_id,project_code,project_name,project_english_name,project_status,design_status,sign_company,project_origination,project_origination_remark,design_cooperation_name,design_cooperation_english_name,project_sales_team,design_team,is_high_rise_building,info_register_time,project_approval_time,has_contract,project_base_on,info_code,project_model,design_qualification,business_type,fire_resistance_rating,estimate_the_design_areas,project_total_investment,actual_design_areas,total_building_areas,building_height,building_floors,decorate_money_control,project_level,province,city,detail_address,project_end_time,project_type,client_id,customer_name,customer_address,bid_project,construction_unit,architectural_design_units,rate_of_bid_winning,follow_up_date,equityedge_remark,order_sign_up_time,design_service_book,service_suggest_book,total_money,covered_area,design_area,has_formal_contracts,has_out_quotations,has_split_price,has_order_release,has_deposit,has_rate_of_payment,parent_id,has_formal_contract_time,has_out_quotation_time,has_order_release_time,has_split_price_time,bid_type,outsource,secretary,manager,leader,charge_person";
	private final static String[] fieldNames = { "project_code", "project_name", "has_contract", "project_base_on", "info_code", "design_team", "leader", "secretary", "manager", "charge_person"};
}
