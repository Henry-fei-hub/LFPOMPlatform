...
public class QueryTravelReimbursementTrans extends AbstractQuery<BaseTravelReimbursementTrans, ConditionTravelReimbursementTrans>
{

	private static final Logger __logger = Logger.getLogger(QueryTravelReimbursementTrans.class);

	public QueryTravelReimbursementTrans() throws java.sql.SQLException 
	{
	...
	@Override
	public BaseCollection<BaseTravelReimbursementTrans> executeQuery( KeyValuePair[] replacements, ConditionTravelReimbursementTrans condition ) throws java.sql.SQLException {
	...
	@Override
	public BaseTravelReimbursementTrans generateBase(Object[] __data) throws java.sql.SQLException {
		BaseTravelReimbursementTrans __base = new BaseTravelReimbursementTrans();
		int count = 0;
		Object val;
	...
	private final static String __SQLText = "select a.process_instance_id, a.process_type, a.business_id, a.create_time as record_date, a.process_status, a.complete_time, b.drafter, b.currency as currency_type, concat(b.code, case when b.code is null then '' else '-' end, c.department_name, case when c.department_name is null then '' else '-' end, d.employee_name, case when d.employee_name is null then '' else '-' end, b.remark) as summary, e.process_bill_list_id, e.pay_for, e.employee_or_company_id, e.pay_money, e.balance_amount, b.company_id from system_process_instances a LEFT JOIN travel_reimbursements b on a.business_id = b.travel_reimbursement_id LEFT JOIN departments c on b.department_id = c.department_id LEFT JOIN employees d on b.drafter = d.employee_id LEFT JOIN process_bill_list e on a.process_type = e.process_type and a.business_id = e.business_id where a.process_instance_id = ? and a.process_type = ? and a.business_id = ? and a.process_status = ? and a.delete_flag = ? and ((e.pay_for = 1 and e.balance_amount IS NOT NULL and e.balance_amount != 0) or e.pay_for = 2)" ;
