...
import pomplatform.projectteambuild.dao.ProjectWithP;
...
public class ProjectWithPHandler implements GenericDao {
	...
	public static boolean isProjectWithPExists(pomplatform.projectteambuild.bean.BaseProjectWithP bean, String additional) {
	...
	public static int countProjectWithP(pomplatform.projectteambuild.bean.BaseProjectWithP bean, String additional) {
	...
	public String update(String json) throws Exception {
		...
		int num = 0;
		if (dao.load()) {
			List<BaseProjectEmployee> list = bean.getDetailProjectEmployee(); 
			if(null!=list && !list.isEmpty()){
				for(BaseProjectEmployee obj : list){
					if(null==obj.getPlanIntegral()){
						obj.setPlanIntegral(BigDecimal.ZERO);
					}
					if(null == obj.getRealIntegral()){
						obj.setRealIntegral(BigDecimal.ZERO);
					}
				}
			}
			dao.setDataFromBase(bean);
