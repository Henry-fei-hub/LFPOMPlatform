...
import pomplatform.preProjects.bean.BaseCloseProjectApply;
import pomplatform.preProjects.bean.ConditionCloseProjectApply;
import pomplatform.preProjects.query.QueryCloseProjectApply;
...
public class CloseProjectApplyHandler implements GenericQuery,  GenericDownloadProcessor {

	private static final Logger __logger = Logger.getLogger(CloseProjectApplyHandler.class);

	public static BaseCollection<BaseCloseProjectApply> executeQueryCloseProjectApply(ConditionCloseProjectApply c, KeyValuePair[] replacements ) throws Exception {
		QueryCloseProjectApply dao = new QueryCloseProjectApply();
		dao.setCurrentPage(c.getCurrentPage());
		dao.setPageLines(c.getPageLines());
		BaseCollection<BaseCloseProjectApply> result = dao.executeQuery( c.getKeyValues(), c) ;
		if(c.isIncludeDetail() && result.getRecordNumber() > 0){
			...
			for(BaseCloseProjectApply bean : result.getCollections()){
	...
	public String find(String creteria) throws Exception {
		ConditionCloseProjectApply c = new ConditionCloseProjectApply();
		c.setDataFromJSON(creteria);
		BaseCollection<BaseCloseProjectApply> result = executeQueryCloseProjectApply(c, c.getKeyValues());
		return result.toJSON(null);
	}

	@Override
	public void execute(String creteria, File downFile, HttpServletRequest request, HttpServletResponse response) throws Exception {
		BaseCollection<BaseCloseProjectApply> result;
		ConditionCloseProjectApply c = new ConditionCloseProjectApply();
		c.setDataFromJSON(creteria);
		c.setCurrentPage(0);
		QueryCloseProjectApply dao = new QueryCloseProjectApply();
		...
		try(PrintStream ps = new PrintStream(downFile, "GBK")){
			ps.println(BaseCloseProjectApply.ALL_CAPTIONS);
			for(BaseCloseProjectApply b : result.getCollections()){
