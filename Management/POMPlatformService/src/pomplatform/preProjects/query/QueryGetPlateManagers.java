...
public class QueryGetPlateManagers extends AbstractQuery<BaseGetPlateManagers, ConditionGetPlateManagers>
{

	private static final Logger __logger = Logger.getLogger(QueryGetPlateManagers.class);

	public QueryGetPlateManagers() throws java.sql.SQLException 
	{
		setParameterNumber(0);
		setConnection(ThreadConnection.getConnection());
		setOrderByFields("* FROM employees where employee_id in (6");
	}

	@Override
	public BaseCollection<BaseGetPlateManagers> executeQuery( KeyValuePair[] replacements, ConditionGetPlateManagers condition ) throws java.sql.SQLException {
		return executeQuery(replacements
			);
	}

	@Override
	public BaseGetPlateManagers generateBase(Object[] __data) throws java.sql.SQLException {
		BaseGetPlateManagers __base = new BaseGetPlateManagers();
		int count = 0;
		Object val;
	...
	private final static String __SQLText = "select * FROM employees where employee_id in (SELECT employee_id FROM employee_roles WHERE role_id = 81) and status = 0 ORDER BY employee_id" ;
