package pomplatform.taskattachmentapprovals.query;
...
public class QueryTaskAttachmentApprovals extends AbstractQuery<BaseTaskAttachmentApprovals, ConditionTaskAttachmentApprovals>
{

	private static final Logger __logger = Logger.getLogger(QueryTaskAttachmentApprovals.class);

	public QueryTaskAttachmentApprovals() throws java.sql.SQLException 
	{
	...
	@Override
	public BaseCollection<BaseTaskAttachmentApprovals> executeQuery( KeyValuePair[] replacements, ConditionTaskAttachmentApprovals condition ) throws java.sql.SQLException {
	...
	@Override
	public BaseTaskAttachmentApprovals generateBase(Object[] __data) throws java.sql.SQLException {
		BaseTaskAttachmentApprovals __base = new BaseTaskAttachmentApprovals();
		int count = 0;
		Object val;
	...
	private final static String __SQLText = "SELECT sppt.process_pooled_task_id, sppt.business_id, sppt.business_name, sppt.process_activity_id, sppt.process_id, sppt.process_instance_id, sppt.back_view_name, sppt.node_type, sppt.activity_id, sppt.instance_activity_create_time, sppt.instance_activity_start_time, sppt.status, sppt.operate_time, sppt.process_comment, B.file_url, A .*, sppt.activity_type FROM system_process_pooled_tasks sppt LEFT JOIN budget_attachment A ON sppt.business_id = A.budget_attachment_id LEFT JOIN file_manages B ON A.file_id = B.file_id WHERE sppt.status =? AND sppt.process_type =? AND sppt.employee_id =? AND A.budget_management_id=? AND A.attachment_name =? AND A.upload_employee_id =? AND A.upload_time =? AND A.upload_remark=? AND A.project_id=? AND A.bdd_type=? AND A.is_approval=? AND A .department_id IN ( SELECT child_id FROM department_ids WHERE department_id =? ) ORDER BY sppt.instance_activity_create_time DESC" ;
	private final static String RESULTSETFIELDLIST = "process_pooled_task_id,business_id,business_name,process_activity_id,process_id,process_instance_id,back_view_name,node_type,activity_id,instance_activity_create_time,instance_activity_start_time,status,operate_time,process_comment,file_url,budget_attachment_id,budget_management_id,file_id,attachment_name,upload_employee_id,department_id,upload_time,upload_remark,budget_contract_amount,budget_contract_amount_use,budget_contract_amount_rest,receive_unit_manage_id,project_id,input_tax_invoice_id,bdd_type,is_approval,is_lock,activity_type";
