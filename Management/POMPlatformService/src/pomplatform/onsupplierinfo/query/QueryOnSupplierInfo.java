package pomplatform.onsupplierinfo.query;
...
public class QueryOnSupplierInfo extends AbstractQuery<BaseOnSupplierInfo, ConditionOnSupplierInfo>
{

	private static final Logger __logger = Logger.getLogger(QueryOnSupplierInfo.class);

	public QueryOnSupplierInfo() throws java.sql.SQLException 
	{
	...
	@Override
	public BaseCollection<BaseOnSupplierInfo> executeQuery( KeyValuePair[] replacements, ConditionOnSupplierInfo condition ) throws java.sql.SQLException {
	...
	@Override
	public BaseOnSupplierInfo generateBase(Object[] __data) throws java.sql.SQLException {
		BaseOnSupplierInfo __base = new BaseOnSupplierInfo();
		int count = 0;
		Object val;
	...
	private final static String __SQLText = "SELECT s.supplier_id, s.supplier_name, s.supplier_contact, s.supplier_phone, s.supplier_email, s.supplier_type, s.supplier_main_business_type, s.supplier_scope_supply, s.social_credit_code, s.supplier__province, s.supplier__city, s.supplier_bank_address, s.supplier_address, s.production_cycle, s.logistics_cycle, s.supplier_boss, s.contract_signing_time, s.contract_scope_time, s.contract_scope_time_start, s.contract_scope_time_end, s.operate_employee_id, s.operate_time, s.supplier_origin, s.supplier_status, s.is_use, s.receive_unit_manage_id, s.agreement_type, s.agreement_sign_time, s.agreement_sign_address, s.file_id, s.team_size, s.areas_expertise, f.file_url, f.file_name, A.process_instance_id, A.process_type, A.process_id, A.process_status FROM supplier s LEFT JOIN file_manages f ON s.file_id=f.file_id LEFT JOIN system_process_instances A ON s.supplier_id = A.business_id AND A.process_type = 53 where s.supplier_id = ? and s.supplier_name like ? and s.supplier_contact = ? and s.supplier_phone = ? and s.supplier_email = ? and s.supplier_type = ? and s.supplier_main_business_type = ? and s.supplier_scope_supply = ? and s.social_credit_code = ? and s.supplier__province = ? and s.supplier__city = ? and s.supplier_bank_address = ? and s.supplier_address like ? and s.production_cycle = ? and s.logistics_cycle = ? and s.supplier_boss = ? and s.contract_signing_time = ? and s.contract_scope_time = ? and s.contract_scope_time_start = ?_start and s.contract_scope_time_end = ?_end and s.operate_employee_id = ? and s.operate_time between ? and ? and s.supplier_origin = ? and s.supplier_status = ? and s.is_use = ? and s.receive_unit_manage_id = ? and s.agreement_type = ? and s.agreement_sign_time = ? and s.agreement_sign_address = ? and s.file_id = ? and s.team_size = ? and s.areas_expertise = ? and A.process_status =? ORDER BY supplier_id desc" ;
	private final static String RESULTSETFIELDLIST = "supplier_id,supplier_name,supplier_contact,supplier_phone,supplier_email,supplier_type,supplier_main_business_type,supplier_scope_supply,social_credit_code,supplier__province,supplier__city,supplier_bank_address,supplier_address,production_cycle,logistics_cycle,supplier_boss,contract_signing_time,contract_scope_time,contract_scope_time_start,contract_scope_time_end,operate_employee_id,operate_time,supplier_origin,supplier_status,is_use,receive_unit_manage_id,agreement_type,agreement_sign_time,agreement_sign_address,file_id,team_size,areas_expertise,file_url,file_name,process_instance_id,process_type,process_id,process_status";
	private final static String[] fieldNames = { "supplier_id", "supplier_name", "supplier_contact", "supplier_phone", "supplier_email", "supplier_type", "supplier_main_business_type", "supplier_scope_supply", "social_credit_code", "supplier__province", "supplier__city", "supplier_bank_address", "supplier_address", "production_cycle", "logistics_cycle", "supplier_boss", "contract_signing_time", "contract_scope_time", "contract_scope_time_start", "contract_scope_time_end", "operate_employee_id", "operate_time_start", "operate_time_end", "supplier_origin", "supplier_status", "is_use", "receive_unit_manage_id", "agreement_type", "agreement_sign_time", "agreement_sign_address", "file_id", "team_size", "areas_expertise", "process_status"};
}
