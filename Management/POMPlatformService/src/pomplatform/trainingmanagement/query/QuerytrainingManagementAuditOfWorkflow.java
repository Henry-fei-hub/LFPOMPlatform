...
public class QuerytrainingManagementAuditOfWorkflow extends AbstractQuery<BasetrainingManagementAuditOfWorkflow, ConditiontrainingManagementAuditOfWorkflow>
{

	private static final Logger __logger = Logger.getLogger(QuerytrainingManagementAuditOfWorkflow.class);

	public QuerytrainingManagementAuditOfWorkflow() throws java.sql.SQLException 
	{
	...
	@Override
	public BaseCollection<BasetrainingManagementAuditOfWorkflow> executeQuery( KeyValuePair[] replacements, ConditiontrainingManagementAuditOfWorkflow condition ) throws java.sql.SQLException {
	...
	@Override
	public BasetrainingManagementAuditOfWorkflow generateBase(Object[] __data) throws java.sql.SQLException {
		BasetrainingManagementAuditOfWorkflow __base = new BasetrainingManagementAuditOfWorkflow();
		int count = 0;
		Object val;
	...
	private final static String __SQLText = "SELECT spt.process_type_name, sppt.process_pooled_task_id, CASE WHEN sppt.process_pooled_task_id > 0 THEN 2 ELSE 1 END AS type_flag, spia.process_instance_activity_id, spia.business_id, spia.business_name, spia.process_activity_id, spia.process_id, spia.process_instance_id, spia.node_type, spia.activity_id, spia.next_activity_id, spia.main_activity_id, spia.instance_activity_create_time, spia.instance_activity_start_time, spia.operate_time, spia.process_comment, spia.status, spia.activity_type, spia.back_view_name, spia.process_type, tm.training_management_id, tm.subject, tm.start_time, tm.end_time, tm.compere_type, tm.compere, tm.employee_id, tm.department_id, tm.sign_in_delay, tm.sign_out_delay, tm.course_score, tm.create_person_id, tm.file_manage_id, tm.site, tm.plate_id, tm.is_site, tm.training_type, tm.training_fee, tm.file_id, tm.file_url, tm.is_send, tm.plateids FROM system_process_instance_activities AS spia LEFT JOIN system_process_pooled_tasks sppt ON spia.process_instance_id = sppt.process_instance_id AND spia.process_instance_activity_id = sppt.instance_activity_id LEFT JOIN system_process_instances spi ON spia.process_instance_id = spi.process_instance_id LEFT JOIN system_process_types spt ON spia.process_type = spt.process_type_id LEFT JOIN training_managements tm ON spia.business_id = tm.training_management_id WHERE spia.activity_type IN ( 2, 3 ) AND spi.delete_flag = 0 AND spt.parent_process_type_id =12 AND (( spia.employee_id > 0 AND sppt.process_pooled_task_id IS NULL AND spia.employee_id =? AND spia.status =? ) OR ( spia.employee_id > 0 AND sppt.process_pooled_task_id > 0 AND spia.employee_id =? AND sppt.employee_id =? AND sppt.status =? ) OR ( spia.employee_id IS NULL AND sppt.employee_id =? AND sppt.status =?)) AND spia.process_type =59 AND tm.training_management_id =? AND tm.subject like ? AND tm.compere_type =? AND tm.compere like ? AND tm.department_id =? AND tm.site =? AND tm.plate_id =? AND tm.is_site =? AND tm.training_type =? AND tm.is_send =? ORDER BY spia.instance_activity_create_time DESC" ;
	private final static String RESULTSETFIELDLIST = "process_type_name,process_pooled_task_id,type_flag,process_instance_activity_id,business_id,business_name,process_activity_id,process_id,process_instance_id,node_type,activity_id,next_activity_id,main_activity_id,instance_activity_create_time,instance_activity_start_time,operate_time,process_comment,status,activity_type,back_view_name,process_type,training_management_id,subject,start_time,end_time,compere_type,compere,employee_id,department_id,sign_in_delay,sign_out_delay,course_score,create_person_id,file_manage_id,site,plate_id,is_site,training_type,training_fee,file_id,file_url,is_send,plateids";
	private final static String[] fieldNames = {"employee_id", "status", "employee_id", "employee_id", "status", "employee_id", "status", "training_management_id", "subject", "compere_type", "compere", "department_id", "site", "plate_id", "is_site", "training_type", "is_send"};
}
