...
public class AllQueryMcapitalecoosmeor extends AbstractQuery<BaseMcapitalecoosmeor, ConditionMcapitalecoosmeor>
{

	private static final Logger __logger = Logger.getLogger(AllQueryMcapitalecoosmeor.class);

	public AllQueryMcapitalecoosmeor() throws java.sql.SQLException 
	{
	...
	private final static String __SQLText = "select c.capital_id, c.info_code, c.project_code, c.contract_id,st.contract_name,st.contract_code, c.money_attribute, c.self_name, c.self_bank_name, c.self_bank_account, c.other_name, c.other_bank_name, c.other_bank_account, c.borrow_money, c.loan_money, c.operator, c.happen_date, c.create_date, c.has_invoices, c.has_contract_receivable,e.employee_name,c.remark,c.has_distribution_collection,c.capital_type,c.reimbursement_package_summary_id,c.process_status,c.capital_distribution_id,c.have_voucher, c.record_receive_capital, c.exchange_rate, c.original_currency , fm.file_url, c.issue_date, c.expire_date, c.receive_unit_id, rum.customer_name as receive_unit, c.actual_amount, c.actual_receive_date from capitals c left join employees e on c.operator=e.employee_id left join contracts st on st.contract_id=c.contract_id LEFT JOIN file_manages fm ON fm.file_id = c.file_id left join customers rum on c.receive_unit_id = rum.customer_id where c.contract_id= ? and c.other_name like ? and c.other_bank_account like ? and c.self_name = ? and c.money_attribute = ? and e.employee_id=? and c.has_invoices=? and c.has_contract_receivable=? and c.has_distribution_collection=? and c.capital_type = 1 AND c.happen_date BETWEEN ? AND ? AND process_status =? AND st.contract_code LIKE ? AND st.contract_name LIKE ? AND borrow_money = ? and c.have_voucher = ? order by c.create_date desc" ;
	private final static String RESULTSETFIELDLIST = "capital_id,info_code,project_code,contract_id,contract_name,contract_code,money_attribute,self_name,self_bank_name,self_bank_account,other_name,other_bank_name,other_bank_account,borrow_money,loan_money,operator,happen_date,create_date,has_invoices,has_contract_receivable,employee_name,remark,has_distribution_collection,capital_type,reimbursement_package_summary_id,process_status,capital_distribution_id,have_voucher,record_receive_capital,exchange_rate,original_currency,file_url,issue_date,expire_date,receive_unit_id,receive_unit,actual_amount,actual_receive_date";
	private final static String[] fieldNames = { "contract_id", "other_name", "other_bank_account", "self_name", "money_attribute", "employee_id","has_invoices","has_contract_receivable","has_distribution_collection","min_happen_date","max_happen_date", "process_status", "contract_code", "contract_name", "borrow_money", "have_voucher" , "file_url"};
}
