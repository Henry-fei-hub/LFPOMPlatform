...
public class QueryOnLoadOtherProjectDetailOfPlateAccount extends AbstractQuery<BaseOnLoadOtherProjectDetailOfPlateAccount, ConditionOnLoadOtherProjectDetailOfPlateAccount>
{

	private static final Logger __logger = Logger.getLogger(QueryOnLoadOtherProjectDetailOfPlateAccount.class);

	public QueryOnLoadOtherProjectDetailOfPlateAccount() throws java.sql.SQLException 
	{
	...
	@Override
	public BaseCollection<BaseOnLoadOtherProjectDetailOfPlateAccount> executeQuery( KeyValuePair[] replacements, ConditionOnLoadOtherProjectDetailOfPlateAccount condition ) throws java.sql.SQLException {
	...
	@Override
	public BaseOnLoadOtherProjectDetailOfPlateAccount generateBase(Object[] __data) throws java.sql.SQLException {
		BaseOnLoadOtherProjectDetailOfPlateAccount __base = new BaseOnLoadOtherProjectDetailOfPlateAccount();
		int count = 0;
		Object val;
	...
	private final static String __SQLText = "select a.plate_id,c.contract_code as sheet_code,c.project_name as sheet_name,a.integral,a.record_date,a.remark from plate_account_records a left join projects c on a.business_id = c.project_id and c.project_flag = 2 where a.plate_id = ? and a.business_type_id = ? and c.contract_code like ? and c.project_name like ? and EXTRACT(YEAR from a.record_date) =? and EXTRACT(MONTH from a.record_date) =? ORDER BY c.project_id,a.integral desc" ;
