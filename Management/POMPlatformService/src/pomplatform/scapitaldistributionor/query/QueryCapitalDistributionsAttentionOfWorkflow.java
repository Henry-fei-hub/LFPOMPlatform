...
public class QueryCapitalDistributionsAttentionOfWorkflow extends AbstractQuery<BaseCapitalDistributionsAttentionOfWorkflow, ConditionCapitalDistributionsAttentionOfWorkflow>
{

	private static final Logger __logger = Logger.getLogger(QueryCapitalDistributionsAttentionOfWorkflow.class);

	public QueryCapitalDistributionsAttentionOfWorkflow() throws java.sql.SQLException 
	{
	...
	@Override
	public BaseCollection<BaseCapitalDistributionsAttentionOfWorkflow> executeQuery( KeyValuePair[] replacements, ConditionCapitalDistributionsAttentionOfWorkflow condition ) throws java.sql.SQLException {
	...
	@Override
	public BaseCapitalDistributionsAttentionOfWorkflow generateBase(Object[] __data) throws java.sql.SQLException {
		BaseCapitalDistributionsAttentionOfWorkflow __base = new BaseCapitalDistributionsAttentionOfWorkflow();
		int count = 0;
		Object val;
	...
	private final static String __SQLText = "SELECT spa.system_process_attention_id, spa.business_id, spa.business_name, spa.process_activity_id, spa.process_id, spa.process_instance_id, spa.back_view_name, spa.instance_activity_id, spa.instance_activity_create_time, spa.instance_activity_start_time, spa.status, spa.process_type, spa.operate_time, spa.process_comment, a.department_id, a.employee_id, a.code, a.company_id, a.title, a.contract_change_amount, a.main_professional, a.remark, ct.contract_code, ct.signing_money_sum, ct.contract_name, ct.customer_name, cp.happen_date, cp.other_name, cp.borrow_money, aa.all_borrow_money FROM system_process_attentions spa LEFT JOIN capital_distributions A ON spa.business_id = a.capital_distribution_id LEFT JOIN contracts ct ON ct.contract_id = a.contract_id LEFT JOIN capitals cp ON cp.capital_id = a.capital_id LEFT JOIN ( SELECT SUM (borrow_money) AS all_borrow_money, contract_id FROM capitals GROUP BY contract_id ) aa ON aa.contract_id = a.contract_id WHERE spa.status = ? AND spa.process_type = ? AND spa.employee_id = ? AND spa.delete_flag = 0 AND a.delete_flag = 0 AND a.company_id = ? AND ct.contract_name LIKE ? AND cp.borrow_money = ? AND ct.customer_name LIKE ? AND a.title LIKE ? AND cp.other_name LIKE ? AND a.department_id = ? AND a.code LIKE ? ORDER BY spa.instance_activity_create_time DESC" ;
	private final static String RESULTSETFIELDLIST = "system_process_attention_id,business_id,business_name,process_activity_id,process_id,process_instance_id,back_view_name,instance_activity_id,instance_activity_create_time,instance_activity_start_time,status,process_type,operate_time,process_comment,department_id,employee_id,code,company_id,title,contract_change_amount,main_professional,remark,contract_code,signing_money_sum,contract_name,customer_name,happen_date,other_name,borrow_money,all_borrow_money";
