package pomplatform.employeenew.query;
...
public class QuerySemployeeNew extends AbstractQuery<BaseSemployeeNew, ConditionSemployeeNew>
{

	private static final Logger __logger = Logger.getLogger(QuerySemployeeNew.class);

	public QuerySemployeeNew() throws java.sql.SQLException 
	{
	...
	@Override
	public BaseCollection<BaseSemployeeNew> executeQuery( KeyValuePair[] replacements, ConditionSemployeeNew condition ) throws java.sql.SQLException {
	...
	@Override
	public BaseSemployeeNew generateBase(Object[] __data) throws java.sql.SQLException {
		BaseSemployeeNew __base = new BaseSemployeeNew();
		int count = 0;
		Object val;
	...
	public int setStatementParameters(int count, java.lang.Object ... args) throws java.sql.SQLException {
		...
		if(args[8] != null) __statement.setInt(count++, (java.lang.Integer)args[8]);
		if(args[9] != null) __statement.setBoolean(count++, (java.lang.Boolean)args[9]);
		if(args[10] != null) __statement.setInt(count++, (java.lang.Integer)args[10]);
		return count;
	}
	...
	private final static String __SQLText = "select e.employee_id, e.employee_no, e.employee_name, e.department_id, e.role_id, e.grade_id, e.employee_password, e.mobile, e.phone, e.qq, e.email, e.onboard_date, e.resignation_date, e.status, e.usable_status, e.is_department, concat(s.image_path,case when e.photo is null then 'photo_view.png' else e.photo end) as photo,e.gender, e.autograph, e.age, e.birth, e.card, e.address, e.alternate_field1, e.alternate_field2, e.alternate_field3, e.locked, e.plate_id, e.duty_id, e.user_acct, e.employee_name_en, e.education, e.degree, e.nationality, e.married_status, e.health, e.work_address, e.registered_address,e.direct_leader,e.company_email from employees e left join system_config s on s.system_config_id = 1 where e.employee_id = ? and e.plate_id = ? and e.duty_id = ? and e.employee_no = ? and e.employee_name like ? and e.department_id in (select child_id from department_ids where department_id = ? ) and e.role_id = ? and e.grade_id = ? and e.status = ? and e.is_department = ? and e.gender = ? and plate_id <> 0 " ;
	private final static String RESULTSETFIELDLIST = "employee_id,employee_no,employee_name,department_id,role_id,grade_id,employee_password,mobile,phone,qq,email,onboard_date,resignation_date,status,usable_status,is_department,photo,gender,autograph,age,birth,card,address,alternate_field1,alternate_field2,alternate_field3,locked,plate_id,duty_id,user_acct,employee_name_en,education,degree,nationality,married_status,health,work_address,registered_address,direct_leader,company_email";
	private final static String[] fieldNames = { "employee_id", "plate_id", "duty_id", "employee_no", "employee_name", "department_id", "role_id", "grade_id", "status", "is_department", "gender"};
}
