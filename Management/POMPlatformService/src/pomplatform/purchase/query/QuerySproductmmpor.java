...
public class QuerySproductmmpor extends AbstractQuery<BaseSproductmmpor, ConditionSproductmmpor>
{

	private static final Logger __logger = Logger.getLogger(QuerySproductmmpor.class);

	public QuerySproductmmpor() throws java.sql.SQLException 
	{
	...
	@Override
	public BaseCollection<BaseSproductmmpor> executeQuery( KeyValuePair[] replacements, ConditionSproductmmpor condition ) throws java.sql.SQLException {
	...
	@Override
	public BaseSproductmmpor generateBase(Object[] __data) throws java.sql.SQLException {
		BaseSproductmmpor __base = new BaseSproductmmpor();
		int count = 0;
		Object val;
	...
	private final static String __SQLText = "select p.product_id, p.category_id, p.product_name, p.product_type, p.product_model, p.product_specification, p.product_skins_size, p.is_enabled, p.create_employee_id, p.create_time, p.delete_flag, p.remark, p.product_unit from products p where p.create_time between ? and ? and p.product_model like ? and p.product_specification like ? and p.remark like ? and p.product_id = ? and p.category_id = ? and p.product_type = ? and p.is_enabled = ? and p.create_employee_id = ? and p.delete_flag = ?" ;
	private final static String RESULTSETFIELDLIST = "product_id,category_id,product_name,product_type,product_model,product_specification,product_skins_size,is_enabled,create_employee_id,create_time,delete_flag,remark,product_unit";
