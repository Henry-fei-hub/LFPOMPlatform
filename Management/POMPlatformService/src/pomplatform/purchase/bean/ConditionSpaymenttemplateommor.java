package pomplatform.purchase.bean;

import delicacy.common.GenericCondition;
import java.util.Map;

public class ConditionSpaymenttemplateommor extends GenericCondition{

	public ConditionSpaymenttemplateommor(){
		setParameterCount(5);
	}

	public java.lang.Integer getOperatorId() {
		return this.__operator_id;
	}

	public void setOperatorId( java.lang.Integer value ) {
		this.__operator_id = value;
	}

	public java.lang.Integer getMinPaymentCycle() {
		return this.__min_payment_cycle;
	}

	public void setMinPaymentCycle( java.lang.Integer value ) {
		this.__min_payment_cycle = value;
	}

	public java.lang.Integer getMaxPaymentCycle() {
		return this.__max_payment_cycle;
	}

	public void setMaxPaymentCycle( java.lang.Integer value ) {
		this.__max_payment_cycle = value;
	}

	public java.lang.String getTemplateName() {
		return this.__template_name == null ? null : (this.__template_name.indexOf("%") >= 0 ? this.__template_name : "%"+this.__template_name+"%");
	}

	public void setTemplateName( java.lang.String value ) {
		this.__template_name = value;
	}

	public java.lang.String getRemark() {
		return this.__remark == null ? null : (this.__remark.indexOf("%") >= 0 ? this.__remark : "%"+this.__remark+"%");
	}

	public void setRemark( java.lang.String value ) {
		this.__remark = value;
	}

	@Override
	public java.lang.String toJSONString() {

		StringBuilder sb = new StringBuilder();
		sb.append(super.toJSONString());
		if(getOperatorId() != null) sb.append(__wrapNumber(1, "operatorId", getOperatorId()));
		if(getMinPaymentCycle() != null) sb.append(__wrapNumber(1, "minPaymentCycle", getMinPaymentCycle()));
		if(getMaxPaymentCycle() != null) sb.append(__wrapNumber(1, "maxPaymentCycle", getMaxPaymentCycle()));
		if(getTemplateName() != null) sb.append(__wrapString(1, "templateName", getTemplateName()));
		if(getRemark() != null) sb.append(__wrapString(1, "remark", getRemark()));
		return sb.toString();
	}

	@Override
	public void setDataFromMap(Map<String, Object> values){
		super.setDataFromMap(values);
		Object val;
		if((val = values.get("operatorId")) != null) setOperatorId(__getInt(val)); 
		if((val = values.get("minPaymentCycle")) != null) setMinPaymentCycle(__getInt(val)); 
		if((val = values.get("maxPaymentCycle")) != null) setMaxPaymentCycle(__getInt(val)); 
		if((val = values.get("templateName")) != null) setTemplateName(__getString(val));
		if((val = values.get("remark")) != null) setRemark(__getString(val));
	}

	private java.lang.Integer __operator_id = null;
	private java.lang.Integer __min_payment_cycle = null;
	private java.lang.Integer __max_payment_cycle = null;
	private java.lang.String __template_name = null;
	private java.lang.String __remark = null;
}

