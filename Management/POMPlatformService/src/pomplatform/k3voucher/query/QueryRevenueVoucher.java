...
import pomplatform.k3voucher.bean.BaseRevenueVoucher;
import pomplatform.k3voucher.bean.ConditionRevenueVoucher;

public class QueryRevenueVoucher extends AbstractQuery<BaseRevenueVoucher, ConditionRevenueVoucher>
{

	private static final Logger __logger = Logger.getLogger(QueryRevenueVoucher.class);

	public QueryRevenueVoucher() throws java.sql.SQLException 
	{
	...
	@Override
	public BaseCollection<BaseRevenueVoucher> executeQuery( KeyValuePair[] replacements, ConditionRevenueVoucher condition ) throws java.sql.SQLException {
		return executeQuery(replacements, 
				...
				condition.getCodeId(), 
				condition.getSecondaryCodeType(), 
				condition.getSecondaryCodeId(), 
	...
	@Override
	public BaseRevenueVoucher generateBase(Object[] __data) throws java.sql.SQLException {
		BaseRevenueVoucher __base = new BaseRevenueVoucher();
		int count = 0;
		Object val;
		...
		if((val = __data[count++]) != null) __base.setShowName(GenericBase.__getString(val));
	...
	private final static String __SQLText = "SELECT kv.k3_voucher_id, kv.project_id, kv.company_id, kv.sequence_number, kv.internal_sequence_number, kv.code_type, kv.code_id, kv.code, kv.name, kv.secondary_code_type, kv.secondary_code_id, kv.secondary_code, kv.secondary_name, kv.remark, kv.simplify_remark, kv.debit_side, kv.credit_side, kv.reimbursement_package_code, kv.amount_of_reimbursement, kv.input_tax, kv.self_bank_account, kv.other_bank_account, kv.need_to_change, kv.vesting_date, kv.locked, kv.create_time, kv.update_time, kv.locked_time, kv.link_id, kv.voucher_type, kv.additional_link_id, CONCAT( CASE WHEN kv.additional_link_id >=0 THEN pr.dic_type_value ELSE ct.contract_name END ) AS show_name, ct.customer_id FROM k3_vouchers kv LEFT JOIN contracts ct ON ct.contract_id = kv.link_id LEFT JOIN system_dictionary pr ON pr.dic_type_value_id = kv.link_id AND pr.dic_type_id = 31 WHERE kv.create_time BETWEEN ? AND ? AND kv.update_time BETWEEN ? AND ? AND kv.vesting_date BETWEEN ? AND ? AND kv.code LIKE ? AND kv.name LIKE ? AND kv.secondary_code LIKE ? AND kv.secondary_name LIKE ? AND kv.remark LIKE ? AND kv.simplify_remark LIKE ? AND kv.additional_link_id = ? AND kv.k3_voucher_id = ? AND kv.code_type = ? AND kv.code_id = ? AND kv.secondary_code_type = ? AND kv.secondary_code_id = ? AND kv.debit_side = ? AND kv.credit_side = ? AND kv.locked = ? AND kv.link_id = ? AND kv.voucher_type = ? AND ct.contract_name LIKE ? ORDER BY kv.sequence_number desc, kv.internal_sequence_number" ;
	private final static String RESULTSETFIELDLIST = "k3_voucher_id,project_id,company_id,sequence_number,internal_sequence_number,code_type,code_id,code,name,secondary_code_type,secondary_code_id,secondary_code,secondary_name,remark,simplify_remark,debit_side,credit_side,reimbursement_package_code,amount_of_reimbursement,input_tax,self_bank_account,other_bank_account,need_to_change,vesting_date,locked,create_time,update_time,locked_time,link_id,voucher_type,additional_link_id,show_name,customer_id";
	private final static String[] fieldNames = { "min_create_time", "max_create_time", "min_update_time", "max_update_time", "min_vesting_date", "max_vesting_date", "code", "name", "secondary_code", "secondary_name", "remark", "simplify_remark", "additional_link_id", "k3_voucher_id", "code_type", "code_id", "secondary_code_type", "secondary_code_id", "debit_side", "credit_side", "locked", "link_id", "voucher_type", "contract_name"};
}
