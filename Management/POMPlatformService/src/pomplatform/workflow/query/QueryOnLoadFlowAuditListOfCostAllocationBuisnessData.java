...
public class QueryOnLoadFlowAuditListOfCostAllocationBuisnessData extends AbstractQuery<BaseOnLoadFlowAuditListOfCostAllocationBuisnessData, ConditionOnLoadFlowAuditListOfCostAllocationBuisnessData>
{

	private static final Logger __logger = Logger.getLogger(QueryOnLoadFlowAuditListOfCostAllocationBuisnessData.class);

	public QueryOnLoadFlowAuditListOfCostAllocationBuisnessData() throws java.sql.SQLException 
	{
	...
	@Override
	public BaseCollection<BaseOnLoadFlowAuditListOfCostAllocationBuisnessData> executeQuery( KeyValuePair[] replacements, ConditionOnLoadFlowAuditListOfCostAllocationBuisnessData condition ) throws java.sql.SQLException {
	...
	@Override
	public BaseOnLoadFlowAuditListOfCostAllocationBuisnessData generateBase(Object[] __data) throws java.sql.SQLException {
		BaseOnLoadFlowAuditListOfCostAllocationBuisnessData __base = new BaseOnLoadFlowAuditListOfCostAllocationBuisnessData();
		int count = 0;
		Object val;
	...
	private final static String __SQLText = "SELECT spt.process_type_name, sppt.process_pooled_task_id, CASE WHEN sppt.process_pooled_task_id > 0 THEN 2 ELSE 1 END AS type_flag, spia.process_instance_activity_id, spia.business_id, spia.business_name, spia.process_activity_id, spia.process_id, spia.process_instance_id, spia.node_type, spia.activity_id, spia.next_activity_id, spia.main_activity_id, spia.instance_activity_create_time, spia.instance_activity_start_time, spia.status, spia.process_type, spia.operate_time, spia.process_comment, spia.activity_type, spia.back_view_name, rc.cost_type, rc.code, rc.NAME, rc.COST, rc.drafter, rc.reason FROM system_process_instance_activities AS spia LEFT JOIN reset_costs rc ON spia.business_id = rc.reset_cost_id LEFT JOIN system_process_pooled_tasks sppt ON spia.process_instance_id = sppt.process_instance_id AND spia.process_instance_activity_id = sppt.instance_activity_id LEFT JOIN system_process_instances spi ON spia.process_instance_id = spi.process_instance_id LEFT JOIN system_process_types spt ON spia.process_type = spt.process_type_id WHERE spia.activity_type IN ( 2, 3 ) AND spi.process_status =? AND spi.delete_flag = 0 AND spt.parent_process_type_id = 38 AND ((spia.employee_id > 0 AND sppt.process_pooled_task_id IS NULL AND spia.employee_id =? AND spia.status =?) OR (spia.employee_id > 0 AND sppt.process_pooled_task_id > 0 AND spia.employee_id =? AND sppt.employee_id =? AND sppt.status =?) OR (spia.employee_id IS NULL AND sppt.employee_id =? AND sppt.status =?)) AND spia.process_type =? AND rc.code LIKE ? AND rc.NAME LIKE ? AND rc.COST =? AND rc.cost_type =? ORDER BY spia.instance_activity_create_time DESC" ;
	private final static String RESULTSETFIELDLIST = "process_type_name,process_pooled_task_id,type_flag,process_instance_activity_id,business_id,business_name,process_activity_id,process_id,process_instance_id,node_type,activity_id,next_activity_id,main_activity_id,instance_activity_create_time,instance_activity_start_time,status,process_type,operate_time,process_comment,activity_type,back_view_name,cost_type,code,name,cost,drafter,reason";
	private final static String[] fieldNames = { "process_status", "employee_id", "status", "employee_id", "employee_id", "status", "employee_id", "status", "process_type", "code", "name", "cost", "cost_type"};
}
