...
import pomplatform.workflow.bean.BaseOnLoadFlowNormalReimbursementsApplyBuisnessData;
import pomplatform.workflow.bean.ConditionOnLoadFlowNormalReimbursementsApplyBuisnessData;

public class QueryOnLoadFlowNormalReimbursementsApplyBuisnessData extends AbstractQuery<BaseOnLoadFlowNormalReimbursementsApplyBuisnessData, ConditionOnLoadFlowNormalReimbursementsApplyBuisnessData>
{

	private static final Logger __logger = Logger.getLogger(QueryOnLoadFlowNormalReimbursementsApplyBuisnessData.class);

	public QueryOnLoadFlowNormalReimbursementsApplyBuisnessData() throws java.sql.SQLException 
	{
	...
	@Override
	public BaseCollection<BaseOnLoadFlowNormalReimbursementsApplyBuisnessData> executeQuery( KeyValuePair[] replacements, ConditionOnLoadFlowNormalReimbursementsApplyBuisnessData condition ) throws java.sql.SQLException {
	...
	@Override
	public BaseOnLoadFlowNormalReimbursementsApplyBuisnessData generateBase(Object[] __data) throws java.sql.SQLException {
		BaseOnLoadFlowNormalReimbursementsApplyBuisnessData __base = new BaseOnLoadFlowNormalReimbursementsApplyBuisnessData();
		int count = 0;
		Object val;
	...
	private final static String __SQLText = "SELECT spt.process_type_name, sppt.process_pooled_task_id, CASE WHEN sppt.process_pooled_task_id > 0 THEN 2 ELSE 1 END AS type_flag, spia.process_instance_activity_id, spia.business_id, spia.business_name, spia.process_activity_id, spia.process_id, spia.process_instance_id, spia.node_type, spia.activity_id, spia.next_activity_id, spia.main_activity_id, spia.instance_activity_create_time, spia.instance_activity_start_time, spia.operate_time, spia.process_comment, spia.status, spia.activity_type, spia.back_view_name, nr.code, nr.employee_id, nr.department_id, nr.amount, nr.company_id, fib.receive_status, nr.reimbursement_name AS title, spia.process_type FROM system_process_instance_activities AS spia LEFT JOIN system_process_pooled_tasks sppt ON spia.process_instance_id = sppt.process_instance_id AND spia.process_instance_activity_id = sppt.instance_activity_id LEFT JOIN system_process_instances spi ON spia.process_instance_id = spi.process_instance_id LEFT JOIN system_process_types spt ON spia.process_type = spt.process_type_id LEFT JOIN project_normal_reimbursements nr ON spia.business_id = nr.project_normal_reimbursement_id LEFT JOIN finance_receive_bills fib ON fib.code = nr.code WHERE spia.activity_type IN ( 2, 3 ) AND spi.delete_flag = 0 AND spt.parent_process_type_id = 1 AND ((spia.employee_id > 0 AND sppt.process_pooled_task_id IS NULL AND spia.employee_id =? AND spia.status =?) OR (spia.employee_id > 0 AND sppt.process_pooled_task_id > 0 AND spia.employee_id =? AND sppt.employee_id =? AND sppt.status =?) OR (spia.employee_id IS NULL AND sppt.employee_id =? AND sppt.status =?)) AND spia.process_type =? AND nr.company_id =? AND nr.department_id IN ( SELECT child_id FROM department_ids WHERE department_id =? ) AND nr.code LIKE ? AND nr.apply_employee_id =? ORDER BY spia.instance_activity_create_time DESC" ;
	private final static String RESULTSETFIELDLIST = "process_type_name,process_pooled_task_id,type_flag,process_instance_activity_id,business_id,business_name,process_activity_id,process_id,process_instance_id,node_type,activity_id,next_activity_id,main_activity_id,instance_activity_create_time,instance_activity_start_time,operate_time,process_comment,status,activity_type,back_view_name,code,employee_id,department_id,amount,company_id,receive_status,title,process_type";
	private final static String[] fieldNames = { "employee_id", "status", "employee_id", "employee_id", "status", "employee_id", "status", "process_type", "company_id", "department_id", "code", "apply_employee_id"};
}
