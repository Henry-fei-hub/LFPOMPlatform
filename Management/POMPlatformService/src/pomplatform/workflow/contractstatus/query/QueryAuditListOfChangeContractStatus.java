...
public class QueryAuditListOfChangeContractStatus extends AbstractQuery<BaseAuditListOfChangeContractStatus, ConditionAuditListOfChangeContractStatus>
{

	private static final Logger __logger = Logger.getLogger(QueryAuditListOfChangeContractStatus.class);

	public QueryAuditListOfChangeContractStatus() throws java.sql.SQLException 
	{
	...
	@Override
	public BaseCollection<BaseAuditListOfChangeContractStatus> executeQuery( KeyValuePair[] replacements, ConditionAuditListOfChangeContractStatus condition ) throws java.sql.SQLException {
	...
	@Override
	public BaseAuditListOfChangeContractStatus generateBase(Object[] __data) throws java.sql.SQLException {
		BaseAuditListOfChangeContractStatus __base = new BaseAuditListOfChangeContractStatus();
		int count = 0;
		Object val;
	...
	private final static String __SQLText = "SELECT spt.process_type_name, sppt.process_pooled_task_id, CASE WHEN sppt.process_pooled_task_id > 0 THEN 2 ELSE 1 END AS type_flag, spia.process_instance_activity_id, spia.business_id, spia.business_name, spia.process_activity_id, spia.process_id, spia.process_instance_id, spia.node_type, spia.activity_id, spia.next_activity_id, spia.main_activity_id, spia.instance_activity_create_time, spia.instance_activity_start_time, spia.operate_time, spia.process_comment, spia.status, spia.activity_type, spia.back_view_name, spia.process_type, cl.contract_id, c.contract_code, c.contract_name, cl.content, cl.remark, cl.attachment_id, fm.file_url FROM system_process_instance_activities AS spia LEFT JOIN system_process_pooled_tasks sppt ON spia.process_instance_id = sppt.process_instance_id AND spia.process_instance_activity_id = sppt.instance_activity_id LEFT JOIN system_process_instances spi ON spia.process_instance_id = spi.process_instance_id LEFT JOIN system_process_types spt ON spia.process_type = spt.process_type_id LEFT JOIN contract_logs cl ON spia.business_id = cl.contract_log_id LEFT JOIN contracts c on cl.contract_id = c.contract_id LEFT JOIN file_manages fm on cl.attachment_id = fm.file_id WHERE spia.activity_type IN ( 2, 3 ) AND spi.delete_flag = 0 AND ((spia.employee_id > 0 AND sppt.process_pooled_task_id IS NULL AND spia.employee_id = ? AND spia.status = ?) OR (spia.employee_id > 0 AND sppt.process_pooled_task_id > 0 AND spia.employee_id = ? AND sppt.employee_id = ? AND sppt.status = ?) OR (spia.employee_id IS NULL AND sppt.employee_id = ? AND sppt.status = ?)) AND spia.process_type = ? and cl.operator = ? and cl.content like ? and c.contract_code like ? and c.contract_name like ? ORDER BY spia.instance_activity_create_time DESC" ;
	private final static String RESULTSETFIELDLIST = "process_type_name,process_pooled_task_id,type_flag,process_instance_activity_id,business_id,business_name,process_activity_id,process_id,process_instance_id,node_type,activity_id,next_activity_id,main_activity_id,instance_activity_create_time,instance_activity_start_time,operate_time,process_comment,status,activity_type,back_view_name,process_type,contract_id,contract_code,contract_name,content,remark,attachment_id,file_url";
	private final static String[] fieldNames = { "employee_id", "status", "employee_id", "employee_id", "status", "employee_id", "status", "process_type", "operator", "content", "contract_code", "contract_name"};
}
