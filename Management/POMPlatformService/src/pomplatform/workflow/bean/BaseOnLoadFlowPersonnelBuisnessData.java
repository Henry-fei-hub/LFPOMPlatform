...
public class BaseOnLoadFlowPersonnelBuisnessData extends GenericBase implements BaseFactory<BaseOnLoadFlowPersonnelBuisnessData>, Comparable<BaseOnLoadFlowPersonnelBuisnessData> 
{


	public static BaseOnLoadFlowPersonnelBuisnessData newInstance(){
		return new BaseOnLoadFlowPersonnelBuisnessData();
	}

	@Override
	public BaseOnLoadFlowPersonnelBuisnessData make(){
		BaseOnLoadFlowPersonnelBuisnessData b = new BaseOnLoadFlowPersonnelBuisnessData();
		return b;
	}
	...
	public final static java.lang.String ALL_CAPTIONS = "流程类型名称,主键编码,,主键编码,业务编码,业务名称,活动编码,流程编码,流程实列编码,附加节点，说明节点是预定义的节点还是动态加的节点， 0 - 正常定义的节点， 1 - 申请人或者审批人加的处理人或者审批人,当前流程实例节点的内部编码,下一个流程节点的编码,如果是知会节点，所依附的主节点,流程实例节点创建时间,流程实例节点开始处于活动时间,操作时间,提交意见,状态,节点类型 0 - 开始 1 - 结束 2 - 审批 3 - 处理 4 - 知会,返回数据集,人事业务编码,流程类型,处理人,工号,部门,归属公司,假期类型,开始日期,0上午   1下午,截止日期,0上午   1下午,天数,申请缘由,备注,地址,计划安排,交通工具,预计费用,外访单位,联系人,创建时间,删除状态  0未删除    1已删除,自定义的审核人,假期类型明细项,小时,公司所在省份,目标省份,多胞胎假请填写婴儿数量,附件,是否审批通过,假期类型明细项,项目类型 1项目 2前期项目,项目主键,项目名称,项目编号,父级编码  用于记录是根据哪一条记录进行延期,是否为边远城市,是否提供住宿,生活费用(元/月),住房费用(元/月),起草人,身份证,职务,对等号  与  冲账结合实现 借款与冲账与还款,关联表编码,离职补偿金,薪资1,薪资2,现薪资,月薪,基本比例（显示用）,基本比例（计算用）,月基本工资,月绩效工资,工龄工资,其他补贴,全薪,年度绩效,费用归属(1IT人员,2管理人员,3设计人员,4研发人员,5营销人员),领证时间,项目编码,付款序列编码,预算管理编码,预算附件编码,阶段id,,员工姓名,入职日期,所属公司,发起人";
	...
	public void cloneCopy(BaseOnLoadFlowPersonnelBuisnessData __bean){
	...
	@Override
	public int compareTo(BaseOnLoadFlowPersonnelBuisnessData o) {
	...
	public boolean equals(Object obj) {
		...
		final BaseOnLoadFlowPersonnelBuisnessData o = (BaseOnLoadFlowPersonnelBuisnessData)obj;
	...
	public Map<String, Object> toMap() {
		...
		if(getFee() != null) res.put("fee", getFee());
		if(getUnit() != null) res.put("unit", getUnit());
		if(getContactPerson() != null) res.put("contactPerson", getContactPerson());
		...
		if(getHours() != null) res.put("hours", getHours());
		if(getCompanyProvince() != null) res.put("companyProvince", getCompanyProvince());
		if(getTargetProvince() != null) res.put("targetProvince", getTargetProvince());
		if(getNumberOfBaby() != null) res.put("numberOfBaby", getNumberOfBaby());
		if(getAttachment() != null) res.put("attachment", getAttachment());
		if(getIsCompleted() != null) res.put("isCompleted", getIsCompleted());
		if(getSubTypeStr() != null) res.put("subTypeStr", getSubTypeStr());
		...
		if(getOriginalPay() != null) res.put("originalPay", getOriginalPay());
		if(getMonthPay() != null) res.put("monthPay", getMonthPay());
		if(getBasicProportionStr() != null) res.put("basicProportionStr", getBasicProportionStr());
		if(getBasicProportion() != null) res.put("basicProportion", getBasicProportion());
		if(getMonthBasicPay() != null) res.put("monthBasicPay", getMonthBasicPay());
		if(getMonthPerformancePay() != null) res.put("monthPerformancePay", getMonthPerformancePay());
		if(getForemanPay() != null) res.put("foremanPay", getForemanPay());
		if(getOtherSubsidy() != null) res.put("otherSubsidy", getOtherSubsidy());
		...
		if(getCertificationTime() != null) res.put("certificationTime", getCertificationTime());
		if(getProjectIds() != null) res.put("projectIds", getProjectIds());
