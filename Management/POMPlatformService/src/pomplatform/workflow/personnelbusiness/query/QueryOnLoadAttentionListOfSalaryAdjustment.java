...
public class QueryOnLoadAttentionListOfSalaryAdjustment extends AbstractQuery<BaseOnLoadAttentionListOfSalaryAdjustment, ConditionOnLoadAttentionListOfSalaryAdjustment>
{

	private static final Logger __logger = Logger.getLogger(QueryOnLoadAttentionListOfSalaryAdjustment.class);

	public QueryOnLoadAttentionListOfSalaryAdjustment() throws java.sql.SQLException 
	{
	...
	@Override
	public BaseCollection<BaseOnLoadAttentionListOfSalaryAdjustment> executeQuery( KeyValuePair[] replacements, ConditionOnLoadAttentionListOfSalaryAdjustment condition ) throws java.sql.SQLException {
	...
	@Override
	public BaseOnLoadAttentionListOfSalaryAdjustment generateBase(Object[] __data) throws java.sql.SQLException {
		BaseOnLoadAttentionListOfSalaryAdjustment __base = new BaseOnLoadAttentionListOfSalaryAdjustment();
		int count = 0;
		Object val;
	...
	private final static String __SQLText = "SELECT b.employee_id, b.employee_no, b.target_province, b.department_id, b.start_date, b.end_date, b.fee, convert_from(decrypt(CAST(b.original_pay as bytea),CAST((select image_path from system_config where system_config_id = 2) as bytea),'aes'),'SQL_ASCII') as original_pay, b.severance, convert_from(decrypt(CAST(b.month_pay as bytea),CAST((select image_path from system_config where system_config_id = 2) as bytea),'aes'),'SQL_ASCII') as month_pay, b.sub_type, b.personnel_business_id, a.system_process_attention_id, a.business_id, a.business_name, a.process_activity_id, a.process_id, a.process_instance_id, a.back_view_name, a.instance_activity_id, a.instance_activity_create_time, a.instance_activity_start_time, a.status, a.operate_time, a.process_comment, b.remark from system_process_attentions a LEFT JOIN personnel_business b on a.process_type = b.process_type and a.business_id = b.personnel_business_id and b.delete_flag = 0 where a.delete_flag = 0 and a.process_type = 30 and a.status = ? and (extract(year from b.end_date)) = ? and (extract(month from b.end_date)) = ? and b.target_province = ? and b.employee_id = ? and a.employee_id = ?" ;
	private final static String RESULTSETFIELDLIST = "employee_id,employee_no,target_province,department_id,start_date,end_date,fee,original_pay,severance,month_pay,sub_type,personnel_business_id,system_process_attention_id,business_id,business_name,process_activity_id,process_id,process_instance_id,back_view_name,instance_activity_id,instance_activity_create_time,instance_activity_start_time,status,operate_time,process_comment,remark";
