...
public class SettlementApplyWorker implements GenericWorkflowProcessor {
	...
	public int[] findActivityOwner(int processCreator, Integer employeeId, Integer departmentId, Integer roleId, Integer poolType) throws Exception {
		if(null != roleId){
			if(roleId.equals(StaticUtils.ROLE_MAIN_PROJECT_SECRETARY) || roleId.equals(StaticUtils.ROLE_MAIN_PROJECT_MANAGER) || 
...
//				pb.setConditionProjectId("=",bean.getProjectId());
//				pb.setConditionProjectCode("=",bean.getProjectCode());
//				pb.setConditionProjectName("=",bean.getProjectName());
//				pb.setConditionProcessType("=",81);
//				BasePersonnelBusines dao = pb.executeQueryOneRow();
				//发起人所在部门的部门经理
				if(!BaseHelpUtils.isNullOrEmpty(bean)) {
					Employee e = new Employee();
					e.setEmployeeId(bean.getDrafter());
					if(e.load()){
						return ProcessUtil.findNextOwner(processCreator, e.getEmployeeId(), e.getDepartmentId(), 3);
					}else{
						return new int[0];
					}
				}else {
	...
	private void dataValidate(PersonnelBusines dao, boolean isUpdate) throws Exception{
		...
		if(BaseHelpUtils.isNullOrEmpty(dao.getSubType())) {
			throw new Exception("请填写产值申请数量");
		}
		if(BaseHelpUtils.isNullOrEmpty(dao.getStartDate())){
			throw new Exception("请填写配合时间");
		}
