...
import pomplatform.workflow.dao.ProjectNormalReimbursementWithPn;
import pomplatform.workflow.personnelbusiness.business.SendMSGCommon;

public class PreProjectNormalReimbursementWorker implements GenericWorkflowProcessor {
	...
	public BusinessAbstract startNewWorkflow(SystemProcessDefinition pd, Map params, int employeeId) throws Exception {
		...
		NormalReimbursementLinkMainProject nrldDao = new NormalReimbursementLinkMainProject();
		nrldDao.setConditionProjectNormalReimbursementId("=", dao.getProjectNormalReimbursementId());
		List<BaseNormalReimbursementLinkMainProject> detailList = nrldDao.conditionalLoad();
		for (BaseNormalReimbursementLinkMainProject baseNormalReimbursementLinkDepartment : detailList) {
			if((null != baseNormalReimbursementLinkDepartment.getHaveElectronicInvoice() 
				...
				tmpMap.put("detailId", baseNormalReimbursementLinkDepartment.getNormalReimbursementLinkMainProjectId());
	...
	public BusinessAbstract executeActivity(ProcessInformation pi, Map params, int employeeId, boolean modified,
		...
		if(null == dao.getDetailProjectNormalReimbursementDetail()){
			...
			ProjectNormalReimbursementDetail detailDao = new ProjectNormalReimbursementDetail();
			detailDao.setConditionProjectNormalReimbursementId("=", dao.getProjectNormalReimbursementId());
			detailDao.conditionalDelete();
		}
		if(null == dao.getDetailNormalReimbursementLinkMainProject()){
			NormalReimbursementLinkMainProject linkDao = new NormalReimbursementLinkMainProject();
			linkDao.setConditionProjectNormalReimbursementId("=", dao.getProjectNormalReimbursementId());
			dao.setDetailNormalReimbursementLinkMainProject(linkDao.conditionalLoad());
		}
		dataValidate(dao);
		...
		if (completed) {
			if(pi.getProcessInstance().getProcessStatus().equals(SystemProcessConstants.PROCESS_STATUS_COMPLETED)){
				...
				if(pcrDao.countRows() <= 0){
					List<BaseNormalReimbursementLinkMainProject> list=dao.getDetailNormalReimbursementLinkMainProject();
					if(list.size()>0){
						...
						for(BaseNormalReimbursementLinkMainProject baseNormal:list){
							//保存部门成本
		...
		NormalReimbursementLinkMainProject nrldDao = new NormalReimbursementLinkMainProject();
		nrldDao.setConditionProjectNormalReimbursementId("=", dao.getProjectNormalReimbursementId());
		List<BaseNormalReimbursementLinkMainProject> oriDetailList = nrldDao.conditionalLoad();
		Set<Integer> oriDetailSet = new HashSet<>();
		for (BaseNormalReimbursementLinkMainProject baseNormalReimbursementLinkDepartment : oriDetailList) {
			oriDetailSet.add(baseNormalReimbursementLinkDepartment.getNormalReimbursementLinkMainProjectId());
		}
		dao.update();
...
//		}
	...
	public int[] findActivityOwner(int processCreator, Integer employeeId, Integer departmentId, Integer roleId,
		...
		if(BaseHelpUtils.getIntValue(roleId) != StaticUtils.ROLE_MAIN_PROJECT_MANAGER) {
			return ProcessUtil.findNextOwner(processCreator, employeeId, departmentId, roleId);
		}
		List<BaseNormalReimbursementLinkMainProject> list = bean.getDetailNormalReimbursementLinkMainProject();
		if (list == null || list.isEmpty()) {
			throw new SQLException("请选择一个项目！");
		}
		Set<Integer> ownerSet = new HashSet<>();
		for (BaseNormalReimbursementLinkMainProject b : list) {
			...
			if (!BaseHelpUtils.isNullOrEmpty(basePreProjectRe)) {
				ownerSet.add(BaseHelpUtils.getIntValue(dao.getProjectManager()));
			}
		}
		if (!ownerSet.isEmpty()) {
	...
	public void dataValidate(ProjectNormalReimbursementWithPn dao) throws Exception {
		...
		if (BaseHelpUtils.isNullOrZero(dao.getReimbursementType())) {
		...
		List<BaseNormalReimbursementLinkMainProject> normalReimbursementLinkMainProjectList = dao
				.getDetailNormalReimbursementLinkMainProject();
		if (null == normalReimbursementLinkMainProjectList || normalReimbursementLinkMainProjectList.isEmpty()) {
			throw new Exception("请添加需要承担本次报销金额的项目");
		}

		BigDecimal sum = BigDecimal.ZERO;
		BigDecimal inputTaxAndFinalAmount = BigDecimal.ZERO;
		for (BaseNormalReimbursementLinkMainProject obj : normalReimbursementLinkMainProjectList) {
