...
public class BaseNewContractDistribution extends GenericBase implements BaseFactory<BaseNewContractDistribution>, Comparable<BaseNewContractDistribution> 
{


	public static BaseNewContractDistribution newInstance(){
		return new BaseNewContractDistribution();
	}

	@Override
	public BaseNewContractDistribution make(){
		BaseNewContractDistribution b = new BaseNewContractDistribution();
		return b;
	}
	...
	public final static java.lang.String ALL_CAPTIONS = "合同,主订单编号,主订单名称,客户名称,开始日期,结束日期,结算状态,项目编号,合同类别,项目等级,项目总工期,签约总金额,总设计面积,合同单价,项目地点,合同跟进人,备注,合同签订日期,我方签约公司名称,创建时间,修改时间,业务类别,流转状态，默认为0表示可以提交到运营管理修改承接部门1表示可以提交到运营管理运营价格2表示可以提交到运营管理运营价格审核3表示审核完成,流水号,项目类别,建筑面积,配合专业,省份,城市,详细地址,客户地址,服务建议书,立单时间 提交了立单时间会将保存下来，只是点击保存不会将当前时间保存下来，单每次都会显示当前的时间，提交了下次显示的就是提交时的立单时间,标记补充协议字段；除0之外，其他都表示第n个补充协议,父级编码,信息编号,合同状态  0未启动、1待签署、2已签署、3未签搁置、4解除,客户合同编号,合同是否完成,是否为erp系统添加  true 为系统添加  false 为数据导入,操作人,确认函申请备注,财务合同编号,合同区域 system_dictionary_116,客户编码,合同信息审核无误,合同签订状态  1已寄出  2已签订,合同寄出日期,,实际参与分配的金额,是否原创,是否建筑院分包项目";

	public final static java.lang.String ALL_CAPTIONS_EXPORT = "合同编号,签订日期,合同名称,签约总额,业务类别,营销团队,是否原创,是否建筑院分包项目";
	...
	public void cloneCopy(BaseNewContractDistribution __bean){
...
//		sb.append(",");
//		String strStatus = delicacy.system.executor.SelectValueCache.getSelectValue("production_value_coefficient_type", String.valueOf(getStatus()));
...
//		sb.append(",");
		String strStatus = delicacy.system.executor.SelectValueCache.getSelectValue("production_value_coefficient_type", String.valueOf(getBusinessType()));
		sb.append(strStatus == null ? "" : strStatus);
		sb.append(",");
...
//		sb.append(getActualMoney() == null ? "" : getActualMoney());
	...
	@Override
	public int compareTo(BaseNewContractDistribution o) {
	...
	public boolean equals(Object obj) {
		...
		final BaseNewContractDistribution o = (BaseNewContractDistribution)obj;
