...
public class BaseSelectGridMyProjectValue extends GenericBase implements BaseFactory<BaseSelectGridMyProjectValue>, Comparable<BaseSelectGridMyProjectValue> 
{


	public static BaseSelectGridMyProjectValue newInstance(){
		return new BaseSelectGridMyProjectValue();
	}

	@Override
	public BaseSelectGridMyProjectValue make(){
		BaseSelectGridMyProjectValue b = new BaseSelectGridMyProjectValue();
		return b;
	}
	...
	public final static java.lang.String ALL_CAPTIONS = "项目编码,合同编号,项目编号,项目名称,项目类别,板块,设计类别,归属部门,设计阶段,项目等级,项目状态,专业负责人,预估工时,项目计划开始日期,项目计划结束日期,项目开始日期,项目结束日期,公区面积,后勤面积,核算比例,遗留项目折算,预算,项目总积分,预留积分,项目总金额,项目积分,项目进度,项目大小,项目描述,审核意见,部门经理,BIM面积,设计面积,手工计算积分,追加总积分,追加积分,备注,项目完成百分比,是否为630项目，0是   1不是       默认值为0,项目总完成百分比,2配合专业项目),事业部负责人,上次结算时的百分比,投诉折算比例,投诉保证金,统计营收板块,订单合同金额,创建时间,3管理项目),业务类别,合同编码,子合同编码,true是,合同金额(其实就是订单的合同金额，这字段已有，所以该字段弃用),),是否加锁";
	...
	public void cloneCopy(BaseSelectGridMyProjectValue __bean){
	...
	@Override
	public int compareTo(BaseSelectGridMyProjectValue o) {
	...
	public boolean equals(Object obj) {
		...
		final BaseSelectGridMyProjectValue o = (BaseSelectGridMyProjectValue)obj;
