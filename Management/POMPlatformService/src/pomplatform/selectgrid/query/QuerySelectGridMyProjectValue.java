...
public class QuerySelectGridMyProjectValue extends AbstractQuery<BaseSelectGridMyProjectValue, ConditionSelectGridMyProjectValue>
{

	private static final Logger __logger = Logger.getLogger(QuerySelectGridMyProjectValue.class);

	public QuerySelectGridMyProjectValue() throws java.sql.SQLException 
	{
	...
	@Override
	public BaseCollection<BaseSelectGridMyProjectValue> executeQuery( KeyValuePair[] replacements, ConditionSelectGridMyProjectValue condition ) throws java.sql.SQLException {
	...
	@Override
	public BaseSelectGridMyProjectValue generateBase(Object[] __data) throws java.sql.SQLException {
		BaseSelectGridMyProjectValue __base = new BaseSelectGridMyProjectValue();
		int count = 0;
		Object val;
	...
	private final static String __SQLText = "SELECT pj.* FROM projects pj WHERE pj.project_id IN ( SELECT project_id FROM project_employees WHERE employee_id = ? and project_id in (SELECT project_id from projects)) AND pj.contract_code LIKE ? AND pj.project_code LIKE ? AND pj.project_name LIKE ? AND pj.project_id = ? AND pj.project_type = ? AND pj.plate_id = ? AND pj.status = ? AND pj.contract_id = ? AND pj.project_setup_flag = ? AND pj.project_flag = ?" ;
	...
	private final static String[] fieldNames = { "employee_id", "contract_code", "project_code", "project_name", "project_id", "project_type", "plate_id", "status", "contract_id", "project_setup_flag","project_flag"};
}
