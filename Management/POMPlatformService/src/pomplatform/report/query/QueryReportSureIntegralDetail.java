...
public class QueryReportSureIntegralDetail extends AbstractQuery<BaseReportSureIntegralDetail, ConditionReportSureIntegralDetail>
{

	private static final Logger __logger = Logger.getLogger(QueryReportSureIntegralDetail.class);

	public QueryReportSureIntegralDetail() throws java.sql.SQLException 
	{
	...
	@Override
	public BaseCollection<BaseReportSureIntegralDetail> executeQuery( KeyValuePair[] replacements, ConditionReportSureIntegralDetail condition ) throws java.sql.SQLException {
	...
	@Override
	public BaseReportSureIntegralDetail generateBase(Object[] __data) throws java.sql.SQLException {
		BaseReportSureIntegralDetail __base = new BaseReportSureIntegralDetail();
		int count = 0;
		Object val;
	...
	private final static String __SQLText = "select aaa.*,bbb.plate_january_revenue,bbb.plate_february_revenue,bbb.plate_march_revenue,bbb.plate_april_revenue,bbb.plate_may_revenue,bbb.plate_june_revenue,bbb.plate_july_revenue,bbb.plate_august_revenue,bbb.plate_september_revenue,bbb.plate_october_revenue,bbb.plate_november_revenue,bbb.plate_december_revenue from (select aa.plate_id,aa.project_id,aa.contract_code as sheet_code,aa.project_name,aa.sheet_amount,aa.year,sum(one) as january_revenue,sum(two) as february_revenue ,sum(three) as march_revenue,sum(four) as april_revenue,sum(five) as may_revenue,sum(six) as june_revenue ,sum(seven) as july_revenue,sum(eight) as august_revenue ,sum(nine) as september_revenue,sum(ten) as october_revenue,sum(eleven) as november_revenue ,sum(twelf) as december_revenue from (select b.plate_id,b.project_id,b.contract_code,b.project_name,b.sheet_amount,EXTRACT(YEAR from a.settlement_date) as year,CASE WHEN EXTRACT(MONTH from a.settlement_date) = 1 then sum(a.total_amount) ELSE 0 END as one,CASE WHEN EXTRACT(MONTH from a.settlement_date) = 2 then sum(a.total_amount) ELSE 0 END as two,CASE WHEN EXTRACT(MONTH from a.settlement_date) = 3 then sum(a.total_amount) ELSE 0 END as three,CASE WHEN EXTRACT(MONTH from a.settlement_date) = 4 then sum(a.total_amount) ELSE 0 END as four,CASE WHEN EXTRACT(MONTH from a.settlement_date) = 5 then sum(a.total_amount) ELSE 0 END as five,CASE WHEN EXTRACT(MONTH from a.settlement_date) = 6 then sum(a.total_amount) ELSE 0 END as six,CASE WHEN EXTRACT(MONTH from a.settlement_date) = 7 then sum(a.total_amount) ELSE 0 END as seven,CASE WHEN EXTRACT(MONTH from a.settlement_date) = 8 then sum(a.total_amount) ELSE 0 END as eight,CASE WHEN EXTRACT(MONTH from a.settlement_date) = 9 then sum(a.total_amount) ELSE 0 END as nine,CASE WHEN EXTRACT(MONTH from a.settlement_date) = 10 then sum(a.total_amount) ELSE 0 END as ten,CASE WHEN EXTRACT(MONTH from a.settlement_date) = 11 then sum(a.total_amount) ELSE 0 END as eleven,CASE WHEN EXTRACT(MONTH from a.settlement_date) = 12 then sum(a.total_amount) ELSE 0 END as twelf from project_settlements a LEFT JOIN projects b on a.project_id = b.project_id WHERE a.settlement_status = 2 GROUP BY b.plate_id,b.project_id,b.contract_code,b.project_name,b.sheet_amount,EXTRACT(YEAR from a.settlement_date),EXTRACT(MONTH from a.settlement_date) ) as aa GROUP BY aa.plate_id,aa.project_id,aa.contract_code,aa.project_name,aa.sheet_amount,aa.year ORDER BY aa.plate_id,aa.year,aa.project_id)as aaa LEFT JOIN (select bb.plate_id,bb.project_id,bb.year,sum(one) as plate_january_revenue,sum(two) as plate_february_revenue ,sum(three) as plate_march_revenue,sum(four) as plate_april_revenue,sum(five) as plate_may_revenue,sum(six) as plate_june_revenue ,sum(seven) as plate_july_revenue,sum(eight) as plate_august_revenue ,sum(nine) as plate_september_revenue,sum(ten) as plate_october_revenue,sum(eleven) as plate_november_revenue ,sum(twelf) as plate_december_revenue from (select plate_id,project_id,EXTRACT(YEAR from account_date) as year,CASE WHEN EXTRACT(MONTH from account_date) = 1 then sum(account_integral) ELSE 0 END as one,CASE WHEN EXTRACT(MONTH from account_date) = 2 then sum(account_integral) ELSE 0 END as two,CASE WHEN EXTRACT(MONTH from account_date) = 3 then sum(account_integral) ELSE 0 END as three,CASE WHEN EXTRACT(MONTH from account_date) = 4 then sum(account_integral) ELSE 0 END as four,CASE WHEN EXTRACT(MONTH from account_date) = 5 then sum(account_integral) ELSE 0 END as five,CASE WHEN EXTRACT(MONTH from account_date) = 6 then sum(account_integral) ELSE 0 END as six,CASE WHEN EXTRACT(MONTH from account_date) = 7 then sum(account_integral) ELSE 0 END as seven,CASE WHEN EXTRACT(MONTH from account_date) = 8 then sum(account_integral) ELSE 0 END as eight,CASE WHEN EXTRACT(MONTH from account_date) = 9 then sum(account_integral) ELSE 0 END as nine,CASE WHEN EXTRACT(MONTH from account_date) = 10 then sum(account_integral) ELSE 0 END as ten,CASE WHEN EXTRACT(MONTH from account_date) = 11 then sum(account_integral) ELSE 0 END as eleven,CASE WHEN EXTRACT(MONTH from account_date) = 12 then sum(account_integral) ELSE 0 END as twelf from plate_project_account_records GROUP BY plate_id,project_id,EXTRACT(YEAR from account_date),EXTRACT(MONTH from account_date) ) as bb GROUP BY bb.plate_id,bb.year,bb.project_id) as bbb on aaa.plate_id=bbb.plate_id and aaa.year = bbb.year and aaa.project_id = bbb.project_id WHERE aaa.plate_id = ? and aaa.year = ? and aaa.sheet_code like ? and aaa.project_name like ?" ;
	private final static String RESULTSETFIELDLIST = "plate_id,project_id,sheet_code,project_name,sheet_amount,year,january_revenue,february_revenue,march_revenue,april_revenue,may_revenue,june_revenue,july_revenue,august_revenue,september_revenue,october_revenue,november_revenue,december_revenue,plate_january_revenue,plate_february_revenue,plate_march_revenue,plate_april_revenue,plate_may_revenue,plate_june_revenue,plate_july_revenue,plate_august_revenue,plate_september_revenue,plate_october_revenue,plate_november_revenue,plate_december_revenue";
	private final static String[] fieldNames = { "plate_id", "year", "sheet_code", "project_name"};
}
