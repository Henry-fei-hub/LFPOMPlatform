package pomplatform.client.query;
...
public class QueryMemployeeeor extends AbstractQuery<BaseMemployeeeor, ConditionMemployeeeor>
{

	private static final Logger __logger = Logger.getLogger(QueryMemployeeeor.class);

	public QueryMemployeeeor() throws java.sql.SQLException 
	{
	...
	@Override
	public BaseCollection<BaseMemployeeeor> executeQuery( KeyValuePair[] replacements, ConditionMemployeeeor condition ) throws java.sql.SQLException {
		return executeQuery(replacements
			);
	}

	@Override
	public BaseMemployeeeor generateBase(Object[] __data) throws java.sql.SQLException {
		BaseMemployeeeor __base = new BaseMemployeeeor();
		int count = 0;
		Object val;
		...
		if((val = __data[count++]) != null) __base.setBasicSalary(GenericBase.__getLong(val));
		if((val = __data[count++]) != null) __base.setJobTitleAllowance(GenericBase.__getLong(val));
		if((val = __data[count++]) != null) __base.setPostAllowance(GenericBase.__getLong(val));
		if((val = __data[count++]) != null) __base.setOther(GenericBase.__getLong(val));
		if((val = __data[count++]) != null) __base.setCommunicationSubsidy(GenericBase.__getLong(val));
		if((val = __data[count++]) != null) __base.setSubtotalMonthlyWage(GenericBase.__getLong(val));
		if((val = __data[count++]) != null) __base.setMonthlyPreissuedBonus(GenericBase.__getLong(val));
		if((val = __data[count++]) != null) __base.setTotalMonthlyRemuneration(GenericBase.__getLong(val));
		if((val = __data[count++]) != null) __base.setPhotoId(GenericBase.__getInt(val));
		if((val = __data[count++]) != null) __base.setLaborAttachmentId(GenericBase.__getInt(val));
		if((val = __data[count++]) != null) __base.setCardAttachmentId(GenericBase.__getInt(val));
		if((val = __data[count++]) != null) __base.setTechnicalAttachmentId(GenericBase.__getInt(val));
		if((val = __data[count++]) != null) __base.setEducationProofId(GenericBase.__getInt(val));
		if((val = __data[count++]) != null) __base.setDegreeProofId(GenericBase.__getInt(val));
		if((val = __data[count++]) != null) __base.setBankCardAttachment(GenericBase.__getString(val));
		if((val = __data[count++]) != null) __base.setBankCardAttachmentId(GenericBase.__getInt(val));
		if((val = __data[count++]) != null) __base.setRole(GenericBase.__getInt(val));
		return __base;
	}
